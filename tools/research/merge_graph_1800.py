#!/usr/bin/env python3
"""Merge the 1200-1800 dependency partials into docs/research/y1200/deps/graph_1800.json.

Mirrors tools/research/merge_graph_1200.py one block later.

Sources:
  docs/research/y1200/registry_1800.json           ids, lines, names, years, bands, research_years, redates
  docs/research/y1200/deps/partials/<group>.json   requires_all / requires_any / precedents / conditions / note
  docs/research/y1200/deps/partials/*_year_adjustments.json
...
tools/research/build_research_block.py on the game branch):
  docs/research/y1200/deps/graph_1800.json
  docs/research/y1200/deps/year_adjustments_1800.json   (auto-detected by the build tool)

Redates: an id listed in registry_1800.json `redates` moves INTO this block.
...
known (this block, 0-600 incl. adoptions, or 600-1200); no id repeats an
earlier id except recorded redates; every id the design graphs place before
1200 is baked in the game's blocks and vice versa; the combined 0-1800 graph is
...
list is a list; every proposed year is inside the window.
  ...
  python tools/research/merge_graph_1800.py            # merge + write
  python tools/research/merge_graph_1800.py --stats    # also print report statistics (JSON)
"""
import glob
import heapq
import json
...
OUTPUT = os.path.join(DEPS, "graph_1800.json")
ADJUSTMENTS = os.path.join(DEPS, "year_adjustments_1800.json")
WINDOW = (1200, 1800)
...
def main():
    ...
    amendments = git_json(AMENDMENTS_600)
    adopted = {}
    ...
    for block, path in BAKED.items():
        for item in git_json(path)["items"]:
    ...
    graph = {
        "meta": {
            "description": "Merged research dependency graph for game years 1200-1800 (12 lines), merged from "
                           "partials/{kicl,pils,nhde}.json by tools/research/merge_graph_1800.py. Edges run "
                           "prerequisite -> dependent; parents may be 0-600 ids (docs/research/deps/graph.json, plus the "
                           "game's adopted 0-600 items) or 600-1200 ids (docs/research/y600/deps/graph_1200.json). "
                           "proposed_year = target_year unless listed in year_adjustments_1800.json. "
                           ...
                           "one project at a time per line from year 1200, items in proposed_year order. "
                           "Redated ids (registry_1800 redates) are placed here and must leave their earlier block at bake.",
    ...
    with open(ADJUSTMENTS, "w", encoding="utf-8", newline="\n") as handle:
        json.dump({"description": "Year adjustments for graph_1800.json, combined from partials/*_year_adjustments.json. "
    ...
    if stats_wanted:
        ...
        out = {
            ...
            "spot": {i: {k: by_id[i][k] for k in ("proposed_year", "band_low", "band_high", "requires_all", "requires_any", "conditions")}
                     for i in ("printing_process", "blast_furnace", "black_powder", "ocean_sailing", "great_domed_temple") if i in by_id},
        }
