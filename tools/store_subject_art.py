...
source=Path(data['source']);assert source.is_file() and '.codex/generated_images/' in source.as_posix()
...
data['asset']='res://'+dest.relative_to(root).as_posix();data['sha256']=hashlib.sha256(dest.read_bytes()).hexdigest();data['mode']=data.get('mode','built-in image_gen / edit' if version>1 else 'built-in image_gen / new image');data['review']=data.get('review','pending visual audit')
