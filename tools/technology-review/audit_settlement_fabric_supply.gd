...
func run()->void:
	...
	var proposed:Array=preload("res://scripts/settlement_fabric_knowledge.gd").entries()
	...
	for filename:String in ["materials-construction-flight.json","buildings-settlements-depth.json"]:
	...
	for row:Dictionary in baseline.get("items",[]):
		...
		if reconciliation.get("editorial_source","") not in ["materials-construction-flight.json","buildings-settlements-depth.json"]:continue
