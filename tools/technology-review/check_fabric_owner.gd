...
func run()->void:
 ...
 WorldSimulation.scoped("fabric_owner",func()->void:
  ...
  state.population_allocations.Crafting=0
  WorldSimulation.military._process_equipment_production_day()
  assert(float(job.progress_days)==0.0)
  assert(float(state.resource_stockpiles.get("Building Shade Lattices",0))==0.0)
  state.population_allocations.Crafting=8
  state.population_allocations.Logistics=4
  # Exercise the real daily workforce allocator, not injected workshop work.
  for day in range(20,380):
   state.elapsed_days=day
   WorldSimulation.military._process_equipment_production_day()
   if float(state.resource_stockpiles.get("Building Shade Lattices",0))>=1.0:break
  ...
  var first_month:int=(int(state.elapsed_days)/30+1)*30
  state.elapsed_days=first_month
  model.process_month()
  var found:=false
  ...
  state.elapsed_days=first_month+30
  model.process_month()
  ...
  for day in range(first_month+60,first_month+331,30):
   state.elapsed_days=day
   model.process_month()
 ...
 WorldSimulation.clear()
 if not completed[0]:
  printerr("FAIL: fabric owner scenario stopped before completion")
  quit(1)
  return
 print("PASS: daily allocated workshop labor and zero-worker guard, autonomous settlement start, paid assembly/trial, installed result, duplicate-month guard and plot validation")
 quit()
