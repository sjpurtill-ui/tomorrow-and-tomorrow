...
func run()->void:
	...
	var result:=preload("res://tools/production_dependency_audit.gd").audit(load("res://scripts/civilian_industry.gd").PRODUCTS,load("res://scripts/technology_operations.gd").PLANTS,root.get_node("ResourceSystem").catalog.keys()+preload("res://scripts/household_clothing.gd").HUNTING_BYPRODUCTS.keys(),ids)
