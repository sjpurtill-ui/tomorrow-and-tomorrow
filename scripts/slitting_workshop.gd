...
static func advance(job:Dictionary,spec:Dictionary,work:float)->void:
	...
	if not job.has("slitting_pending"):
		...
		if spec.get("slitting_external",false):
			source=Source.take()
		...
		if not source.is_empty():job.slitting_pending.external_source=source
	...
	if float(p.work)>=1.0 and not p.has("piece"):p.piece=p.external_source.piece.duplicate(true) if spec.get("slitting_external",false) else S.prepared(float(spec.slitting_curvature))
	while p.readings.size()<3 and float(p.work)+.000001>=float(p.readings.size()+2):
		p.readings.append(S.reading(p.piece,p.readings.size()+1,float(spec.get("slitting_resolution",S.RESOLUTION))))
	if p.readings.size()!=3:return
	p.observation=S.measure(p.readings)
	var passed:=bool(p.observation.qualified)
	if spec.get("slitting_external",false):
		for stress:float in p.observation.get("profile",[]):
			if absf(stress)+float(p.observation.uncertainty)>.2:passed=false
	p.accepted=passed
	var output:=String(spec.output if passed else ("Stress-Rejected Steel Blanks" if spec.get("slitting_external",false) else "Unresolved Slitting Records"))
	state.resource_stockpiles[output]=float(state.resource_stockpiles.get(output,0))+(.8 if spec.get("slitting_external",false) else 1.0)
	...
	job.completed+=1;job.progress_days=0.0;job.last_output=int(job.get("last_output",0))+(1 if passed else 0)
...
static func validate_job(job:Dictionary,spec:Dictionary)->String:
	...
	for key:String in ["slitting_pending","slitting_last"]:
		...
		if spec.get("slitting_external",false) and (not Source.valid_source(p.get("external_source")) or not p.external_source.consumed or p.external_source.site!=p.site):return "Invalid external slitting workpiece."
		if float(p.work)>=1.0:
			if p.get("piece")!=(p.external_source.piece if spec.get("slitting_external",false) else S.prepared(float(spec.slitting_curvature))):return "Invalid slitting preparation."
		...
		for index:int in range(p.readings.size()):
			if p.readings[index]!=S.reading(p.piece,index+1,float(spec.get("slitting_resolution",S.RESOLUTION))):return "Invalid strain release record."
		if finished:
			...
			if spec.get("slitting_external",false):
				for stress:float in p.observation.get("profile",[]):
					if absf(stress)+float(p.observation.uncertainty)>.2:passed=false
			if p.get("accepted")!=passed:return "Invalid residual stress decision."
