...
static func recommendation(plan:Dictionary={})->Dictionary:
	...
	if not Purchase.available() or not world.diplomatic_mission.is_empty() or state.effective_workers("Knowledge")<=0:return {}
	...
	for item:Dictionary in E.data().collections.values():
		...
		for gift:Dictionary in world.diplomatic_gift_options(source):
			...
			var quote:=Purchase.quote(source,subject,String(gift.resource))
			...
			if score>best_score:
				best_score=score;best={"kind":"research_purchase","source":source,"subject":subject,"resource":String(gift.resource)}
	return best
