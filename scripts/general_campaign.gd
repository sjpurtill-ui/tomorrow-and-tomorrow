...
func reset_for_new_world()->void:
	...
	screen=null
	if WorldSimulation.system("GeneralDialogue")!=null:WorldSimulation.general_dialogue.reset()
...
func start_scenario()->Dictionary:
	active=true
	WorldSimulation.state.select_founding_focus("provision")
	WorldSimulation.direction.choose(String(PeopleDirection.AMBITIONS.keys()[0]))
	WorldSimulation.state.settlement_site_committed=true;WorldSimulation.state.settlement_name="Alderford"
	WorldSimulation.state.settlement_founded_at=terrain.settler_marker.position
	WorldSimulation.state.settlement_completed=["Hearth Circle"]
	WorldSimulation.state.population_exact=1200;WorldSimulation.state.population_total=1200;WorldSimulation.state.population_cohorts.clear();WorldSimulation.state.initialize_population_model()
	WorldSimulation.state.synchronize_population_allocations()
	WorldSimulation.settlements.ensure_founded()
	WorldSimulation.food.receive_external_food(24000)
	...
	WorldSimulation.military.military_inventory["spear"]=180;WorldSimulation.military.military_inventory["bow"]=60
	WorldSimulation.military.home_army=WorldSimulation.military.simulator.create_formation_force("Alderford reserve",[],.9,.85)
	WorldSimulation.military._assemble_field_army(formations,"Alderford expedition")
	var army:Dictionary=WorldSimulation.military.field_armies[-1]
	army["general_managed"]=true;army["supply_level"]=1.0
	var leader:Dictionary=army.commander
	state={"seed":551188,"army_id":int(army.army_id),"origin":Vector2(WorldSimulation.state.settlement_founded_at.x,WorldSimulation.state.settlement_founded_at.z),"cell":Vector2i.ZERO,"general_name":String(leader.name),"figure_id":String(leader.get("figure_id","")),"character":{"care":.8,"ambition":.48,"confidence":.65,"loyalty":.6,"fear":.25,"affection":.55,"competence":float(leader.get("tactics",.6)),"resentment":0.0},"treatment":[],"messages":[],"reports":[],"rivals":[],"mission":{},"proposal":{},"status":"deliberating","food":240.0*8.0*RATION,"initial":240,"losses":0,"turn":0,"battle":{},"battle_initial":[],"war":"The Alderford War","outcome":"","seen":{},"events":[],"exhaustion":0.0,"authority":"Personal oath; removal needs the captains' cooperation."}
	WorldSimulation.food.issue_for_obligation(float(state.food),"military","Alderford carried rations",8,240)
	_build_grid()
	...
	WorldSimulation.world.civilizations.resize(2);WorldSimulation.world.foreign_formations.clear()
	for civ:Dictionary in WorldSimulation.world.civilizations:civ.relations.clear()
	WorldSimulation.world._initialize_relations(551188)
	for i in 2:
		var civ:Dictionary=WorldSimulation.world.civilizations[i]
		var home:Vector2i=chosen[i];var pos:=world_position(home)
		civ.name=["Bracken Hold","Mere Council"][i];civ.world_position=pos
		civ.population=1000.0;civ.cohorts=WorldSimulation.world._cohorts_for_population(1000.0);civ.military_population=160.0;civ.food_capacity=1100.0
		...
		var force:Dictionary=WorldSimulation.military.simulator.create_formation_force(String(civ.name)+" guard",[{"unit":"line_infantry","weapon":"spear","count":160,"equipment":160,"training":.58,"experience":.18}],.88,.8)
	...
	for formation:Dictionary in WorldSimulation.world.foreign_formations:formation["disabled_until_day"]=2147483647
	WorldSimulation.world._rebuild_competition()
	WorldSimulation.world.city_intelligence.seed_known_homes()
	WorldSimulation.world._add_revealed_area(state.origin,50,"River war: established local geography")
...
func _build_grid()->void:
	...
	for x in range(-RADIUS,RADIUS+1):
		for y in range(-RADIUS,RADIUS+1):
			var c:=Vector2i(x,y)
			if WorldSimulation.world._scout_land_at(world_position(c)):cells[c]=true
...
func route(start:Vector2i,goal:Vector2i,avoid:Array[Vector2i]=[])->Array[Vector2i]:
	...
	while cursor<queue.size():
		...
		for offset in [Vector2i.LEFT,Vector2i.RIGHT,Vector2i.UP,Vector2i.DOWN]:
			...
			if cells.has(next) and (next==goal or next not in avoid) and not parents.has(next) and not WorldSimulation.military.field_route_availability(world_position(c),world_position(next)).has("error"):
				parents[next]=c;queue.append(next)
	return result

func army()->Dictionary:
	var index:=WorldSimulation.military._field_army_index(int(state.get("army_id",-1)))
	return WorldSimulation.military.field_armies[index] if index>=0 else {}
...
func commit_proposal(insist:=false)->Dictionary:
	...
	if decision.has("error"):
		state.treatment.append({"day":WorldSimulation.state.elapsed_days,"event":decision.kind,"order":order.duplicate(true)})
	...
	if order.action=="relieve":
		...
		state.treatment.append({"day":WorldSimulation.state.elapsed_days,"event":"command replaced"})
		var replacement:=WorldSimulation.figures.commander(WorldSimulation.military._acting_field_commander(false),"campaign_replacement_%d"%state.treatment.size())
...
func _start_step()->void:
	...
	if action in ["withdraw","recover","defend"]:
		for report:Dictionary in state.seen.values():
			if report.get("control","rival")=="rival" and WorldSimulation.state.elapsed_days-float(report.day)<3:avoid.append(report.cell)
...
func _rival_decision(r:Dictionary,duration:float)->Vector2i:
	...
	if reconsider:
		...
		if difficulty!="easy" and r.adaptations in ([2,5,9] if difficulty=="hard" else [3,8]):WorldSimulation.general_dialogue.request_opponent(r.id)
...
func _finish_step()->void:
	...
	for r:Dictionary in state.rivals:
		...
		if r.cell.distance_to(state.cell)<=3:
			state.seen[String(r.id)]={"name":r.name,"home":r.home,"cell":r.cell,"troops":int(r.force.troops),"day":WorldSimulation.state.elapsed_days,"control":r.control}
			r.seen={"cell":state.cell,"troops":int(a.get("troops",0)),"day":WorldSimulation.state.elapsed_days}
	...
	if step.kind in ["recover","resupply"] and state.cell==Vector2i.ZERO:
		...
		state.food+=WorldSimulation.food.issue_for_obligation(needed,"military","General replenishes carried rations",8,int(a.troops))
	...
	if state.cell==Vector2i.ZERO and state.mission.action=="defend" and food_days()<3:
		state.food+=WorldSimulation.food.issue_for_obligation(maxf(0,int(a.troops)*RATION*8-float(state.food)),"military","General replenishes home defense",8,int(a.troops))
...
func _secure_home(r:Dictionary,home_result:Dictionary,enemy_result:Dictionary)->bool:
	for civ:Dictionary in WorldSimulation.world.civilizations:
		...
		var captured:=WorldSimulation.world._capture_region(civ,String(region.id),survivors,enemy_result)
		if bool(captured.outcome.get("region_captured",false)):
			WorldSimulation.military.establish_occupation_force(String(r.id),captured.outcome.region,float(captured.outcome.occupation_required),int(state.army_id))
...
func _battle(id:String)->void:
	...
	for civ:Dictionary in WorldSimulation.world.civilizations:
		if civ.id==r.id:
			WorldSimulation.world._remove_foreign_scout_population(civ,killed,true);civ.military_population=int(r.force.troops)
	...
	if state.has("figure_id"):
		var person:=WorldSimulation.figures.by_id(String(state.figure_id))
		if not person.is_empty() and person.get("status","living")!="living":
			var replacement:=WorldSimulation.figures.commander(WorldSimulation.military._acting_field_commander(false),"army_%d"%int(state.army_id))
			army().commander=replacement;state.general_name=replacement.name;state.figure_id=replacement.get("figure_id","")
			state.treatment.append({"day":WorldSimulation.state.elapsed_days,"event":"command succeeded after battle","previous":person.name,"successor":replacement.name})
	WorldSimulation.military.battle_history.push_front(result.duplicate(true))
	if WorldSimulation.military.battle_history.size()>40:WorldSimulation.military.battle_history.resize(40)
	...
	var seen:Dictionary=state.seen[String(r.id)];seen.troops=int(r.force.troops);seen.cell=r.cell;seen.day=WorldSimulation.state.elapsed_days;seen["control"]=r.control
	r.memory.append({"event":String(result.outcome),"day":WorldSimulation.state.elapsed_days,"losses":int(enemy.troops)-int(r.force.troops)})
...
func _report(text:String,halt:bool=true)->void:
	...
	if state.get("outcome","")!="":
		for civ:Dictionary in WorldSimulation.world.civilizations:
			civ.player_relation.at_war=false;civ.player_relation.treaty="truce";civ.player_relation.truce_until_day=int(WorldSimulation.state.elapsed_days)+365
	state.reports.append({"day":WorldSimulation.state.elapsed_days,"text":text})
...
func export_state()->Dictionary:return {"active":active,"difficulty":difficulty,"state":state.duplicate(true),"dialogue":WorldSimulation.general_dialogue.export_state(),"budget":budget,"resolving":resolving,"pending_step":pending_step.duplicate(true)}
func import_state(data:Dictionary)->Dictionary:
	...
	if resolving:state.status="paused during execution"
	WorldSimulation.general_dialogue.import_state(data.get("dialogue",{}))
	return {"ok":true}
