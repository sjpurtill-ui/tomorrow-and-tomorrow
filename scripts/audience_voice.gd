...
const ENVOY_OPEN:={
	"gift":[
		"{oath} {leader} sends you {amt} {res}, {address}. No trade, no debt; {leader} wants you thinking well of {civ}.",
		"We hauled {amt} {res} across bad country to bring it here, {address}. It's yours, from {leader}, and my feet would like it noticed.",
		"{leader} told me to hand you {amt} {res} and ask for nothing back, {address}, so that's what I'm doing.",
		"A gift from {civ}, {address}: {amt} {res}. {leader} wants nothing for it but a friendly border.",
...
const COURT_KIND:={
	"gift":[
		...
		"{civ} will likely ask a favour later, {address}. Take it, but decide now what you'd say when they do.",
		"{amt} {res} for nothing, {address}? Then {civ} wants something from us next season, and I'd like to know what.",
		"Accept it, {address}. Sending a gift back would insult {civ}, and we don't need that quarrel.",
	],
	"request":[
...
func _command_instruction(s:Dictionary,extra:Dictionary)->String:
	...
	parts.append(_gate_words(s).strip_edges()+" Nobody refuses or undoes anything unless REFUSED is written above. The dead and the removed never speak. No miracles. mood_shift 0.")
	return " ".join(parts)
