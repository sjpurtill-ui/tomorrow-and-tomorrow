...
static func evidence(work:float)->Dictionary:
	...
	for index:int in range(count):
		var point:=F.sample(index,float(r.initial_front[1]),r.geometry)
		r.trace.append({"force":point.force,"opening":point.opening})
	if work+0.000001>=4.5:
		var extension:=float(F.sample(14,float(r.initial_front[1]),r.geometry).crack_extension)
		r.final_front=[]
...
static func advance(job:Dictionary,spec:Dictionary,work:float)->void:
	...
	job.last_work=float(job.last_work)+used;job.progress_days=float(p.work);p.evidence=evidence(float(p.work))
	if float(p.work)+.000001<4.5:return
	p.report=F.measure(p.evidence)
	var output:=String(spec.output if p.report.qualified else "Comparison Fracture Records")
...
static func validate_job(job:Dictionary,spec:Dictionary)->String:
	...
	for key:String in ["fracture_pending","fracture_last"]:
		...
		if p.evidence!=evidence(float(p.work)):return "Invalid fracture specimen evidence."
		if finished:
