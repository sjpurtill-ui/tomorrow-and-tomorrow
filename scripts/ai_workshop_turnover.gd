extends RefCounted
## An AI workshop may yield after a paid ordinary batch, never during a trial.
const P=preload("res://scripts/persistent_production.gd")
const I=preload("res://scripts/civilian_industry.gd")
const ORDINARY_FIELDS=["name","output","gate","materials","days","tooling","co_products","power","daily_power","services"]

static func request(id:String,host:Node,item:String,target:int)->void:
	if String(WorldSimulation.actors.get(id,{}).get("controller",""))!="ai":return
...
static func advance(id:String,host:Node)->void:
	if String(WorldSimulation.actors.get(id,{}).get("controller",""))!="ai":return
	for job:Dictionary in host.equipment_queue:
