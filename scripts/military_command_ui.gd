...
func _refresh()->void:
	...
	inventory.text=_inventory_summary(army)+"\n\nLogistics %d%%  •  Field supply %d%%\nLabor withheld %d  •  Workshop diversion %d%%\nPublic upkeep basis %.2f/day" % [roundi(float(capabilities.get("logistics_practice",0.0))*100.0),roundi(MilitaryCampaign.field_provision_delivery_ratio()*100.0),int(burden.get("mobilized_citizens",0)),roundi(float(burden.get("workshop_diversion",0.0))*100.0),float(burden.get("currency_upkeep_units",0.0))]
