...
func _calculate_aggregate_demand(traveling: bool) -> Dictionary:
	...
	var military_campaign:Node=get_node_or_null("/root/MilitaryCampaign") if GameState.resource_settlement_id=="" else null
	if military_campaign!=null and GameState.resource_settlement_id=="":
...
func _produce(workers: float,labor_efficiency: float,ecology: float,traveling: bool) -> Dictionary:
	...
	var coastal_profile:Dictionary={}
	var settlement_model:=get_node_or_null("/root/SettlementModel")
	if not traveling and settlement_model!=null and settlement_model.has_method("settlement_intrinsic_profile"):
		coastal_profile=settlement_model.settlement_intrinsic_profile()
	# Shore fishing is a modest additional subsistence option, not an automatic
	# stockpile or a free maritime route. The bounded share comes from the site
	# profile and disappears for traveling parties.
	var fishing_weight:float=(0.16 if float(access.freshwater)>0.0 else 0.0)+float(coastal_profile.get("marine_food_share",0.0))
	fishing_weight=clampf(fishing_weight,0.0,0.22)
	...
	result["Fresh plants"]=workers*gathering_weight*4.55*terrain_gather*plant_season*efficiency*ecological*float(GameState.food_source_health.get("Wild gathering",0.9))*practice*variation*route_factor*float(coastal_profile.get("foraging_factor",1.0))
	...
	result["Fish"]=workers*fishing_weight*5.00*fish_season*efficiency*float(GameState.food_source_health.get("Fishing",0.9))*(0.82+float(access.freshwater)*0.28)*practice*variation*route_factor*float(coastal_profile.get("early_food_factor",1.0))
