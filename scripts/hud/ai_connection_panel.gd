...
func _ready()->void:
	...
	var note:=Label.new();note.text="Use your API key on this device for civics and leader conversations. It stays in memory for this game session.";note.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART;root.add_child(note)
	key=_field(root,"API key","");key.secret=true;key.placeholder_text="Paste key here · never in chat";key.max_length=8192
...
func _apply()->void:
	var result:=PronouncementInterpreter.configure_connection(key.text,model.text,endpoint.text)
	key.clear();status.text=String(result.get("error",result.get("message","")));_fit.call_deferred()
