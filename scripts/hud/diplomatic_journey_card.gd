...
func _ready()->void:
	...
	var flag:=TextureRect.new();flag.texture=preload("res://scripts/city_map_identity.gd").foreign(String(status.get("civ_id",""))).texture;flag.custom_minimum_size=Vector2(42,42);flag.expand_mode=TextureRect.EXPAND_IGNORE_SIZE;flag.stretch_mode=TextureRect.STRETCH_KEEP_ASPECT_CENTERED;title.add_child(flag)
