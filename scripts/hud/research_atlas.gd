...
func _ready()->void:
	...
	var dismiss:=ColorRect.new();dismiss.color=Color(0.12,0.10,0.08,.55);dismiss.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT);add_child(dismiss)
	...
	# An opaque sheet over the scrim: nothing shows through a panel.
	var sheet_bg:=T.PANEL_BG;sheet_bg.a=1.0
	panel=PanelContainer.new();panel.mouse_filter=Control.MOUSE_FILTER_STOP;panel.add_theme_stylebox_override("panel",T.flat(sheet_bg,T.BORDER,1,4,16));add_child(panel)
