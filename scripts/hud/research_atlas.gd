...
func select(id:String,open_detail:bool=false)->void:
	...
	for item:Dictionary in records:
		...
		if item.exposed:
			...
			if not definition.get("preservation_profile",{}).is_empty() or not definition.get("training_profile",{}).is_empty():Art.label(detail_body,WorldSimulation.discovery._discovery_effect_summary(definition),11,T.TEXT_SOFT,true)
		var assignment:Dictionary=item.assignment
