...
func setup(block:Dictionary)->void:
	...
	if not money and not metal:
		...
		for item:Dictionary in data.accounts:
			...
			var name_label:=_serif(String(item.name),20);name_label.size_flags_horizontal=Control.SIZE_EXPAND_FILL;name_label.text_overrun_behavior=TextServer.OVERRUN_TRIM_ELLIPSIS;row.add_child(name_label)
