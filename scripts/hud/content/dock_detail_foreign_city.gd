...
func tab(_sub:int)->Dictionary:
	...
	for key in ["population","garrison","fortification","supply","damage","production","logistics"]:
		...
		if not field.is_empty():
			...
			value="%d–%d %s" % [roundi(float(field.low)*scale),roundi(float(field.high)*scale),"%" if scale>1 else String(INTEL.FIELDS[key].unit)]
		rows.append({"name":String(INTEL.FIELDS[key].label),"value":value,"sub":"Stale estimate" if bool(field.get("stale",false)) else "","accent":Tokens.AMBER,"tip":"Only returned observations are shown."})
