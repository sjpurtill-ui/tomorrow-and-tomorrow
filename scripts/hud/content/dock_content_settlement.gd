...
func _city_tab(sub:int)->Dictionary:
	...
	if settlement_id.is_empty() and sub==0:
		...
		if committed:items.append({"label":"FOUNDING WORK","sub":"Progress toward the Hearth Circle","on_press":jump("construction",0)})
			...
			if traveling:items.push_front({"label":"HALT HERE","sub":"Override the caravan leader and hold","primary":false,"on_press":terrain._halt_founding_convoy_to_forage,"tip":"The caravan leader normally decides when to camp. Halting overrides that: the party holds (moving to water first if there is none here) until you order it on."})
			elif led_mode in ["held","watering","foraging","resting","provisioning"]:items.push_front({"label":"MARCH ON","sub":"Override the leader's camp and continue","primary":led_mode=="held","on_press":func()->void:terrain._on_caravan_override("founding","resume"),"tip":"The caravan leader resumes on its own when water, food and health allow. This orders the march to continue now."})
