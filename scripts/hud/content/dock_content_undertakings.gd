...
func _local(id:String)->Dictionary:
	...
	for r:Dictionary in city.get("undertakings",[]):
		...
		if r.status=="functioning":
			blocks.append({"type":"text","heading":"CURRENT CONTRIBUTION","text":"+%.1f%% local %s effectiveness at its present condition. Staff reserve upkeep labor and materials; lasting recognition follows sustained operation." % [float(d.bonus)*float(r.condition)*100,String(d.role).to_lower()]})
