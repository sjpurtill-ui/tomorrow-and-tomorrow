...
static var icon_cutout:ShaderMaterial
...
static func icon(index:int)->TextureRect:
	...
	return symbol(Rect2(15,ys[index],47,40),40,36)
static func symbol(rect:Rect2,width:float,height:float)->TextureRect:
	if icon_cutout==null:
		icon_cutout=ShaderMaterial.new()
		icon_cutout.shader=preload("res://scripts/hud/nav_art_cutout.gdshader")
	var image:=picture(rect,width,height)
	image.material=icon_cutout
