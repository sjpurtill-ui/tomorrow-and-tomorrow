...
func _draw()->void:
	...
	for item:Dictionary in owner_view.records:
		...
		var image_rect:=Rect2(at(origin+Vector2(2,2)),Vector2(CARD.x-4,70)*zoom_level)
		if texture:
			draw_texture_rect_region(texture,image_rect,Art.crop_region(texture,image_rect.size,Art.focus_for(item)))
		else:
			draw_rect(image_rect,Color("102027"))
