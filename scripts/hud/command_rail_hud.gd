...
func _layout()->void:
	...
	if dock:
		...
		dock.set_deferred("size",Vector2(minf(_work_queue_width(view.x) if active_section in ["production","construction","economy","settlement"] else Tokens.DOCK_WIDTH,view.x-Tokens.DOCK_X-12),view.y-64-Tokens.DOCK_MARGIN_Y))
	if detail_dock:
...
func force_dock_layout()->void:
	...
	for panel in [dock,detail_dock]:
		...
		panel.size=Vector2(minf(_work_queue_width(view.x) if panel==dock and active_section in ["production","construction","economy","settlement"] else Tokens.DOCK_WIDTH,view.x-Tokens.DOCK_X-12),view.y-64-Tokens.DOCK_MARGIN_Y)
		for sort_pass in 3:
			panel.propagate_notification(Container.NOTIFICATION_SORT_CHILDREN)
