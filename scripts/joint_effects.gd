...
func advance(day:int)->void:
	for army:Dictionary in [op.host.home_army]+op.host.field_armies:
		...
		if army==op.host.home_army:
			var city=CivilizationSystem.city_intelligence.site(CivilizationSystem.city_intelligence.primary_id("player"))
	...
	for force:Dictionary in op.state.forces:
		...
		if force.mission=="reconnaissance":
			for city:Dictionary in CivilizationSystem.city_intelligence.sites():
				if same_region(op.region_at(G.unpack(city.position),"air"),force.region):CivilizationSystem.city_intelligence.publish(String(force.owner),CivilizationSystem.city_intelligence.capture(String(force.owner),String(city.city_id),.65,day,"air reconnaissance",str(force.id)),day)
		if force.mission not in ["strategic_bombing","logistics_strike","invasion_support"]:continue
		var targets:Array=CivilizationSystem.city_intelligence.known_cities(String(force.owner),"",false)
		for city:Dictionary in targets:
			...
			if String(city.civ_id)=="player":
				SettlementModel.with_city_resources(String(city.city_id),func():
...
func _supply(force:Dictionary)->void:
	...
	for army:Dictionary in op.host.field_armies:
		...
		var amount:float=SettlementModel.with_city_resources(String(origin.city_id),func():return FoodSystem.issue_for_obligation(minf(need,capacity),"military","Air-dropped field supplies",0,0))
	...
	for city:Dictionary in GameState.player_settlements:
		...
		var amount:float=SettlementModel.with_city_resources(String(origin.city_id),func():return FoodSystem.issue_for_obligation(capacity,"military","Air supply to "+String(city.name),0,0))
		SettlementModel.with_city_resources(String(city.id),func():FoodSystem.receive_external_food(amount))
		return
