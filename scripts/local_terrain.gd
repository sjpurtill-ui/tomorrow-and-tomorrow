...
var settlement_selector:OptionButton
...
func _live_report_signature(kind:String)->String:
	var parts:Array=[int(floor(GameState.elapsed_days)),GameState.population_total,GameState.known_discoveries.size(),GameState.discovery_log.size(),GameState.selected_settlement_id,GameState.next_city_trade_id,GameState.city_trade_shipments.size()]
	match kind:
		"provisions":
...
func _adopt_live_report_contents(stable_root:Control,replacement_root:Control)->void:
	...
	_apply_modal_screen_contract_by_id.call_deferred(stable_root.get_instance_id())
...
func _discovery_context() -> Dictionary:
	...
	if settler_marker:
		...
		if GameState.resource_settlement_id!="":
			var city:Dictionary=_settlement_model().settlement_record(GameState.resource_settlement_id)
			var point:Vector2=city.get("position",Vector2.ZERO)
			origin=Vector3(point.x,0.0,point.y)
...
func _settlement_map_label_text(zoom:float)->String:
	var name:=_primary_settlement_display_name().to_upper()
...
func _resource_overlay_view_key()->String:
	if camera==null: return GameState.selected_settlement_id+":"+"no-camera"
...
func _refresh_settlement_footprint(force := false) -> void:
	...
	if not founded:
		return
	if settlement_map_label:
		settlement_map_label.text="%s  •  %s" % [_primary_settlement_display_name().to_upper(),_compact_population(roundi(_settlement_model().primary_population_exact()))]
...
func _process_settlement_day() -> void:
	_settlement_model().with_local_population(_process_settlement_day_local)

func _process_settlement_day_local() -> void:
	...
	if project.is_empty():
		if GameState.resource_settlement_id=="": _update_settlement_progress_text()
		return
...
func _process_settlement_convoy()->void:
	...
	if bool(completed.get("ok",false)):
		...
		GameState.simulation_events.push_front(event)
		if GameState.simulation_events.size()>80: GameState.simulation_events.resize(80)
		_initialize_city_resource_sites(String(settlement.id))
		_select_city(String(settlement.id))
	_update_time_interface()
...
func _schedule_modal_screen_contract(node:Node)->void:
	if node is Control: _apply_modal_screen_contract_by_id.call_deferred(node.get_instance_id())

func _apply_modal_screen_contract_by_id(instance_id:int)->void:
	var screen:=instance_from_id(instance_id) as Control
	if is_instance_valid(screen): _apply_modal_screen_contract(screen)
...
func _build_interface() -> void:
	...
	top_bar.size = Vector2(viewport_width, 46)
	top_bar.color = Color("#0a1114f5")
	layer.add_child(top_bar)
	var top_rule := ColorRect.new()
	top_rule.position = Vector2(0, 45)
	top_rule.size = Vector2(viewport_width, 1)
	top_rule.color = Color("#70664f")
	layer.add_child(top_rule)
	world_header_label=Label.new()
	world_header_label.position = Vector2(12, 8)
	world_header_label.size = Vector2(164, 30)
	world_header_label.text = GameState.province_name.to_upper()
	world_header_label.clip_text = true
	world_header_label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	world_header_label.add_theme_font_size_override("font_size", 12)
	world_header_label.add_theme_color_override("font_color", Color("#ded6c4"))
	layer.add_child(world_header_label)
	settlement_selector=OptionButton.new()
	settlement_selector.name="CitySelector"
	settlement_selector.position=Vector2(12,52)
	settlement_selector.size=Vector2(230,32)
	settlement_selector.fit_to_longest_item=false
	settlement_selector.add_theme_font_size_override("font_size",12)
	settlement_selector.tooltip_text="Select a city to view its own stores and move the map there."
	settlement_selector.item_selected.connect(_on_city_selected)
	layer.add_child(settlement_selector)
	population_summary_label = Button.new()
	population_summary_label.name="NavSettlement"
	# Five direct destinations share one predictable navigation row. Status values
	# live in tooltips and inside their dashboard instead of multiplying permanent
	# top-bar widgets.
	population_summary_label.position = Vector2(viewport_width-600, 8)
	population_summary_label.size = Vector2(92, 30)
	...
	population_summary_label.tooltip_text = "Open settlement population, labor, health, housing, and commitments."
	population_summary_label.pressed.connect(_open_settlement_dashboard)
	population_summary_label.add_theme_font_size_override("font_size", 10)
	population_summary_label.add_theme_color_override("font_color", Color("#aaa99f"))
	population_summary_label.add_theme_stylebox_override("normal",_hud_chip_style(Color("#6f7772")))
	population_summary_label.add_theme_stylebox_override("hover",_hud_chip_style(Color("#a0aaa3"),true))
	...
	date_label.size = Vector2(138, 30)
	...
	date_label.tooltip_text="Calendar and current hour. Use the adjacent controls or keys 0–5 to change time speed."
	layer.add_child(date_label)
	var speed_row:=HBoxContainer.new()
	speed_row.position=Vector2(324,8)
	speed_row.size=Vector2(158,30)
	...
	for speed_entry in [[0,"Ⅱ","Pause"],[1,"1","0.5 hour per second"],[2,"2","2 hours per second"],[3,"3","8 hours per second"],[4,"4","1 day per second"],[5,"5","3 days per second"]]:
		...
		speed_button.custom_minimum_size=Vector2(24,30)
	...
	travel_status_label.position = Vector2(viewport_width * 0.5 - 390, 48)
	...
	menu_button.name="UtilityMenu"
	menu_button.position=Vector2(viewport_width-48,8)
	menu_button.size=Vector2(36,30)
	menu_button.text="MENU"
	menu_button.tooltip_text="Pause, view controls, restart, or begin a new world."
	menu_button.add_theme_font_size_override("font_size",8)
	menu_button.pressed.connect(_open_world_menu)
	layer.add_child(menu_button)
	_build_scale_bar(layer)
	_build_map_help(layer)
	actions_menu_button=Button.new()
	actions_menu_button.name="UtilityActions"
	actions_menu_button.position=Vector2(viewport_width-140,8)
	actions_menu_button.size=Vector2(84,30)
	actions_menu_button.text="ACTIONS"
	actions_menu_button.tooltip_text="Open contextual map actions."
	actions_menu_button.add_theme_font_size_override("font_size",9)
	actions_menu_button.pressed.connect(_toggle_actions_menu)
	layer.add_child(actions_menu_button)
	world_competition_button=Button.new()
	world_competition_button.name="NavWorld"
	world_competition_button.position=Vector2(viewport_width-310,8)
	world_competition_button.size=Vector2(78,30)
	world_competition_button.text="WORLD"
	world_competition_button.tooltip_text="Open the competitive world: rival populations, strategies, power, diplomacy, wars, and victory standing."
	world_competition_button.add_theme_font_size_override("font_size",9)
	world_competition_button.pressed.connect(_open_civilizations_panel)
	layer.add_child(world_competition_button)
	var systems_button:=Button.new()
	systems_button.name="NavCivilization"
	systems_button.position=Vector2(viewport_width-416,8)
	systems_button.size=Vector2(100,30)
	systems_button.text="CIVILIZATION"
	systems_button.tooltip_text="Open society, progression, government, inquiry, and council."
	systems_button.add_theme_font_size_override("font_size",9)
	systems_button.pressed.connect(_open_systems_hub)
	layer.add_child(systems_button)
	...
	actions_menu_panel=PanelContainer.new()
	actions_menu_panel.name="MapActionsMenu"
	actions_menu_panel.position=Vector2(viewport_width-304,46)
	actions_menu_panel.size=Vector2(292,310)
	actions_menu_panel.visible=false
	var actions_style:=StyleBoxFlat.new()
	actions_style.bg_color=Color(0.025,0.038,0.040,0.98)
	actions_style.border_color=Color("#71654b")
	actions_style.set_border_width_all(1)
	actions_style.set_corner_radius_all(3)
	actions_style.set_content_margin_all(12)
	actions_menu_panel.add_theme_stylebox_override("panel",actions_style)
	var actions_root:=VBoxContainer.new()
	actions_root.add_theme_constant_override("separation",8)
	actions_menu_panel.add_child(actions_root)
	var actions_title:=Label.new()
	actions_title.text="MAP ACTIONS"
	actions_title.add_theme_font_size_override("font_size",12)
	actions_title.add_theme_color_override("font_color",Color("#e5d6b5"))
	actions_root.add_child(actions_title)
	actions_menu_status=Label.new()
	actions_menu_status.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART
	actions_menu_status.add_theme_font_size_override("font_size",10)
	actions_menu_status.add_theme_color_override("font_color",Color("#9ca6a1"))
	actions_menu_status.size_flags_vertical=Control.SIZE_EXPAND_FILL
	actions_root.add_child(actions_menu_status)
	actions_menu_settlement_button=Button.new()
	actions_menu_settlement_button.custom_minimum_size=Vector2(0,42)
	actions_menu_settlement_button.alignment=HORIZONTAL_ALIGNMENT_LEFT
	actions_menu_settlement_button.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART
	actions_menu_settlement_button.add_theme_font_size_override("font_size",11)
	actions_menu_settlement_button.pressed.connect(_on_actions_settlement_pressed)
	actions_root.add_child(actions_menu_settlement_button)
	actions_menu_scout_button=Button.new()
	actions_menu_scout_button.custom_minimum_size=Vector2(0,50)
	actions_menu_scout_button.alignment=HORIZONTAL_ALIGNMENT_LEFT
	actions_menu_scout_button.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART
	actions_menu_scout_button.add_theme_font_size_override("font_size",11)
	actions_menu_scout_button.pressed.connect(_on_actions_scout_pressed)
	actions_root.add_child(actions_menu_scout_button)
	actions_menu_diplomat_button=Button.new()
	actions_menu_diplomat_button.custom_minimum_size=Vector2(0,50)
	actions_menu_diplomat_button.alignment=HORIZONTAL_ALIGNMENT_LEFT
	actions_menu_diplomat_button.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART
	actions_menu_diplomat_button.add_theme_font_size_override("font_size",11)
	actions_menu_diplomat_button.pressed.connect(_on_actions_diplomat_pressed)
	actions_root.add_child(actions_menu_diplomat_button)
	layer.add_child(actions_menu_panel)
	resource_view_toggle=Button.new()
	resource_view_toggle.name="ResourceViewToggle"
	resource_view_toggle.position=Vector2(viewport_width-176,get_viewport().get_visible_rect().size.y-54)
	resource_view_toggle.size=Vector2(160,36)
	resource_view_toggle.add_theme_font_size_override("font_size",11)
	resource_view_toggle.tooltip_text="Toggle the resource map layer. It shows only deposits your civilization has actually recognized; unknown resources remain invisible."
	resource_view_toggle.pressed.connect(_toggle_resource_view)
	layer.add_child(resource_view_toggle)
	...
	provisions_button.name="NavEconomy"
	provisions_button.position=Vector2(viewport_width-502,8)
	provisions_button.size=Vector2(80,30)
	provisions_button.alignment=HORIZONTAL_ALIGNMENT_LEFT
	provisions_button.text="ECONOMY"
	provisions_button.tooltip_text="Open the economy dashboard: provisions, water, material flow, storage, and bottlenecks."
	...
	provisions_button.add_theme_stylebox_override("normal",_hud_chip_style(Color("#79906a")))
	provisions_button.add_theme_stylebox_override("hover",_hud_chip_style(Color("#aab778"),true))
	provisions_button.pressed.connect(_open_provisions_panel)
	layer.add_child(provisions_button)
	materials_button=Button.new()
	materials_button.position=Vector2(-1000,-1000)
	materials_button.size=Vector2(92,30)
	...
	materials_button.add_theme_stylebox_override("normal",_hud_chip_style(Color("#8a7658")))
	materials_button.add_theme_stylebox_override("hover",_hud_chip_style(Color("#b69b68"),true))
...
func _open_settlement_naming_panel() -> void:
	...
	heading.text="NAME THIS SETTLEMENT"
	...
	settlement_name_input.text=String(_settlement_model().selected_settlement().get("name",GameState.settlement_name))
...
func _commit_settlement_name() -> void:
	...
	if chosen=="":
		return
	var selected:Dictionary=_settlement_model().selected_settlement()
	if selected.is_empty() or bool(selected.get("primary",false)):
		GameState.settlement_name=chosen.substr(0,32)
	if not selected.is_empty(): selected["name"]=chosen.substr(0,32)
	GameState.settlement_network_revision+=1
	var event:={"day":int(GameState.elapsed_days),"title":"Settlement Named","description":"This settlement is now known as %s." % chosen,"domain":"settlement","severity":"major"}
	GameState.simulation_events.push_front(event)
	if GameState.simulation_events.size()>80: GameState.simulation_events.resize(80)
	if travel_status_label:
		travel_status_label.text="%s HAS BEEN NAMED" % chosen.to_upper()
	_update_time_interface()
...
func _refresh_discovered_resource_overlays()->void:
	var city:Dictionary=_settlement_model().selected_settlement()
	_settlement_model().with_city_resources(String(city.get("id","")),func()->void: _refresh_discovered_resource_overlays_local())

func _refresh_discovered_resource_overlays_local() -> void:
	if camera==null: return
...
func _inspect_location(position: Vector3)->void:
	var city:Dictionary=_settlement_model().selected_settlement()
	_settlement_model().with_city_resources(String(city.get("id","")),func()->void: _inspect_location_local(position))
...
func _open_provisions_panel()->void:
	var city:Dictionary=_settlement_model().selected_settlement()
	_settlement_model().with_city_resources(String(city.get("id","")),func()->void:
		_settlement_model().with_local_population(func()->void: _open_provisions_panel_local())
		_append_city_trade_status(provisions_panel)
	)

func _open_provisions_panel_local() -> void:
...
func _open_materials_panel()->void:
	var city:Dictionary=_settlement_model().selected_settlement()
	_settlement_model().with_city_resources(String(city.get("id","")),func()->void:
		_settlement_model().with_local_population(func()->void: _open_materials_panel_local())
		_append_city_trade_status(materials_panel)
	)

func _open_materials_panel_local() -> void:
	_close_primary_destinations_except("economy")
...
func _open_materials_panel_legacy()->void:
	var city:Dictionary=_settlement_model().selected_settlement()
	_settlement_model().with_city_resources(String(city.get("id","")),func()->void:
		_settlement_model().with_local_population(func()->void: _open_materials_panel_legacy_local())
	)

func _open_materials_panel_legacy_local() -> void:
	if materials_panel: materials_panel.queue_free()
	materials_panel=Control.new()
	materials_panel.size=get_viewport().get_visible_rect().size
	materials_panel.mouse_filter=Control.MOUSE_FILTER_STOP
...
func _cycle_material_priority(resource_name:String)->void:
	var city:Dictionary=_settlement_model().selected_settlement()
	_settlement_model().with_city_resources(String(city.get("id","")),func()->void:
		_settlement_model().with_local_population(func()->void: _cycle_material_priority_local(resource_name))
	)
...
func _open_provisions_detail_overlay()->void:
	var city:Dictionary=_settlement_model().selected_settlement()
	_settlement_model().with_city_resources(String(city.get("id","")),func()->void:
		_settlement_model().with_local_population(func()->void: _open_provisions_detail_overlay_local())
	)

func _open_provisions_detail_overlay_local()->void:
...
func _open_settlement_dashboard()->void:
	var city:Dictionary=_settlement_model().selected_settlement()
	_settlement_model().with_city_resources(String(city.get("id","")),func()->void:
		_settlement_model().with_local_population(func()->void: _open_settlement_dashboard_local())
	)

func _open_settlement_dashboard_local()->void:
	...
	labor.text="CIVILIZATION LABOR"
	labor.custom_minimum_size=Vector2(150,40)
	labor.tooltip_text="Set the civilization’s workforce priorities, applied to each city’s local workers."
...
func _update_time_interface() -> void:
	...
	if convoy_map_label:
		...
		convoy_map_label.text="%s  •  %s  •  %s\nDEFENSE: %s" % [_primary_settlement_display_name(),settlement_classification,_compact_population(primary_population),defense_name] if settlement_classification!="" else "%s  •  %s\nDEFENSE: %s" % [_primary_settlement_display_name(),_compact_population(primary_population),defense_name]
	...
	if travel_active:
		...
		travel_status_label.text = ("FOUNDING CONVOY READY  •  LEFT-CLICK LAND TO TRAVEL  •  OPEN ACTIONS TO SETTLE HERE" if not GameState.settlement_site_committed else "RECOGNIZED RESOURCES %s  •  MOUSE WHEEL TO ZOOM  •  ACTIONS FOR MAP COMMANDS" % ("SHOWN" if resource_view_enabled else "HIDDEN"))
...
func _refresh_city_selector()->void:
	if settlement_selector==null: return
	var selected:Dictionary=_settlement_model().selected_settlement()
	settlement_selector.visible=not selected.is_empty()
	var ids:Array[String]=[]
	for city in GameState.player_settlements: ids.append(String(city.id))
	var rebuild:=settlement_selector.item_count!=ids.size()
	if not rebuild:
		for index in ids.size():
			if String(settlement_selector.get_item_metadata(index))!=ids[index] or settlement_selector.get_item_text(index)!=String(GameState.player_settlements[index].name): rebuild=true
	if rebuild:
		settlement_selector.clear()
		for city in GameState.player_settlements:
			settlement_selector.add_item(String(city.name))
			settlement_selector.set_item_metadata(settlement_selector.item_count-1,String(city.id))
	var selected_index:=ids.find(String(selected.get("id","")))
	if selected_index>=0: settlement_selector.select(selected_index)

func _append_city_trade_status(panel:Control)->void:
	if not panel: return
	var columns:=panel.find_children("*","VBoxContainer",true,false)
	if columns.is_empty(): return
	var city:Dictionary=_settlement_model().selected_settlement()
	var city_id:=String(city.get("id",""))
	var trade:Dictionary=_settlement_model().city_trade_snapshot(city_id)
	var button:=Button.new()
	button.name="CityTradeStatus"
	button.text="%s  •  CITY TRADE  •  %d SHIPMENTS IN TRANSIT" % [_settlement_display_name(),(trade.shipments as Array).size()]
	if not bool(trade.capacity.ready): button.text="%s  •  CITY TRADE  •  DEVELOP LOGISTICS" % _settlement_display_name()
	button.tooltip_text="Local leaders arrange surplus deliveries as transport improves. Open routes, cargo, and delivery history."
	button.pressed.connect(_open_city_trade_report.bind(city_id))
	columns[0].add_child(button)
	columns[0].move_child(button,mini(1,columns[0].get_child_count()-1))

func _open_city_trade_report(city_id:String)->void:
	var trade:Dictionary=_settlement_model().city_trade_snapshot(city_id)
	var city:Dictionary=_settlement_model().settlement_record(city_id)
	var dialog:=AcceptDialog.new()
	dialog.title="%s — Intercity trade" % String(city.get("name","City"))
	dialog.min_size=Vector2i(620,360)
	var text:=RichTextLabel.new()
	text.custom_minimum_size=Vector2(580,300)
	var lines:PackedStringArray=[]
	lines.append("Leaders retain a 45-day food reserve before supplying a city with fewer than 14 days. Material deliveries use only local surplus. Goods remain in transit until arrival.")
	lines.append("\nTransport: %.0f km/day • Reach: %.0f km" % [float(trade.capacity.speed_km_per_day),float(trade.capacity.range_km)])
	if not bool(trade.capacity.ready): lines.append(String(trade.capacity.reason))
	lines.append("\nIN TRANSIT")
	if (trade.shipments as Array).is_empty(): lines.append("No shipments underway.")
	for shipment in trade.shipments:
		lines.append("%s → %s: %.1f %s • arrives in %d days" % [String(shipment.source_name),String(shipment.destination_name),float(shipment.quantity),String(shipment.resource),maxi(0,ceili(float(shipment.arrival_day)-GameState.elapsed_days))])
	lines.append("\nRECENT DELIVERIES")
	for entry in trade.history:
		if String(entry.status)!="delivered": continue
		lines.append("%s → %s: %.1f %s delivered • %.2f lost in transit" % [String(entry.source_name),String(entry.destination_name),float(entry.delivered),String(entry.resource),float(entry.lost)])
	text.text="\n".join(lines)
	dialog.add_child(text)
	interface_layer.add_child(dialog)
	dialog.confirmed.connect(dialog.queue_free)
	dialog.canceled.connect(dialog.queue_free)
	dialog.popup_centered()

func _on_city_selected(index:int)->void:
	if index<0 or index>=settlement_selector.item_count: return
	_select_city(String(settlement_selector.get_item_metadata(index)))

func _select_city(settlement_id:String)->void:
	if not _settlement_model().select_settlement(settlement_id): return
	_initialize_city_resource_sites(settlement_id)
	_close_primary_destinations_except("")
	var city:Dictionary=_settlement_model().selected_settlement()
	...
	var final_size:=clampf(float(city.get("claim_radius_km",1.0))*3.0,0.8,6.0)
	settlement_camera_tween=create_tween()
	settlement_camera_tween.set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_IN_OUT)
	settlement_camera_tween.tween_method(func(weight:float)->void:
		_set_camera_target(start.lerp(destination,weight))
		camera.size=lerpf(start_size,final_size,weight)
		_update_camera()
		_update_scale_lod()
	,0.0,1.0,0.65)
	_refresh_city_selector()
...
func _initialize_city_resource_sites(settlement_id:String)->void:
	...
	if city.is_empty() or bool(city.get("primary",false)) or bool(city.get("resource_sites_initialized",false)): return
	...
	for deposit in GameState.resource_deposits:
		...
		if nearest_id==settlement_id and distance<=6.0 and (deposit.get("shipments",[]) as Array).is_empty():
...
func _city_trade_route_assessment(source:Dictionary,destination:Dictionary)->Dictionary:
	...
	var route:=_analyze_convoy_route(Vector3(start.x,0.0,start.y),Vector3(finish.x,0.0,finish.y))
	if bool(route.get("valid",false)): return route
	var source_profile:Dictionary=_settlement_model().settlement_intrinsic_profile(String(source.id))
	var destination_profile:Dictionary=_settlement_model().settlement_intrinsic_profile(String(destination.id))
	if bool(source_profile.get("maritime_trade_ready",false)) and bool(destination_profile.get("maritime_trade_ready",false)) and float(source_profile.get("shoreline_access",0.0))>0.0 and float(destination_profile.get("shoreline_access",0.0))>0.0:
		return {"valid":true,"terrain_modifier":1.25,"mode":"coastal shipping"}
	return route

func _settlement_display_name() -> String:
	var selected:Dictionary=_settlement_model().selected_settlement()
	if not selected.is_empty(): return String(selected.name).to_upper()
	return _primary_settlement_display_name()

func _primary_settlement_display_name()->String:
	...
	if event is InputEventKey and event.pressed:
		if event.keycode == KEY_ESCAPE:
			if settlement_convoy_targeting:
				...
				get_viewport().set_input_as_handled()
				return
			if actions_menu_panel and actions_menu_panel.visible:
				_close_actions_menu()
				get_viewport().set_input_as_handled()
				return
...
func _focus_settlement_from_screen(screen_position: Vector2, close_inspection: bool) -> bool:
	...
	for settlement in _settlement_model().settlement_network_snapshot().settlements:
		...
		if bool(settlement.get("primary",false)) and settlement_map_label and settlement_map_label.visible:
			...
			if absf(screen_position.x-label_screen.x)<=105.0 and absf(screen_position.y-label_screen.y)<=22.0: distance=0.0
	...
	_settlement_model().select_settlement(String(selected.id))
	_initialize_city_resource_sites(String(selected.id))
	if settlement_camera_tween and settlement_camera_tween.is_running(): settlement_camera_tween.kill()
	_refresh_city_selector()
