...
func ask(message:String)->bool:
	...
	WorldSimulation.campaign.pause_to_speak()
	WorldSimulation.campaign.state.messages.append({"role":"user","content":text})
	while WorldSimulation.campaign.state.messages.size()>40:WorldSimulation.campaign.state.messages.pop_front()
	...
	if config.is_empty():
		...
		else:WorldSimulation.campaign._report("I cannot reach the live conversation service. You can use: defend home; withdraw; recover; besiege Bracken Hold; attack Bracken Hold; approve; override; or relieve command. No order was inferred from your question.",false)
		WorldSimulation.campaign.changed.emit();return true
	var messages:Array=[{"role":"system","content":PROMPT},{"role":"system","content":"KNOWN CAMPAIGN: "+JSON.stringify(WorldSimulation.campaign.public_context())}]
	for m:Dictionary in WorldSimulation.campaign.state.messages:messages.append(m.duplicate())
...
func local_order(text:String)->Dictionary:
	...
	for r:Dictionary in WorldSimulation.campaign.state.get("rivals",[]):
...
func request_opponent(id:String)->void:
	...
	var context:={"name":r.leader,"goal":r.goal,"plan":r.plan,"own_troops":r.force.troops,"own_food":r.food,"own_position":str(r.cell),"observations":r.seen,"memory":r.memory,"difficulty":WorldSimulation.campaign.difficulty}
...
func _request(id:String,messages:Array,config:Dictionary)->void:
	...
	pending[id]={"http":http,"started":Time.get_ticks_msec(),"model":String(config.model),"turn":int(WorldSimulation.campaign.state.turn)}
...
func _completed(result:int,code:int,_headers:PackedStringArray,body:PackedByteArray,id:String,http:HTTPRequest)->void:
	...
	if id=="player":
		...
		if not r.is_empty() and int(WorldSimulation.campaign.state.turn)==int(request.turn) and value is Dictionary and value.get("action","") in ["hold","pressure","intercept","resupply"]:
...
func _accept(value:Variant)->bool:
	...
	var last_user:=""
	for message:Dictionary in WorldSimulation.campaign.state.messages:
	...
	if value.action!="discuss" and is_discussion(last_user):
		...
		WorldSimulation.campaign.state.messages.append({"role":"assistant","content":String(value.reply)+"\nNo objective changed."})
		return true
	var result:=WorldSimulation.campaign.propose({"action":value.action,"target":value.target})
	var reply:=String(value.reply)
	if result.has("error"):reply+="\n\n"+String(result.error)
	WorldSimulation.campaign.state.messages.append({"role":"assistant","content":reply})
	while WorldSimulation.campaign.state.messages.size()>40:WorldSimulation.campaign.state.messages.pop_front()
