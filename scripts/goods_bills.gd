...
const GOODS_PER_RECIPE_DAY:=2.0
...
## Extracted resources (such as Refractory Clay) are never expanded.
static func manufactured(item:String)->bool:
	if item in Goods.LEGACY_PRODUCTS:return true
	if EXTRACTED.has(item):return false
...
static func _unit(item:String,depth:int,visiting:Dictionary)->Dictionary:
	...
	if not manufactured(item):return {item:1.0}
