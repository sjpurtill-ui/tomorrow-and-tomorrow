...
static func quote(subject:String,item:String)->Dictionary:
	...
	var resource:=String(Industry.product(item).output)
	if float(WorldSimulation.state.resource_stockpiles.get(resource,0))<1:return {"error":"Needs one physically owned batch of "+resource+"."}
	return {"subject":subject,"item":item,"resource":resource,"message":"Consume one batch of %s for destructive examination. Researchers need 180 study-work, then gain a 1.35× evidence multiplier on this investigation. Local foundations and subsequent research are still required." % resource}
static func begin(subject:String,item:String)->Dictionary:
	...
	var record:={"id":"reverse:"+subject,"kind":"artifact","name":"Disassembled example: "+String(entry.name),"source_id":"","source_name":"Owned workshop specimen","position":{"x":0.0,"z":0.0},"observed_day":day,"returned_day":day,"discovery_id":subject,"study":0.0,"work":180.0,"signals":entry.get("signals",[]).duplicate(),"reverse_engineered":true,"specimen_item":item,"acquisition":"One physical batch consumed for destructive examination"}
