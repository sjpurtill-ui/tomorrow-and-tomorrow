...
func _ensure_state()->void:
	if WorldSimulation.state.economy_stage not in [STAGE_SUBSISTENCE,STAGE_METAL,STAGE_CURRENCY]:
		WorldSimulation.state.economy_stage=STAGE_SUBSISTENCE
	if WorldSimulation.state.public_spending_priority not in PUBLIC_SPENDING_PRIORITIES:
		WorldSimulation.state.public_spending_priority="balanced"
	if WorldSimulation.state.economy_metrics.is_empty():
		WorldSimulation.state.economy_metrics={"stage":WorldSimulation.state.economy_stage,"stage_name":STAGE_NAMES[WorldSimulation.state.economy_stage],"monetization":0.0,"market_access":0.0,"distribution_reach":0.0,"price_index":0.0,"price_observations":0,"inflation":0.0,"trade_volume":0.0,"reciprocal_surplus":0.0,"active_trade_partners":0,"foreign_trade_available":false,"tax_revenue":0.0,"subsistence_share":1.0,"metal_reserve":0.0,"money_supply":0.0,"velocity":0.0,"fiscal_balance":0.0}
	if WorldSimulation.state.currency_supply>0.0 and WorldSimulation.state.private_currency+WorldSimulation.state.currency_hoards+WorldSimulation.state.public_treasury+WorldSimulation.state.mutual_aid_reserve<=0.0:
		WorldSimulation.state.private_currency=maxf(0.0,WorldSimulation.state.currency_supply-WorldSimulation.state.public_treasury)
	var composed_metal:=_weighed_metal_value()
	if WorldSimulation.state.weighed_metal_circulation>0.0 and composed_metal<=0.0:
		WorldSimulation.state.weighed_metal_composition["Coin"]=WorldSimulation.state.weighed_metal_circulation
	else:
		WorldSimulation.state.weighed_metal_circulation=composed_metal

func process_day(context:Dictionary={}) -> Array[Dictionary]:
	initialize()
	for resource_name_variant in WorldSimulation.state.resource_stockpiles:
		if float(WorldSimulation.state.resource_stockpiles[resource_name_variant])>0.001:
			WorldSimulation.state.economy_known_goods[String(resource_name_variant)]=true
	...
	WorldSimulation.state.economy_metrics={
		"stage":WorldSimulation.state.economy_stage,"stage_name":STAGE_NAMES[WorldSimulation.state.economy_stage],
		"monetization":monetization,"market_access":market_access,"distribution_reach":market_access,"price_index":price_index,"price_observations":int(WorldSimulation.state.economy_metrics.get("price_observations",0))+(1 if recorded_comparison_today else 0),
		...
		"subsistence_share":1.0-monetization,"metal_available":available_metal,"weighed_metal_circulation":WorldSimulation.state.weighed_metal_circulation,"weighed_metal_composition":WorldSimulation.state.weighed_metal_composition.duplicate(true),"metal_exchange":metal_exchange,"metal_trade_turnover":metal_exchange.turnover,"metal_velocity":metal_exchange.velocity,"weighed_metal_losses":WorldSimulation.state.weighed_metal_losses,"metal_reserve":reserve,"reserve_composition":WorldSimulation.state.monetary_reserve_metals.duplicate(true),
		"money_supply":WorldSimulation.state.currency_supply,"money_demand":WorldSimulation.state.currency_demand,"liquid_money":liquid_money,"transactional_money":liquid_money,"transactional_share":currency_liquidity.transactional_share,"treasury":WorldSimulation.state.public_treasury,"private_currency":WorldSimulation.state.private_currency,"currency_hoards":WorldSimulation.state.currency_hoards,"currency_liquidity":currency_liquidity,"currency_confidence":currency_liquidity.confidence,"hoard_share":currency_liquidity.hoard_share,"velocity":velocity,
		"exchange_reliability":reliability,"shortage_pressure":_shortage_pressure(),
		"credit_outstanding":WorldSimulation.state.credit_outstanding,"credit_created":credit.created,"credit_repaid":credit.repaid,"credit_defaulted":credit.defaulted,"default_rate":credit.default_rate,
		"credit_limit":credit.limit,"credit_utilization":credit.utilization,
		"mutual_aid":mutual_aid,"mutual_aid_reserve":WorldSimulation.state.mutual_aid_reserve,
		"inequality":inequality,"social_pressure":economic_pressure,"household_welfare":household_welfare,"household_hardship_share":household_welfare.hardship_share,"lower_household_access":household_welfare.lower_access,"household_access_gap":household_welfare.access_gap,"reserve_ratio":reserve/maxf(1.0,WorldSimulation.state.currency_supply),"exchange_mix":exchange_mix,
		"external_trade":external_trade,"external_trade_policy":WorldSimulation.state.external_trade_policy,"external_trade_credit":WorldSimulation.state.external_trade_credit,"external_exports":WorldSimulation.state.external_trade_exports,"external_imports":WorldSimulation.state.external_trade_imports,"external_trade_losses":WorldSimulation.state.external_trade_losses,
		...
		"resource_obligation_pressure":real_accounts.obligation_pressure,"public_obligations":public_obligations,"obligation_regime":public_obligations.regime,"in_kind_labor_arrears":WorldSimulation.state.in_kind_labor_arrears,"in_kind_material_arrears":WorldSimulation.state.in_kind_material_arrears,
	...
	WorldSimulation.state.economy_metrics["military_upkeep_due"]=finance.military_due
	WorldSimulation.state.economy_metrics["public_spending_priority"]=finance.spending_priority
	WorldSimulation.state.economy_metrics["civil_payment_coverage"]=finance.civil_coverage
	WorldSimulation.state.economy_metrics["military_payment_coverage"]=finance.military_coverage
	WorldSimulation.state.economy_metrics["civil_arrears"]=WorldSimulation.state.civil_arrears
	WorldSimulation.state.economy_metrics["military_arrears"]=WorldSimulation.state.military_arrears
	WorldSimulation.state.economy_metrics["public_arrears"]=WorldSimulation.state.civil_arrears+WorldSimulation.state.military_arrears
	WorldSimulation.state.economy_metrics["public_debt"]=WorldSimulation.state.public_debt
	WorldSimulation.state.economy_metrics["public_borrowing"]=finance.borrowing
	WorldSimulation.state.economy_metrics["debt_service"]=finance.debt_service
	WorldSimulation.state.economy_metrics["interest_accrued"]=finance.interest_accrued
	WorldSimulation.state.economy_metrics["debt_capacity"]=finance.debt_capacity
	WorldSimulation.state.economy_metrics["fiscal_outlook"]=fiscal_outlook
	WorldSimulation.state.economy_metrics["fiscal_headroom"]=fiscal_outlook.discretionary_headroom
	WorldSimulation.state.economy_metrics["fiscal_coverage"]=fiscal_outlook.coverage_ratio
	var audit:=accounting_audit()
	WorldSimulation.state.economy_metrics["accounting_audit"]=audit
	...
	if String(public_obligations.regime)=="scheduled in-kind levies" and not WorldSimulation.state.economy_benchmarks.has("scheduled_public_levies"):
		WorldSimulation.state.economy_benchmarks["scheduled_public_levies"]={"day":day,"from":"customary obligations"}
	...
	if float(public_obligations.pressure)>0.12: _threshold_event(events,"in_kind_arrears","In-Kind Obligations Accumulate","Unrendered collective work and material contributions now equal %.1f labor-days and %.1f value in carried obligations." % [WorldSimulation.state.in_kind_labor_arrears,WorldSimulation.state.in_kind_material_arrears],30)
...
func _update_benchmarks(events:Array[Dictionary])->void:
	var adoption_measures:=WorldSimulation.discovery.adoption("standard_measures")
	var adoption_tallies:=WorldSimulation.discovery.adoption("tallies")
	var production:=float(WorldSimulation.state.society_capacities.get("production",0.0))
	var institutions:=float(WorldSimulation.state.society_capacities.get("institutions",0.0))
	var logistics:=float(WorldSimulation.state.society_capacities.get("logistics",0.0))
	var surplus_days:=float(WorldSimulation.state.simulation_metrics.get("food_days",0.0))
	if WorldSimulation.state.economy_stage==STAGE_SUBSISTENCE:
		var metal_available:=_available_metal_value()>=metal_stage_requirement() and WorldSimulation.discovery.adoption("copper_smelting")>=0.18
	...
	elif WorldSimulation.state.economy_stage==STAGE_METAL:
...
func metal_stage_requirement()->float:
	return maxf(6.0,WorldSimulation.state.population_exact*0.20)

func currency_metal_requirement()->float:
	return maxf(20.0,WorldSimulation.state.population_exact*(2.0/3.0))

func founding_reserve_requirement()->float:
	return maxf(10.0,WorldSimulation.state.population_exact/3.0)

func founding_currency_issue()->float:
	return maxf(10.0,WorldSimulation.state.population_exact*0.75)

func founding_weighed_metal_requirement()->float:
	return maxf(2.0,WorldSimulation.state.population_exact*0.08)

func _transition(next_stage:String,events:Array[Dictionary],reason:String)->void:
	var prior:=WorldSimulation.state.economy_stage
	WorldSimulation.state.economy_stage=next_stage
...
func _monetization(market_access:float)->float:
	match WorldSimulation.state.economy_stage:
		STAGE_METAL:
			var potential:=clampf(0.08+market_access*0.38+WorldSimulation.discovery.adoption("standard_measures")*0.18,0.08,0.62)
		...
		STAGE_CURRENCY:
			...
			var recent_inflation:=maxf(0.0,float(WorldSimulation.state.economy_metrics.get("inflation",0.0)))
			var liquid_share:=clampf(WorldSimulation.state.private_currency/maxf(1.0,WorldSimulation.state.currency_supply),0.0,1.0)
...
func _update_prices(market_access:float,trade_volume:float=1.0)->float:
	...
	if trade_volume<=0.001:
		return float(WorldSimulation.state.economy_metrics.get("price_index",0.0)) if int(WorldSimulation.state.economy_metrics.get("price_observations",0))>0 else 0.0
	var weighted:=0.0
	var weights:=0.0
	var population:=maxf(1.0,WorldSimulation.state.population_exact)
	for resource_name in BASE_VALUES:
		var stock:=float(WorldSimulation.state.resource_stockpiles.get(resource_name,0.0))
		...
		if WorldSimulation.state.economy_stage==STAGE_CURRENCY and WorldSimulation.state.currency_demand>0.0:
			var spendable_supply:=maxf(0.0,WorldSimulation.state.private_currency)
...
func active_external_trade_partner_count()->int:
	return maxi(0,int(WorldSimulation.world.player_effects().get("active_trade_partners",0)))
...
func _resource_is_economically_known(resource_name:String,stock:float)->bool:
	...
	if bool(WorldSimulation.state.economy_known_goods.get(resource_name,false)): return true
...
func _process_external_trade(market_access:float,domestic_trade:float,contract_partners_override:int=-1)->Dictionary:
	...
	# always pass the authoritative treaty count produced by WorldSimulation.world.
	...
	var result:={"exports":0.0,"imports":0.0,"exported_goods":{},"imported_goods":{},"friction":0.0,"credit":WorldSimulation.state.external_trade_credit,"claim_limit":0.0,"claim_loss":0.0,"partners":active_partners,"available":active_partners>0}
	...
	var stage_capacity:=0.42 if WorldSimulation.state.economy_stage==STAGE_METAL else 0.72
	var value_capacity:=maxf(WorldSimulation.state.population_exact*0.015,domestic_trade)*reach*stage_capacity
	var institutions:=clampf(float(WorldSimulation.state.society_capacities.get("institutions",0.25)),0.0,1.0)
	var claim_limit:=maxf(WorldSimulation.state.population_exact*3.0,value_capacity*120.0*(0.55+institutions))
	...
	if WorldSimulation.state.external_trade_credit>claim_limit:
		...
		result.credit=WorldSimulation.state.external_trade_credit
		_ledger("external_claim_loss",impairment,"regional_trade_account","written_off","Foreign counterpart capacity contracted")
	if WorldSimulation.state.external_trade_policy=="closed" or market_access<0.28:
		_update_food_import_dependence(0.0)
		return result
	var friction:=clampf(0.22-market_access*0.10-(0.05 if WorldSimulation.state.economy_stage==STAGE_CURRENCY else 0.0)-WorldSimulation.discovery.effect("standardization")*0.10,0.06,0.24)
	...
	for candidate_variant in export_candidates:
		...
		var unit_value:=maxf(0.01,float(WorldSimulation.state.market_prices.get(resource_name,BASE_VALUES.get(resource_name,1.0))))
		...
		WorldSimulation.state.external_trade_credit+=earned
		WorldSimulation.state.external_trade_exports+=earned
	...
	for candidate_variant in import_candidates:
		if import_budget<=0.001 or WorldSimulation.state.external_trade_credit<=0.001: break
		var candidate:Dictionary=candidate_variant
		var resource_name:=String(candidate.resource)
		var unit_cost:=maxf(0.01,float(WorldSimulation.state.market_prices.get(resource_name,BASE_VALUES.get(resource_name,1.0)))*(1.0+friction))
		var requested:=minf(float(candidate.quantity),minf(import_budget,WorldSimulation.state.external_trade_credit)/unit_cost)
		var received:=_receive_trade_resource(resource_name,requested)
		var paid:=minf(WorldSimulation.state.external_trade_credit,received*unit_cost)
		if paid<=0.0: continue
		WorldSimulation.state.external_trade_credit-=paid
		WorldSimulation.state.external_trade_imports+=paid
	...
	WorldSimulation.state.external_trade_credit=maxf(0.0,WorldSimulation.state.external_trade_credit)
	result.credit=WorldSimulation.state.external_trade_credit
...
func cycle_external_trade_policy()->String:
	...
	var index:=policies.find(WorldSimulation.state.external_trade_policy)
	WorldSimulation.state.external_trade_policy=String(policies[(index+1)%policies.size()])
...
func _real_economy_accounts(trade_volume:float,market_access:float,military_burden:Dictionary={})->Dictionary:
	...
	var food_need:=maxf(0.01,float(WorldSimulation.state.simulation_metrics.get("food_consumption",population)))
	var fallback_eaten:=minf(food_need,maxf(0.0,float(WorldSimulation.state.resource_stockpiles.get("Food",0.0))))
	var food_eaten:=maxf(0.0,float(WorldSimulation.state.simulation_metrics.get("food_eaten",fallback_eaten)))
	var food_coverage:=clampf(float(WorldSimulation.state.simulation_metrics.get("food_intake_ratio",food_eaten/food_need)),0.0,1.0)
	...
	for resource_name in ["Timber","Stone","Clay","Fiber Plants"]:
		...
		material_coverage+=clampf(float(WorldSimulation.state.resource_stockpiles.get(resource_name,0.0))/maxf(0.01,desired),0.0,1.0)
	material_coverage/=4.0
	var housing_ratio:=clampf(float(WorldSimulation.state.housing_capacity)/population,0.0,1.0)
	...
	var collective_labor:=float(WorldSimulation.state.population_allocations.get("Administration",0))+float(WorldSimulation.state.population_allocations.get("Construction",0))*0.35+float(WorldSimulation.state.population_allocations.get("Logistics",0))*0.20+security_duty+diverted_craft_labor
...
func _process_resource_obligations(real_accounts:Dictionary,monetization:float,reliability:float)->Dictionary:
	...
	var admin_coverage:=clampf(WorldSimulation.state.effective_workers("Administration")/maxf(1.0,population*0.04),0.0,1.0)
	var scheduled:=levy_adoption>=0.25
	var regime:="customary obligations"
	if scheduled: regime="scheduled in-kind levies"
	if WorldSimulation.state.economy_stage==STAGE_CURRENCY: regime="monetary levy with residual dues"
	...
	var settlement_complexity:=minf(0.035,float(WorldSimulation.state.settlement_completed.size())/population*0.10)
	...
	var labor_arrears_days:=WorldSimulation.state.in_kind_labor_arrears/maxf(0.05,assessed_labor_due)
	var material_arrears_days:=WorldSimulation.state.in_kind_material_arrears/maxf(0.05,assessed_material_due)
...
func _public_debt_capacity(trade_volume:float,monetization:float,admin_coverage:float,tax_rate_override:float=-1.0,compliance_override:float=1.0)->float:
	...
	var effective_tax_rate:=WorldSimulation.state.tax_rate if tax_rate_override<0.0 else clampf(tax_rate_override,0.0,0.25)
	var annual_revenue_base:=maxf(WorldSimulation.state.population_exact*0.05,trade_volume*clampf(effective_tax_rate,0.01,0.25)*admin_coverage*monetization*clampf(compliance_override,0.0,1.0)*365.0)
	var institutions:=clampf(float(WorldSimulation.state.society_capacities.get("institutions",0.25)),0.0,1.0)
	return maxf(0.0,annual_revenue_base*(0.35+adoption*1.65)*clampf(0.45+institutions,0.45,1.45)+WorldSimulation.state.currency_supply*adoption*0.12)

func _public_upkeep(military_burden:Dictionary={})->Dictionary:
	if WorldSimulation.state.economy_stage!=STAGE_CURRENCY: return {"civil":0.0,"military":0.0,"total":0.0}
	var civil:=WorldSimulation.state.settlement_completed.size()*0.018+float(WorldSimulation.state.population_allocations.get("Administration",0))*0.004
	...
	military=clampf(military,0.0,maxf(1.0,WorldSimulation.state.population_exact*0.50))
...
func tax_capacity_snapshot(rate:float=-1.0,trade_volume:float=-1.0,monetization:float=-1.0)->Dictionary:
	var effective_rate:=WorldSimulation.state.tax_rate if rate<0.0 else rate
	var observed_trade:=float(WorldSimulation.state.economy_metrics.get("trade_volume",0.0)) if trade_volume<0.0 else trade_volume
	var observed_monetization:=float(WorldSimulation.state.economy_metrics.get("monetization",0.0)) if monetization<0.0 else monetization
	return _tax_capacity_for(effective_rate,observed_trade,observed_monetization,WorldSimulation.state.private_currency)
...
func _tax_capacity_for(rate:float,trade_volume:float,monetization:float,private_currency:float)->Dictionary:
	...
	var result:={"active":WorldSimulation.state.economy_stage==STAGE_CURRENCY,"statutory_rate":statutory_rate,"compliance":0.0,"administrative_reach":0.0,"taxable_exchange":0.0,"statutory_assessment":0.0,"administratively_assessed":0.0,"compliant_assessment":0.0,"collectible":0.0,"effective_rate":0.0,"noncompliance_gap":0.0,"liquidity_gap":0.0}
	if WorldSimulation.state.economy_stage!=STAGE_CURRENCY: return result
	var admin_coverage:=clampf(WorldSimulation.state.effective_workers("Administration")/maxf(1.0,WorldSimulation.state.population_exact*0.04),0.0,1.0)
	var legitimacy:=clampf(float(WorldSimulation.state.simulation_metrics.get("legitimacy",0.50)),0.0,1.0)
	var institutions:=clampf(float(WorldSimulation.state.society_capacities.get("institutions",0.25)),0.0,1.0)
...
func set_public_spending_priority(priority:String)->String:
	...
	WorldSimulation.state.public_spending_priority=normalized
...
func cycle_public_spending_priority()->String:
	var current:=PUBLIC_SPENDING_PRIORITIES.find(WorldSimulation.state.public_spending_priority)
...
func fiscal_outlook(days:float=30.0,trade_volume:float=-1.0,monetization:float=-1.0,military_burden:Dictionary={})->Dictionary:
	...
	var observed_trade:=trade_volume if trade_volume>=0.0 else float(WorldSimulation.state.economy_metrics.get("trade_volume",0.0))
	var observed_monetization:=monetization if monetization>=0.0 else float(WorldSimulation.state.economy_metrics.get("monetization",0.0))
	var burden:Dictionary=military_burden.duplicate(true) if not military_burden.is_empty() else _military_burden_snapshot()
	return _fiscal_outlook_for(horizon,observed_trade,observed_monetization,burden,WorldSimulation.state.public_treasury,WorldSimulation.state.private_currency,WorldSimulation.state.tax_rate)
...
func _fiscal_outlook_for(days:float,trade_volume:float,monetization:float,military_burden:Dictionary,treasury:float,private_currency:float,tax_rate:float,spending_priority:String="")->Dictionary:
	...
	var institutions:=clampf(float(WorldSimulation.state.society_capacities.get("institutions",0.25)),0.0,1.0)
	var utilization:=WorldSimulation.state.public_debt/maxf(1.0,debt_capacity)
	var annual_interest_rate:=clampf(0.025+(1.0-institutions)*0.055+utilization*0.035-WorldSimulation.discovery.adoption("public_credit")*0.015,0.012,0.14) if WorldSimulation.state.public_debt>0.0 else 0.0
	var daily_interest:=WorldSimulation.state.public_debt*annual_interest_rate/365.0
	var civil_arrears:=maxf(0.0,WorldSimulation.state.civil_arrears)
	var military_arrears:=maxf(0.0,WorldSimulation.state.military_arrears)
	...
	var borrowing_headroom:=minf(debt_room,borrowable_private) if WorldSimulation.discovery.adoption("public_credit")>=0.25 else 0.0
	var funded_available:=cash_available+borrowing_headroom
	var normalized_priority:=WorldSimulation.state.public_spending_priority if spending_priority=="" else spending_priority
...
func _metal_processing_efficiency()->float:
	var smelting:=WorldSimulation.discovery.adoption("copper_smelting")
	var casting:=WorldSimulation.discovery.adoption("copper_casting")
...
func _available_metal_value()->float:
	...
	for resource_name in METAL_VALUES:
		...
		result+=float(WorldSimulation.state.resource_stockpiles.get(resource_name,0.0))*float(METAL_VALUES[resource_name])*processing
	return result

func _monetary_reserve_value()->float:
	var result:=0.0
	for value in WorldSimulation.state.monetary_reserve_metals.values(): result+=maxf(0.0,float(value))
	return result

func _weighed_metal_value()->float:
	var result:=0.0
	for value in WorldSimulation.state.weighed_metal_composition.values(): result+=maxf(0.0,float(value))
	return result

func _process_weighed_metal_exchange(trade_volume:float,monetization:float,reliability:float)->Dictionary:
	var result:={"active":false,"turnover":0.0,"velocity":0.0,"wear_loss":0.0,"circulation":WorldSimulation.state.weighed_metal_circulation}
	if WorldSimulation.state.economy_stage==STAGE_SUBSISTENCE or WorldSimulation.state.weighed_metal_circulation<=0.0001: return result
	result.active=true
	var measures:=WorldSimulation.discovery.adoption("standard_measures")
	var turnover_capacity:=WorldSimulation.state.weighed_metal_circulation*clampf(0.07+reliability*0.13+measures*0.08,0.06,0.28)
	...
	result.velocity=turnover/maxf(0.001,WorldSimulation.state.weighed_metal_circulation)
	result.wear_loss=wear_loss
	result.circulation=WorldSimulation.state.weighed_metal_circulation
	return result
...
func _remove_weighed_metal_value(requested_value:float)->float:
	...
	for resource_name_variant in WorldSimulation.state.weighed_metal_composition: names.append(String(resource_name_variant))
	names.sort()
	for resource_name in names:
		if remaining<=0.000001: break
		var held:=maxf(0.0,float(WorldSimulation.state.weighed_metal_composition.get(resource_name,0.0)))
		var taken:=minf(held,remaining)
		WorldSimulation.state.weighed_metal_composition[resource_name]=held-taken
		if float(WorldSimulation.state.weighed_metal_composition[resource_name])<=0.000001: WorldSimulation.state.weighed_metal_composition.erase(resource_name)
		remaining-=taken
		removed+=taken
	return removed
...
func _shortage_pressure()->float:
	...
	for resource_name in ["Food","Timber","Stone","Fiber Plants"]:
		var desired:=_desired_stock(resource_name,maxf(1.0,WorldSimulation.state.population_exact))
		total+=clampf(1.0-float(WorldSimulation.state.resource_stockpiles.get(resource_name,0.0))/maxf(1.0,desired),0.0,1.0)
	return total/4.0
...
func _exchange_reliability(market_access:float,market_volatility:float=-1.0)->float:
	...
	var institutions:=float(WorldSimulation.state.society_capacities.get("institutions",0.25))
...
func _market_volatility(current_index:float,lookback_days:int=30)->float:
	...
	var start:=maxi(0,WorldSimulation.state.economy_history.size()-maxi(2,lookback_days))
	for index in range(start,WorldSimulation.state.economy_history.size()):
		samples.append(maxf(0.001,float(WorldSimulation.state.economy_history[index].get("price_index",1.0))))
...
func _process_currency_liquidity(reliability:float,market_volatility:float)->Dictionary:
	var result:={"active":false,"confidence":0.0,"hoard_share":0.0,"desired_hoard_share":0.0,"transactional_share":0.0,"nontransactional_share":0.0,"hoarded_today":0.0,"released_today":0.0,"liquid_private":WorldSimulation.state.private_currency,"hoards":WorldSimulation.state.currency_hoards}
	if WorldSimulation.state.economy_stage!=STAGE_CURRENCY or WorldSimulation.state.currency_supply<=0.0: return result
	result.active=true
	var institutions:=clampf(float(WorldSimulation.state.society_capacities.get("institutions",0.25)),0.0,1.0)
	var reserve_security:=clampf(_monetary_reserve_value()/maxf(1.0,WorldSimulation.state.currency_supply*0.40),0.0,1.0)
	var recent_inflation:=absf(float(WorldSimulation.state.economy_metrics.get("inflation",0.0)))
	...
	var target:=WorldSimulation.state.currency_supply*desired_share
	...
	result.hoard_share=WorldSimulation.state.currency_hoards/maxf(1.0,WorldSimulation.state.currency_supply)
	result.desired_hoard_share=desired_share
	result.hoarded_today=hoarded_today
	result.released_today=released_today
	result.liquid_private=WorldSimulation.state.private_currency
	result.hoards=WorldSimulation.state.currency_hoards
	result.transactional_share=WorldSimulation.state.private_currency/maxf(1.0,WorldSimulation.state.currency_supply)
	result.nontransactional_share=(WorldSimulation.state.public_treasury+WorldSimulation.state.currency_hoards+WorldSimulation.state.mutual_aid_reserve)/maxf(1.0,WorldSimulation.state.currency_supply)
	return result
...
func _process_credit(trade_volume:float,market_access:float,reliability:float,events:Array[Dictionary])->Dictionary:
	...
	var opening:=WorldSimulation.state.credit_outstanding
	var shortage:=_shortage_pressure()
	var records:=WorldSimulation.discovery.adoption("property_registers")
	var courts:=WorldSimulation.discovery.adoption("specialized_courts")
	var secured_base:=WorldSimulation.state.population_exact*0.12+trade_volume*45.0+float(WorldSimulation.state.economy_metrics.get("exchangeable_surplus_value",0.0))*0.025
	...
	return {"created":created,"repaid":repaid,"defaulted":defaulted,"default_rate":default_rate,"limit":credit_limit,"utilization":WorldSimulation.state.credit_outstanding/maxf(1.0,credit_limit)}

func _process_mutual_risk_pool(trade_volume:float,defaulted_claims:float)->Dictionary:
	var adoption:=WorldSimulation.discovery.adoption("risk_pools")
	var result:={"active":false,"contribution":0.0,"payout":0.0,"capacity":0.0,"reserve":WorldSimulation.state.mutual_aid_reserve}
	if WorldSimulation.state.economy_stage!=STAGE_CURRENCY or adoption<0.25: return result
	result.active=true
...
func household_welfare_snapshot(real_accounts:Dictionary={},monetization:float=-1.0,market_access:float=-1.0,private_liquidity:float=-1.0,statutory_tax_rate:float=-1.0)->Dictionary:
	...
	if accounts.is_empty():
		var observed_accounts:Variant=WorldSimulation.state.economy_metrics.get("real_economy",{})
		if observed_accounts is Dictionary: accounts=(observed_accounts as Dictionary)
	var observed_monetization:=monetization if monetization>=0.0 else float(WorldSimulation.state.economy_metrics.get("monetization",0.0))
	var observed_access:=market_access if market_access>=0.0 else float(WorldSimulation.state.economy_metrics.get("market_access",0.0))
	var liquid_private:=private_liquidity if private_liquidity>=0.0 else WorldSimulation.state.private_currency
	var tax_rate:=statutory_tax_rate if statutory_tax_rate>=0.0 else WorldSimulation.state.tax_rate
	var physical_coverage:=clampf(float(accounts.get("essential_coverage",0.0)),0.0,1.0)
	var basket_cost:=maxf(0.01,float(accounts.get("essential_basket_cost",maxf(1.0,WorldSimulation.state.population_exact))))
...
func _exchange_mix(monetization:float,reliability:float)->Dictionary:
	...
	match WorldSimulation.state.economy_stage:
		...
		_:
			...
			var credit_share:=clampf(WorldSimulation.state.credit_outstanding/maxf(1.0,WorldSimulation.state.credit_outstanding+WorldSimulation.state.currency_supply)*0.55,0.03,0.18)
...
func _economic_social_pressure(inflation:float,default_rate:float,inequality:float,market_access:float,reliability:float,real_accounts:Dictionary={},finance:Dictionary={},mutual_aid:Dictionary={},currency_liquidity:Dictionary={},household_welfare:Dictionary={})->float:
	var arrears_pressure:=clampf((WorldSimulation.state.civil_arrears+WorldSimulation.state.military_arrears)/maxf(1.0,WorldSimulation.state.population_exact)*0.08,0.0,0.08)
	...
	var debt_pressure:=maxf(0.0,WorldSimulation.state.public_debt/maxf(1.0,debt_capacity)-0.55)*0.04 if debt_capacity>0.0 else 0.0
...
func set_tax_rate(rate:float)->float:
	...
	_ledger("policy",WorldSimulation.state.tax_rate,"sovereign","economy","Exchange levy rate")
	return WorldSimulation.state.tax_rate
...
func preview_policy(action:String,amount:float=0.0)->Dictionary:
	...
	var circulation:=WorldSimulation.state.weighed_metal_circulation
	var supply:=WorldSimulation.state.currency_supply
	var treasury:=WorldSimulation.state.public_treasury
	var tax_rate:=WorldSimulation.state.tax_rate
	var trade_policy:=WorldSimulation.state.external_trade_policy
	var spending_priority:=WorldSimulation.state.public_spending_priority
	var observed_trade:=float(WorldSimulation.state.economy_metrics.get("trade_volume",0.0))
	var observed_monetization:=float(WorldSimulation.state.economy_metrics.get("monetization",0.0))
	var burden:Dictionary={}
	var burden_variant:Variant=WorldSimulation.state.economy_metrics.get("military_burden",{})
	...
	var before_fiscal:=_fiscal_outlook_for(30.0,observed_trade,observed_monetization,burden,WorldSimulation.state.public_treasury,WorldSimulation.state.private_currency,WorldSimulation.state.tax_rate)
	...
	match normalized:
		...
		"BACK METAL":
			accepted=minf(requested,available) if WorldSimulation.discovery.adoption("copper_smelting")>=0.18 else 0.0
			available-=accepted
			reserve+=accepted
		...
		"RELEASE METAL":
			var required:=supply/2.5 if WorldSimulation.state.economy_stage==STAGE_CURRENCY else 0.0
			accepted=minf(requested,maxf(0.0,reserve-required))
			available+=_preview_returned_available_value(WorldSimulation.state.monetary_reserve_metals,accepted)
			reserve-=accepted
		...
		"WITHDRAW TRADE METAL":
			...
			available+=_preview_returned_available_value(WorldSimulation.state.weighed_metal_composition,accepted)
			circulation-=accepted
		...
		"RETIRE":
			if WorldSimulation.state.economy_stage==STAGE_CURRENCY: accepted=minf(requested,minf(treasury,supply))
			supply-=accepted
			treasury-=accepted
	...
	var after_fiscal:=_fiscal_outlook_for(30.0,observed_trade,observed_monetization,burden,maxf(0.0,treasury),WorldSimulation.state.private_currency,tax_rate,spending_priority)
	...
	return {"action":normalized,"requested":requested,"accepted":accepted,"reason":reason,"warning":warning,"before":{"available_metal":_available_metal_value(),"circulating_metal":WorldSimulation.state.weighed_metal_circulation,"reserve":_monetary_reserve_value(),"issue_ceiling":_monetary_reserve_value()*2.5,"money_supply":WorldSimulation.state.currency_supply,"transactional_money":WorldSimulation.state.private_currency,"transactional_share":WorldSimulation.state.private_currency/maxf(1.0,WorldSimulation.state.currency_supply),"treasury":WorldSimulation.state.public_treasury,"tax_rate":WorldSimulation.state.tax_rate,"trade_policy":WorldSimulation.state.external_trade_policy,"spending_priority":WorldSimulation.state.public_spending_priority,"fiscal_outlook":before_fiscal},"after":{"available_metal":maxf(0.0,available),"circulating_metal":maxf(0.0,circulation),"reserve":maxf(0.0,reserve),"issue_ceiling":maxf(0.0,reserve*2.5),"money_supply":maxf(0.0,supply),"transactional_money":WorldSimulation.state.private_currency,"transactional_share":WorldSimulation.state.private_currency/maxf(1.0,supply),"treasury":maxf(0.0,treasury),"tax_rate":tax_rate,"trade_policy":trade_policy,"spending_priority":spending_priority,"fiscal_outlook":after_fiscal}}
...
func issue_currency(requested:float,reason:String="Public issue")->Dictionary:
	initialize()
	if WorldSimulation.state.economy_stage!=STAGE_CURRENCY: return {"accepted":0.0,"reason":"Currency has not emerged"}
	var accepted:=_issue_currency_internal(requested,reason,1.0)
	return {"accepted":accepted,"requested":requested,"money_supply":WorldSimulation.state.currency_supply,"treasury":WorldSimulation.state.public_treasury,"reserve_ratio":_monetary_reserve_value()/maxf(1.0,WorldSimulation.state.currency_supply)}

func transfer_currency(requested:float,source:String,destination:String,reason:String="Currency transfer")->Dictionary:
	initialize()
	if WorldSimulation.state.economy_stage!=STAGE_CURRENCY: return {"accepted":0.0,"requested":requested,"reason":"Currency has not emerged"}
	...
	return {"accepted":accepted,"requested":requested,"source":from_account,"destination":to_account,"money_supply":WorldSimulation.state.currency_supply,"source_balance":_currency_account_balance(from_account),"destination_balance":_currency_account_balance(to_account)}
...
func _currency_account_balance(account:String)->float:
	match account:
		"public": return WorldSimulation.state.public_treasury
		"private": return WorldSimulation.state.private_currency
		"hoards": return WorldSimulation.state.currency_hoards
		"mutual_aid": return WorldSimulation.state.mutual_aid_reserve
	return 0.0

func _set_currency_account(account:String,value:float)->void:
	match account:
		"public": WorldSimulation.state.public_treasury=maxf(0.0,value)
		"private": WorldSimulation.state.private_currency=maxf(0.0,value)
		"hoards": WorldSimulation.state.currency_hoards=maxf(0.0,value)
		"mutual_aid": WorldSimulation.state.mutual_aid_reserve=maxf(0.0,value)

func commit_metal_to_reserve(requested_value:float,reason:String="Monetary reserve commitment")->Dictionary:
	initialize()
	if WorldSimulation.discovery.adoption("copper_smelting")<0.18: return {"accepted":0.0,"requested":requested_value,"reason":"Usable metal cannot yet be produced reliably"}
	...
	for resource_name_variant in processed_values:
		var resource_name:=String(resource_name_variant)
		WorldSimulation.state.monetary_reserve_metals[resource_name]=float(WorldSimulation.state.monetary_reserve_metals.get(resource_name,0.0))+float(processed_values[resource_name])
...
func place_weighed_metal_in_circulation(requested_value:float,reason:String="Metal placed in exchange")->Dictionary:
	...
	for resource_name_variant in processed_values:
		var resource_name:=String(resource_name_variant)
		WorldSimulation.state.weighed_metal_composition[resource_name]=float(WorldSimulation.state.weighed_metal_composition.get(resource_name,0.0))+float(processed_values[resource_name])
	WorldSimulation.state.weighed_metal_circulation=_weighed_metal_value()
	if accepted>0.0: _ledger("weighed_metal_placed",accepted,"material_stores","metal_circulation",reason)
	return {"accepted":accepted,"requested":requested_value,"consumed":transfer.consumed,"circulation":WorldSimulation.state.weighed_metal_circulation,"composition":WorldSimulation.state.weighed_metal_composition.duplicate(true),"available_metal":_available_metal_value(),"processing_efficiency":transfer.processing_efficiency}
...
func withdraw_weighed_metal(requested_value:float,reason:String="Metal withdrawn from exchange")->Dictionary:
	...
	for resource_name in names:
		if remaining<=0.0001: break
		var held_value:=maxf(0.0,float(WorldSimulation.state.weighed_metal_composition.get(resource_name,0.0)))
		var withdrawn_value:=minf(held_value,remaining)
		if withdrawn_value<=0.0: continue
		WorldSimulation.state.weighed_metal_composition[resource_name]=held_value-withdrawn_value
		if float(WorldSimulation.state.weighed_metal_composition[resource_name])<=0.0001: WorldSimulation.state.weighed_metal_composition.erase(resource_name)
		...
		WorldSimulation.state.resource_stockpiles[store_resource]=float(WorldSimulation.state.resource_stockpiles.get(store_resource,0.0))+store_quantity
	...
	return {"accepted":withdrawn,"requested":requested_value,"returned":returned,"circulation":WorldSimulation.state.weighed_metal_circulation,"composition":WorldSimulation.state.weighed_metal_composition.duplicate(true),"available_metal":_available_metal_value()}
...
func _take_physical_metal_from_stores(requested_value:float)->Dictionary:
	...
	for resource_name in ["Coin","Copper Ore","Tin Ore","Iron Ore"]:
		...
		WorldSimulation.state.resource_stockpiles[resource_name]=stock-raw_used
...
func release_surplus_reserve(requested_value:float,reason:String="Surplus reserve release")->Dictionary:
	...
	var required_reserve:=WorldSimulation.state.currency_supply/2.5 if WorldSimulation.state.economy_stage==STAGE_CURRENCY else 0.0
	...
	for reserve_name_variant in WorldSimulation.state.monetary_reserve_metals:
		reserve_names.append(String(reserve_name_variant))
	reserve_names.sort()
	for reserve_name in reserve_names:
		if remaining<=0.0001: break
		var held_value:=maxf(0.0,float(WorldSimulation.state.monetary_reserve_metals.get(reserve_name,0.0)))
		var released_value:=minf(held_value,remaining)
		if released_value<=0.0: continue
		WorldSimulation.state.monetary_reserve_metals[reserve_name]=held_value-released_value
		if float(WorldSimulation.state.monetary_reserve_metals[reserve_name])<=0.0001: WorldSimulation.state.monetary_reserve_metals.erase(reserve_name)
	...
	return {"accepted":released,"requested":requested_value,"releasable_before":releasable,"required_reserve":required_reserve,"reserve_value":_monetary_reserve_value(),"returned":returned,"money_supply":WorldSimulation.state.currency_supply}
...
func receive_war_wealth(amount:float,destination:String="public",reason:String="War wealth received")->Dictionary:
	...
	if WorldSimulation.state.economy_stage!=STAGE_CURRENCY:
		WorldSimulation.state.resource_stockpiles["Coin"]=float(WorldSimulation.state.resource_stockpiles.get("Coin",0.0))+accepted
	...
	WorldSimulation.state.monetary_reserve_metals["War Bullion"]=float(WorldSimulation.state.monetary_reserve_metals.get("War Bullion",0.0))+accepted
	...
	return {"accepted":accepted,"destination":"public" if to_public else "private","medium":"reserve-backed currency","material_deposited":0.0,"currency_deposited":deposited,"reserve_value":_monetary_reserve_value(),"money_supply":WorldSimulation.state.currency_supply}

func retire_currency(requested:float,reason:String="Currency retirement")->Dictionary:
	initialize()
	if WorldSimulation.state.economy_stage!=STAGE_CURRENCY: return {"accepted":0.0,"reason":"Currency has not emerged"}
	var accepted:=minf(maxf(0.0,requested),minf(WorldSimulation.state.public_treasury,WorldSimulation.state.currency_supply))
	WorldSimulation.state.public_treasury-=accepted
	WorldSimulation.state.currency_supply-=accepted
	WorldSimulation.state.currency_retired+=accepted
	if accepted>0.0: _ledger("currency_retired",accepted,"public","retired",reason)
	return {"accepted":accepted,"requested":requested,"money_supply":WorldSimulation.state.currency_supply,"treasury":WorldSimulation.state.public_treasury}
...
func _issue_currency_internal(requested:float,reason:String,public_share:float)->float:
	...
	var accepted:=minf(maxf(0.0,requested),maxf(0.0,maximum-WorldSimulation.state.currency_supply))
	var public_amount:=accepted*clampf(public_share,0.0,1.0)
	WorldSimulation.state.currency_supply+=accepted
	WorldSimulation.state.currency_issued+=accepted
	WorldSimulation.state.public_treasury+=public_amount
	WorldSimulation.state.private_currency+=accepted-public_amount
	if accepted>0.0: _ledger("currency_issued",accepted,"mint","circulation",reason)
	return accepted

func _validate_currency_conservation()->void:
	var accounted:=WorldSimulation.state.private_currency+WorldSimulation.state.currency_hoards+WorldSimulation.state.public_treasury+WorldSimulation.state.mutual_aid_reserve
	var difference:=WorldSimulation.state.currency_supply-accounted
	if absf(difference)<=0.0001: return
	if difference>0.0:
		WorldSimulation.state.private_currency+=difference
	else:
		var excess:=-difference
		var private_reduction:=minf(excess,WorldSimulation.state.private_currency)
		WorldSimulation.state.private_currency-=private_reduction
		excess-=private_reduction
		if excess>0.0:
			var treasury_reduction:=minf(excess,WorldSimulation.state.public_treasury)
			WorldSimulation.state.public_treasury-=treasury_reduction
			excess-=treasury_reduction
		if excess>0.0:
			var aid_reduction:=minf(excess,WorldSimulation.state.mutual_aid_reserve)
			WorldSimulation.state.mutual_aid_reserve-=aid_reduction
			excess-=aid_reduction
		if excess>0.0: WorldSimulation.state.currency_hoards=maxf(0.0,WorldSimulation.state.currency_hoards-excess)
...
func accounting_audit()->Dictionary:
	...
	var currency_accounts:=WorldSimulation.state.private_currency+WorldSimulation.state.currency_hoards+WorldSimulation.state.public_treasury+WorldSimulation.state.mutual_aid_reserve
	var currency_difference:=WorldSimulation.state.currency_supply-currency_accounts
	...
	if WorldSimulation.state.economy_stage==STAGE_CURRENCY and WorldSimulation.state.currency_supply>currency_ceiling+0.001: violations.append("currency supply exceeds committed reserve ceiling")
	var expected_trade_claims:=WorldSimulation.state.external_trade_exports-WorldSimulation.state.external_trade_imports-WorldSimulation.state.external_trade_losses
	var trade_difference:=WorldSimulation.state.external_trade_credit-expected_trade_claims
	if absf(trade_difference)>0.001: violations.append("regional trade claims do not reconcile")
	if WorldSimulation.state.external_trade_credit< -0.001: violations.append("regional trade claims are negative")
	if WorldSimulation.state.public_debt< -0.001: violations.append("public debt is negative")
	if WorldSimulation.state.mutual_aid_reserve< -0.001: violations.append("mutual-aid reserve is negative")
	if WorldSimulation.state.currency_hoards< -0.001: violations.append("currency hoards are negative")
	if WorldSimulation.state.in_kind_labor_arrears< -0.001: violations.append("in-kind labor arrears are negative")
	if WorldSimulation.state.in_kind_material_arrears< -0.001: violations.append("in-kind material arrears are negative")
	var weighed_metal_composed:=_weighed_metal_value()
	var weighed_metal_difference:=WorldSimulation.state.weighed_metal_circulation-weighed_metal_composed
	if absf(weighed_metal_difference)>0.001: violations.append("weighed-metal circulation differs from its physical composition")
	if WorldSimulation.state.weighed_metal_circulation< -0.001: violations.append("weighed-metal circulation is negative")
	if WorldSimulation.state.weighed_metal_losses< -0.001: violations.append("weighed-metal losses are negative")
	for resource_name_variant in WorldSimulation.state.resource_stockpiles:
		var resource_name:=String(resource_name_variant)
		if float(WorldSimulation.state.resource_stockpiles[resource_name])< -0.001: violations.append("negative material store: "+resource_name)
	for reserve_name_variant in WorldSimulation.state.monetary_reserve_metals:
		var reserve_name:=String(reserve_name_variant)
		if float(WorldSimulation.state.monetary_reserve_metals[reserve_name])< -0.001: violations.append("negative monetary reserve: "+reserve_name)
	return {"ok":violations.is_empty(),"violations":violations,"currency_difference":currency_difference,"currency_ceiling":currency_ceiling,"reserve_value":reserve_value,"trade_difference":trade_difference,"expected_trade_claims":expected_trade_claims,"currency_hoards":WorldSimulation.state.currency_hoards,"weighed_metal_difference":weighed_metal_difference,"weighed_metal_circulation":WorldSimulation.state.weighed_metal_circulation,"weighed_metal_losses":WorldSimulation.state.weighed_metal_losses,"public_debt":WorldSimulation.state.public_debt,"mutual_aid_reserve":WorldSimulation.state.mutual_aid_reserve,"in_kind_labor_arrears":WorldSimulation.state.in_kind_labor_arrears,"in_kind_material_arrears":WorldSimulation.state.in_kind_material_arrears}

func _ledger(kind:String,amount:float,source:String,destination:String,memo:String)->void:
	WorldSimulation.state.economic_ledger.append({"day":int(WorldSimulation.state.elapsed_days),"kind":kind,"amount":amount,"source":source,"destination":destination,"memo":memo})
	if WorldSimulation.state.economic_ledger.size()>1000: WorldSimulation.state.economic_ledger.pop_front()

func quote(resource_name:String,quantity:float=1.0)->Dictionary:
	initialize()
	var observed:=WorldSimulation.state.market_prices.has(resource_name) and int(WorldSimulation.state.economy_metrics.get("price_observations",0))>0
	var unit:=float(WorldSimulation.state.market_prices.get(resource_name,0.0)) if observed else 0.0
	return {"resource":resource_name,"quantity":maxf(0.0,quantity),"unit_value":unit,"total_value":unit*maxf(0.0,quantity),"observed":observed,"stage":WorldSimulation.state.economy_stage,"settlement_medium":settlement_medium()}
...
func known_market_snapshot()->Array[Dictionary]:
	...
	if int(WorldSimulation.state.economy_metrics.get("price_observations",0))<=0:
		return result
	for resource_name_variant in BASE_VALUES:
		var resource_name:=String(resource_name_variant)
		var stock:=float(WorldSimulation.state.resource_stockpiles.get(resource_name,0.0))
		...
		result.append({"resource":resource_name,"stock":stock,"unit_value":float(WorldSimulation.state.market_prices.get(resource_name,BASE_VALUES[resource_name])),"base_value":float(BASE_VALUES[resource_name]),"trend_30d":trend.change,"high_30d":trend.high,"low_30d":trend.low})
...
func market_trend(resource_name:String,lookback_days:int=30)->Dictionary:
	initialize()
	if not WorldSimulation.state.market_prices.has(resource_name):
		return {"resource":resource_name,"days":lookback_days,"first":0.0,"current":0.0,"change":0.0,"high":0.0,"low":0.0,"observed":false}
	var current:=float(WorldSimulation.state.market_prices.get(resource_name,0.0))
	var first:=current
	var high:=current
	var low:=current
	var found:=false
	var start:=maxi(0,WorldSimulation.state.economy_history.size()-maxi(1,lookback_days))
	for index in range(start,WorldSimulation.state.economy_history.size()):
		var prices:Dictionary=WorldSimulation.state.economy_history[index].get("prices",{})
...
func benchmark_status()->Dictionary:
	...
	if WorldSimulation.state.economy_stage==STAGE_METAL:
	...
	return {"next_stage":"Mature currency institutions","requirements":[
		{"name":"Exchange reliability","value":float(WorldSimulation.state.economy_metrics.get("exchange_reliability",0.0)),"target":0.75},
		{"name":"Reserve ratio","value":_monetary_reserve_value()/maxf(1.0,WorldSimulation.state.currency_supply),"target":0.50},
		{"name":"Market access","value":float(WorldSimulation.state.economy_metrics.get("market_access",0.0)),"target":0.70},
		{"name":"Public credit practice","value":WorldSimulation.discovery.adoption("public_credit"),"target":0.50}
	]}
...
func _event(title:String,description:String,severity:String)->Dictionary:
	return {"day":int(WorldSimulation.state.elapsed_days),"title":title,"description":description,"domain":"economy","severity":severity}
