...
func home_unavailable()->bool:
	var home:=WorldSimulation.settlements.settlement_record(WorldSimulation.settlements._primary_settlement_id())
...
func prepare(people:int,days:int)->Dictionary:
	...
	if WorldSimulation.military.active_siege.is_empty() or String(WorldSimulation.military.active_siege.mode)!="defensive":return {"error":"Escape preparation requires a siege of your home settlement."}
	if not data.preparation.is_empty() or not data.remnant.is_empty():return {"error":"An escape group is already committed."}
	if bool(WorldSimulation.state.settlement_convoy.get("active",false)):return {"error":"An existing founding convoy must finish before another displaced group is organized."}
	var available:=maxi(0,floori(WorldSimulation.settlements.primary_population_exact())-WorldSimulation.military._mobilized_count()-int(WorldSimulation.world.player_population_commitments().total_absent)-1)
	var limit:=mini(available,floori(WorldSimulation.settlements.primary_population_exact()*.35))
	...
	if WorldSimulation.food.total_stored()<food:return {"error":"The group needs %.0f food rations from your actual stores." % food}
	if float(WorldSimulation.state.resource_stockpiles.get("Timber",0))<timber:return {"error":"The group needs %.1f Timber for portable shelter and rebuilding." % timber}
	var issued:=WorldSimulation.food.issue_for_obligation(food,"siege_escape","Escape group supplies",days,people)
	WorldSimulation.state.resource_stockpiles.Timber=float(WorldSimulation.state.resource_stockpiles.get("Timber",0))-timber
	var cohorts:Dictionary={}
	for key in WorldSimulation.state.population_cohorts:cohorts[key]=float(WorldSimulation.state.population_cohorts[key])*people/maxf(1,WorldSimulation.state.population_exact)
	var functions:Dictionary=WorldSimulation.state.proportional_population_commitment(people)
	functions.productive=int(functions.productive)+int(functions.mobilized);functions.mobilized=0
	WorldSimulation.settlements.settlement_record(WorldSimulation.settlements._primary_settlement_id()).recovery_capital=true
	data.preparation={"people":people,"food":issued,"timber":timber,"cohorts":cohorts,"functions":functions,"prepared_day":int(WorldSimulation.state.elapsed_days),"siege_id":String(WorldSimulation.military.active_siege.id),"origin_city":WorldSimulation.settlements._primary_settlement_id(),"origin_name":WorldSimulation.state.settlement_name,"origin_position":WorldSimulation.world.city_intelligence.point(WorldSimulation.world.player_world_origin),"mortality_remainder":0.0}
...
func defense_factor()->float:
	return clampf(1.0-float(absent_people())/maxf(1,WorldSimulation.settlements.primary_population_exact())*.8,.5,1)
func escape(heading:String="east")->Dictionary:
	...
	if siege.is_empty():
		for past:Dictionary in WorldSimulation.military.siege_history:
	...
	var start:=WorldSimulation.world.city_intelligence.vector(preparation.origin_position)
	...
	var route:Dictionary=WorldSimulation.world._plan_scout_land_route(start,destination)
	if not bool(route.get("ok",false)):return {"error":String(route.get("reason","No escape route is available."))}
	var preparation_days:=maxi(0,int(WorldSimulation.state.elapsed_days)-int(preparation.prepared_day))
	...
	var rng:=RandomNumberGenerator.new();rng.seed=WorldSimulation.state.world_seed^int((siege.get("threat",{}) as Dictionary).get("seed",0))^int(data.attempts)*104729
	...
	remnant.merge({"position":preparation.origin_position.duplicate(true),"route":route.route,"distance":float(route.distance_km),"traveled":0.0,"destination":WorldSimulation.world.city_intelligence.point(destination),"phase":"escaping","heading":heading,"depart_day":int(WorldSimulation.state.elapsed_days)},true)
...
func capture_capacity(force:Dictionary)->Dictionary:
	var population:=maxf(1,WorldSimulation.settlements.primary_population_exact())
	var cohesion:=clampf(float(WorldSimulation.state.simulation_metrics.get("cohesion",.58)),0,1)
...
func capture(civ_id:String,force:Dictionary={},yielded:bool=false)->Dictionary:
	var city:=WorldSimulation.settlements.settlement_record(WorldSimulation.settlements._primary_settlement_id())
	...
	var region:Dictionary={"population":maxf(1,WorldSimulation.settlements.primary_population_exact()),"resistance":.45,"integration":0.0,"damage":.35}
	var government:=GOVERNANCE.state(region)
	var index:int=WorldSimulation.world._civilization_index(civ_id)
	if index>=0 and float(WorldSimulation.world.civilizations[index].get("aggression",0))>.6:government.policy="military_rule"
	region.governance=government
	var entry:Dictionary={"city_id":String(city.id),"occupier":civ_id,"captured_day":int(WorldSimulation.state.elapsed_days),"region":region,"order":{},"report":{},"report_due":-1,"last_report_day":int(WorldSimulation.state.elapsed_days),"next_order_day":int(WorldSimulation.state.elapsed_days)}
	data.occupied.append(entry)
	if int(data.last_day)<0:data.last_day=int(WorldSimulation.state.elapsed_days)
	var active_people:=int(WorldSimulation.military.home_army.get("troops",0))+int(WorldSimulation.military.home_army.get("wounded_pool",0))+int(WorldSimulation.military.home_army.get("scattered_pool",0))+WorldSimulation.military.aggregate_recruits+WorldSimulation.military._queued_trainees()+WorldSimulation.military.training_injury_pool
	entry.captive_people=active_people
	entry.captive_injuries=int(WorldSimulation.military.home_army.get("wounded_pool",0))+WorldSimulation.military.training_injury_pool
	entry.captive_disabled=int(WorldSimulation.military.home_army.get("severe_disabled_pool",0))
	entry.seized_training=WorldSimulation.military.training_queue.duplicate(true)
	WorldSimulation.military.aggregate_recruits=0;WorldSimulation.military.training_queue.clear();WorldSimulation.military.training_injury_pool=0
	entry.seized_production=WorldSimulation.military.equipment_queue.duplicate(true);WorldSimulation.military.equipment_queue.clear()
	WorldSimulation.military.home_army.captured_pool=int(WorldSimulation.military.home_army.get("captured_pool",0))+active_people
	WorldSimulation.military.home_army.formations=[];WorldSimulation.military.home_army.troops=0
	for key in ["wounded_pool","disabled_pool","severe_disabled_pool","scattered_pool"]:WorldSimulation.military.home_army[key]=0
	entry["seized_equipment"]=WorldSimulation.military.military_inventory.duplicate(true)
	entry["seized_consumables"]=WorldSimulation.military.military_consumables.duplicate(true)
...
func _activate_capital(city:Dictionary)->void:
	var previous:=WorldSimulation.settlements.settlement_record(WorldSimulation.settlements._primary_settlement_id())
	if String(previous.get("id",""))==String(city.id):return
	var old_count:=maxf(1,WorldSimulation.settlements.primary_population_exact())
	var layout_fields:Array[String]=["settlement_nuclei","settlement_routes"]
	previous.recovery_layout={}
	for field in layout_fields:previous.recovery_layout[field]=WorldSimulation.state.get(field).duplicate(true)
	var old_resources:Dictionary={}
	for key in SettlementModel.CITY_RESOURCE_DEFAULTS:
		var value:Variant=WorldSimulation.state.get(key)
		old_resources[key]=value.duplicate(true) if value is Dictionary or value is Array else value
	previous.local_resources=old_resources;previous.primary=false;previous.population_share=old_count/maxf(1,WorldSimulation.state.population_exact)
	WorldSimulation.settlements._ensure_city_resources(city)
	...
	for key in SettlementModel.CITY_RESOURCE_DEFAULTS:
		...
		WorldSimulation.state.set(key,value.duplicate(true) if value is Dictionary or value is Array else value)
	for field in layout_fields:
		var target:Array=WorldSimulation.state.get(field)
	...
	WorldSimulation.state.settlement_name=String(city.name)
	WorldSimulation.state.settlement_founded_day=int(city.get("founded_day",WorldSimulation.state.elapsed_days))
	var point:Vector2=WorldSimulation.settlements._record_position(city)
	WorldSimulation.state.settlement_founded_at=Vector3(point.x,0,point.y);WorldSimulation.state.selected_player_settlement_id=String(city.id)
	WorldSimulation.world.register_player_origin(point)
	WorldSimulation.state.settlement_network_revision+=1;WorldSimulation.state.morphology_revision+=1
	WorldSimulation.settlements.ensure_founded()
	# Construction and equipment at the captured place never move with the title.
	WorldSimulation.military.settlement_defense={"stage":0,"integrity":1.0,"project_stage":-1,"project_progress":0.0,"reserved_materials":{}}
func queue_resistance(city_id:String,order:String)->Dictionary:
	...
	for entry:Dictionary in data.occupied:
		...
		if not entry.order.is_empty() or int(WorldSimulation.state.elapsed_days)<int(entry.next_order_day):return {"error":"The current local effort needs time before another can begin."}
		...
		var stored:float=WorldSimulation.settlements.with_city_resources(city_id,func()->float:return WorldSimulation.food.total_stored())
		if stored<food_cost:return {"error":"Residents need %.0f local rations to sustain this effort." % food_cost}
		var delay:=0 if home_unavailable() else maxi(1,ceili(WorldSimulation.settlements._record_position(city).distance_to(WorldSimulation.world.player_world_origin)/17.0))
		entry.order={"kind":order,"arrival_day":int(WorldSimulation.state.elapsed_days)+delay,"resolve_day":int(WorldSimulation.state.elapsed_days)+delay+30,"food_cost":food_cost,"funded":false}
		entry.next_order_day=int(WorldSimulation.state.elapsed_days)+delay+60
...
func _resolve_resistance(entry:Dictionary,day:int)->void:
	...
	var rng:=RandomNumberGenerator.new();rng.seed=WorldSimulation.state.world_seed^day*8191^hash(String(entry.city_id))
	var success:=false
	...
	if success:
		var city:=WorldSimulation.settlements.settlement_record(String(entry.city_id));city.erase("occupied_by")
		entry["liberated"]=true
		var released:=mini(int(entry.get("captive_people",0)),int(WorldSimulation.military.home_army.get("captured_pool",0)))
		WorldSimulation.military.home_army.captured_pool=int(WorldSimulation.military.home_army.get("captured_pool",0))-released
		WorldSimulation.state.receive_injured_veterans(mini(released,int(entry.get("captive_injuries",0))),mini(released,int(entry.get("captive_disabled",0))))
	...
	var city:=WorldSimulation.settlements.settlement_record(String(entry.city_id))
	var delay:=0 if home_unavailable() else maxi(1,ceili(WorldSimulation.settlements._record_position(city).distance_to(WorldSimulation.world.player_world_origin)/17))
	entry.report_due=day+delay
func assess_site(point:Vector2,actual:bool=false)->Dictionary:
	if not WorldSimulation.world._scout_land_at(point):return {"valid":false,"reason":"This site is not traversable land."}
	...
	if not WorldSimulation.world.ground_survey_authority.is_valid():return {"valid":false,"reason":"Water and terrain conditions are not surveyed."}
	var ground:Dictionary=WorldSimulation.world.ground_survey_authority.call(point)
	...
	var cities:Array=WorldSimulation.world.city_intelligence.sites() if actual else WorldSimulation.world.city_intelligence.known_cities()
	for site:Dictionary in cities:
		var center:=WorldSimulation.world.city_intelligence.vector(site.position)
		var clearance:=8.0
		if actual and String(site.civ_id)!="player":
			var index:int=WorldSimulation.world._civilization_index(String(site.civ_id))
			if index>=0:clearance+=float(WorldSimulation.world.civilizations[index].logistics)*50
...
func seek_site(heading:String,distance:float=32)->Dictionary:
	...
	var start:=WorldSimulation.world.city_intelligence.vector(data.remnant.position)
	var angle:=deg_to_rad(float(CivilizationSystem.SCOUT_HEADINGS.get(heading,0)))
	...
	data.remnant.merge({"route":route.route,"distance":float(route.distance_km),"traveled":0.0,"destination":WorldSimulation.world.city_intelligence.point(destination),"phase":"seeking_site"},true)
...
func _resettle()->Dictionary:
	var remnant:Dictionary=data.remnant;var point:=WorldSimulation.world.city_intelligence.vector(remnant.position)
	...
	if WorldSimulation.state.player_settlements.size()>=SettlementModel.MAX_PLAYER_SETTLEMENTS:return {"error":"Settlement capacity is full."}
	var sequence:=WorldSimulation.state.next_player_settlement_id;WorldSimulation.state.next_player_settlement_id+=1
	var city:Dictionary={"id":"settlement_%03d" % sequence,"sequence":sequence,"primary":false,"name":"New "+String(remnant.origin_name),"position":point,"population_share":float(remnant.people)/maxf(1,WorldSimulation.state.population_exact),"founded_day":int(WorldSimulation.state.elapsed_days),"status":"established","source_settlement_id":String(remnant.origin_city),"territory_context":{},"environment_profile":{},"auto_manage":true,"management_focus":"establishment","leader_person_id":0,"recovery_capital":true}
	data.remnant={}
	WorldSimulation.state.player_settlements.append(city);WorldSimulation.settlements._ensure_city_resources(city)
	city.local_resources.resource_stockpiles.Timber=float(remnant.timber)
	WorldSimulation.settlements.with_city_resources(String(city.id),func()->void:WorldSimulation.food.receive_external_food(float(remnant.food)))
...
func _day(day:int)->void:
	for group:Dictionary in [data.preparation,data.remnant]:
		...
		var people:=mini(int(group.people),maxi(0,WorldSimulation.state.population_total-1));group.people=people
	...
	if not data.remnant.is_empty():
		...
		if int(remnant.people)<=0:data.remnant={};_record("The escaped group did not survive.")
			...
			var position:Vector2=WorldSimulation.world.city_intelligence.route_position(remnant.route,float(remnant.traveled)/maxf(.001,float(remnant.distance)))
			remnant.position=WorldSimulation.world.city_intelligence.point(position)
	...
	for entry:Dictionary in data.occupied:
		if bool(entry.get("liberated",false)):
			...
			continue
		var city_record:=WorldSimulation.settlements.settlement_record(String(entry.city_id))
		entry.region.population=WorldSimulation.settlements._settlement_population(city_record)
		var welfare:=float(entry.region.governance.get("welfare",.5))
		var food_report:Dictionary=WorldSimulation.settlements.with_city_resources(String(entry.city_id),func()->Dictionary:return WorldSimulation.food.process_day({"traveling":false},.3+welfare*.4,.6))
		...
		if day%30==0:
			var index:int=WorldSimulation.world._civilization_index(String(entry.occupier))
			var occupier:Dictionary=WorldSimulation.world.civilizations[index] if index>=0 else {}
			var presence:={"control":clampf(float(occupier.get("military_population",0))/maxf(1,float(entry.region.population)*.07),0,1),"logistics":float(occupier.get("logistics",.2))}
			if WorldSimulation.enabled:presence=preload("res://scripts/civilization_combat.gd").occupation_presence(String(entry.occupier),String(entry.city_id))
			entry.region=GOVERNANCE.advance(entry.region,float(presence.control),float(presence.logistics),false)
		if not entry.order.is_empty() and day>=int(entry.order.arrival_day) and not bool(entry.order.get("funded",false)):
			...
			var available:float=WorldSimulation.settlements.with_city_resources(String(entry.city_id),func()->float:return WorldSimulation.food.total_stored())
			if available>=needed:
				WorldSimulation.settlements.with_city_resources(String(entry.city_id),func()->void:WorldSimulation.food.issue_for_obligation(needed,"resistance","Local recovery effort",30,0))
				...
				entry.report_due=day+maxi(1,ceili(WorldSimulation.settlements._record_position(city_record).distance_to(WorldSimulation.world.player_world_origin)/17))
...
func _record(message:String)->void:
	data.history.push_front({"day":int(WorldSimulation.state.elapsed_days),"message":message})
	while data.history.size()>32:data.history.pop_back()
	WorldSimulation.state.simulation_events.push_front({"day":int(WorldSimulation.state.elapsed_days),"title":"Survival and independence","description":message,"domain":"institutions","severity":"major"})
...
func cancel_preparation()->Dictionary:
	...
	var city:=WorldSimulation.settlements.settlement_record(String(group.origin_city))
	if not String(city.get("occupied_by","")).is_empty():return {"error":"These supplies are no longer under your control."}
	WorldSimulation.settlements.with_city_resources(String(group.origin_city),func()->void:
		WorldSimulation.food.receive_external_food(float(group.food))
		WorldSimulation.state.resource_stockpiles.Timber=float(WorldSimulation.state.resource_stockpiles.get("Timber",0))+float(group.timber)
	)
	data.preparation={}
...
func _lose_people(count:int,reason:String,city_id:String="")->Dictionary:
	...
	for city:Dictionary in WorldSimulation.state.player_settlements:
		counts[String(city.id)]=WorldSimulation.settlements._settlement_population(city)
	var result:=WorldSimulation.state.register_population_deaths(count,reason)
	for city:Dictionary in WorldSimulation.state.player_settlements:
...
func capture_city(city_id:String,civ_id:String,force:Dictionary)->Dictionary:
	...
	data.occupied.append({"city_id":city_id,"occupier":civ_id,"captured_day":int(WorldSimulation.state.elapsed_days),"region":region,"order":{},"report":region.duplicate(true),"report_due":-1,"last_report_day":int(WorldSimulation.state.elapsed_days),"next_order_day":int(WorldSimulation.state.elapsed_days)})
	if int(data.last_day)<0:data.last_day=int(WorldSimulation.state.elapsed_days)
	return {"ok":true,"occupation_required":required,"message":"The city is occupied. Its people and stores remain local."}
