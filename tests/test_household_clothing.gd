...
func equip(id:String)->void:
	...
	for item:String in spec.cost:WorldSimulation.state.resource_stockpiles[item]=1000.0
...
func test_six_methods_have_real_inputs_and_graph_contracts()->void:
...
func test_full_save_file_restores_clothes_and_accepts_missing_legacy_field()->void:
	...
	prepare();equip("knitted_loop_fabrics");C.add("knit",12,.8,.2,false,false,3)
...
func test_secondary_city_clothes_are_local_and_save_separately()->void:
	WorldSimulation.scoped("clothes",func()->void:
		...
		WorldSimulation.settlements.with_city_resources("second",func()->void:
			assert_float(C.count()).is_equal(0.0);C.add("pile",3)
	...
	WorldSimulation.scoped("clothes",func()->void:
		assert_float(C.count()).is_equal(20.0)
		WorldSimulation.settlements.with_city_resources("second",func()->void:assert_float(C.count()).is_equal(3.0))
	)
