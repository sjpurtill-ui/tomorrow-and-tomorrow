...
func test_world_board_uses_returned_reports_and_opens_archive()->void:
	...
	assert_str(String(data.blocks[1].type)).is_equal("world_board")
	assert_str(String(data.blocks[0].items[2].value)).is_equal(str(CivilizationSystem.scout_reports.size()))
	data.blocks[1].actions[1].on_press.call()
...
func test_returned_finding_opens_its_report_and_live_count_updates()->void:
	...
	var found:=false
	for item:Dictionary in data.blocks[1].sections[1].items:
