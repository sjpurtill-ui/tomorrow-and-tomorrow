...
func test_annual_expedition_creates_a_strategic_find_and_named_place()->void:
	...
	var line:=CivilizationSystem._resolve_scout_landmark(mission,route,1524)
	assert_str(line).is_not_empty()
	assert_bool(mission.discoveries.any(func(item:Dictionary)->bool: return String(item.kind)=="landmark")).is_true()
...
func test_no_landmarks_or_deposits_are_invented_on_open_water()->void:
	...
	CivilizationSystem._resolve_scout_windfalls(mission,route,1524)
	assert_str(CivilizationSystem._resolve_scout_landmark(mission,route,1524)).is_empty()
