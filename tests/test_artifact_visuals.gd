...
func test_illustrated_collection_stays_inside_small_and_large_viewports()->void:
	...
	for shape:Vector2i in [Vector2i(340,640),Vector2i(960,720)]:
		...
		for node:Node in sheet.find_children("*","TextureRect",true,false):
			if node.custom_minimum_size.y in [72.0,160.0]:artwork=node;break
...
func test_legacy_exploration_record_uses_its_stable_prehistoric_catalogue_art()->void:
	var legacy:={"kind":"artifact","name":"Old decorated-object description","source_id":"","catalogue_id":0}
	assert_str(Art.image_path(legacy)).is_equal("res://assets/ui/artifacts/prehistoric-v1/artifact-0000.png")
	assert_str(Art.presentation(legacy).name).is_equal("Irregular rough stone bowl · earth-darkened")
