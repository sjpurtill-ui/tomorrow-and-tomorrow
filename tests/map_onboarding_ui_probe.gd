...
func _ready()->void:
	...
	_expect(terrain.population_summary_label and terrain.population_summary_label.text=="SETTLEMENT","top bar did not expose Settlement as a direct destination")
	_expect(terrain.provisions_button and terrain.provisions_button.text=="ECONOMY","top bar did not consolidate provisions and materials into Economy")
	_expect(terrain.materials_button and not terrain.materials_button.visible,"legacy Materials status chip remained as a duplicate permanent destination")
	_expect(ResourceSystem.stored_bulk()>0.0 and "bulk" in terrain.materials_button.tooltip_text,"physical founding cargo disappeared from the Economy explanation")
	...
	_expect(terrain.map_help_body and "Click land" in terrain.map_help_body.text and "START SETTLEMENT" in terrain.map_help_body.text,"first-use help did not explain the immediate founding action")
	...
	_expect(terrain.actions_menu_button and "NEXT" in terrain.actions_menu_button.text,"new game did not identify ACTIONS as the next command")
	terrain._toggle_actions_menu()
	_expect(terrain.actions_menu_panel.visible,"ACTIONS did not open its contextual menu")
	_expect("START SETTLEMENT HERE" in terrain.actions_menu_settlement_button.text,"founding action was not explicit")
	_expect(terrain.actions_menu_diplomat_button.disabled and "LOCATE A FOREIGN SETTLEMENT FIRST" in terrain.actions_menu_diplomat_button.text,"diplomacy did not expose its physical-destination blocker")
	_expect(terrain.resource_view_toggle and terrain.resource_view_toggle.position.x+terrain.resource_view_toggle.size.x<=viewport_size.x and terrain.resource_view_toggle.position.y+terrain.resource_view_toggle.size.y<=viewport_size.y,"resource toggle was not a bounded lower-right map control")
	terrain._close_actions_menu()
