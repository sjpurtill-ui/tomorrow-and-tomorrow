...
const IDS=["zipper_chain_closures","leather_thickness_skiving"]
const ITEMS=["zipper_forming_dies","zipper_tape_pairs","zipper_element_rows","zipper_slider_stops","matched_zipper_chains","checked_garment_zippers","zippered_garment_fronts","leather_skiving_knives","leather_edge_blanks","skived_leather_panels","checked_skived_panels","folded_skived_panels"]
...
func test_exact_predicates_and_fractional_paid_recipe_save_continuation()->void:
	WorldSimulation.scoped("closures",func()->void:
		assert_array(K.METHODS[IDS[0]].requires_all).is_equal(["sewing_machine_mechanisms","dimensional_metrology"])
		assert_array(K.METHODS[IDS[1]].requires_all).is_equal(["leather_goods_patterning","dimensional_metrology"])
		...
		for item:String in ITEMS:
			...
			for resource:String in spec.get("co_products",{}):assert_float(float(WorldSimulation.state.resource_stockpiles.get(resource,0))-float(before.get(resource,0))).is_equal_approx(float(spec.co_products[resource]),.000001)
			...
			WorldSimulation.military.cancel_equipment_job(int(job.id)))
func test_manufactured_matched_zipper_parts_and_checked_fronts_become_garments()->void:
	WorldSimulation.scoped("closures",func()->void:
		prepare(IDS[0]);stock_external_inputs()
		for item:String in ITEMS.slice(0,7):finish(item,8 if item=="zipper_forming_dies" else 5 if item in ["zipper_tape_pairs","zipper_element_rows","zipper_slider_stops","matched_zipper_chains"] else 3)
		assert_bool(C.install(IDS[0]).get("ok",false)).is_true()
		var fronts:=C.available("Zippered Garment Fronts");var r:=report();C.operate(IDS[0],1,100,0,r)
		assert_float(C.count()).is_greater(0);assert_str(C.data().lots[0].kind).is_equal("fit")
		assert_float(C.available("Zippered Garment Fronts")).is_less(fronts)
...
func test_manufactured_skived_checked_and_folded_panels_feed_leather()->void:
	WorldSimulation.scoped("closures",func()->void:
		prepare(IDS[1]);stock_external_inputs()
		for item:String in ITEMS.slice(7):finish(item,8 if item=="leather_skiving_knives" else 5 if item in ["leather_edge_blanks","skived_leather_panels"] else 3)
		assert_bool(C.install(IDS[1]).get("ok",false)).is_true()
		var panels:=C.available("Folded Skived Leather Panels");var r:=report();C.operate(IDS[1],1,100,0,r)
		assert_float(C.count()).is_greater(0);assert_str(C.data().lots[0].kind).is_equal("leather")
		assert_float(C.available("Folded Skived Leather Panels")).is_less(panels)
...
func test_unchecked_parts_and_missing_slider_cannot_substitute_for_finished_closure()->void:
	WorldSimulation.scoped("closures",func()->void:
		for id:String in IDS:
			prepare(id);C.install(id)
			var output:String="Zippered Garment Fronts" if id==IDS[0] else "Folded Skived Leather Panels"
			WorldSimulation.state.resource_stockpiles[output]=0.0
			for raw:String in ["Zipper Element Tape Pairs","Assembled Garment Zippers","Gauged Leather Edge Blanks","Flexible Leather"]:WorldSimulation.state.resource_stockpiles[raw]=100.0
			C.operate(id,10,100,0,report());assert_float(C.count()).is_equal(0.0)
		stock_external_inputs();finish("zipper_forming_dies",8);finish("zipper_tape_pairs");finish("zipper_element_rows")
		var before:Dictionary=WorldSimulation.state.resource_stockpiles.duplicate(true)
		var blocked:=WorldSimulation.military.start_production_line("matched_zipper_chains",1)
		assert_bool(blocked.has("error")).is_true();assert_str(String(blocked.get("error",""))).contains("Zipper Slider and Stop Sets")
		assert_dict(WorldSimulation.state.resource_stockpiles).is_equal(before);assert_float(C.available("Assembled Garment Zippers")).is_equal(0.0))
func test_paid_installation_known_prerequisites_and_daily_work_do_not_repeat()->void:
	WorldSimulation.scoped("closures",func()->void:
		prepare(IDS[0]);WorldSimulation.state.known_discoveries.erase("dimensional_metrology")
		assert_bool(C.quote(IDS[0]).has("error")).is_true();learn("dimensional_metrology")
		var needles:=C.available("Treadle Sewing Machines")
		WorldSimulation.state.elapsed_days=1;var result:=C.advance(5,100,false)
		assert_int(int(C.data().tools.get(IDS[0],0))).is_equal(1)
		assert_float(C.available("Treadle Sewing Machines")).is_equal(needles-1)
...
func test_planner_orders_missing_checked_front_and_controller_supplies_daily_consumer()->void:
	WorldSimulation.scoped("closures",func()->void:
		prepare(IDS[0]);stock_external_inputs()
		for item:String in ITEMS.slice(0,6):finish(item,8 if item=="zipper_forming_dies" else 5 if item in ["zipper_tape_pairs","zipper_element_rows","zipper_slider_stops","matched_zipper_chains"] else 3)
		var order:=F.clothing_recommendation();assert_str(String(order.get("item",""))).is_equal("zippered_garment_fronts")
		...
		var fronts:=C.available("Zippered Garment Fronts");assert_float(fronts).is_greater(0)
		WorldSimulation.state.elapsed_days=1;C.advance(5,100,false)
		assert_float(C.count()).is_greater(0);assert_float(C.available("Zippered Garment Fronts")).is_less(fronts))
func test_full_save_preserves_partial_closure_work_and_actor_garments()->void:
	WorldSimulation.scoped("closures",func()->void:
		prepare(IDS[0]);stock_external_inputs()
		for item:String in ITEMS.slice(0,6):finish(item,8 if item=="zipper_forming_dies" else 5 if item in ["zipper_tape_pairs","zipper_element_rows","zipper_slider_stops","matched_zipper_chains"] else 3)
		var job:=start("zippered_garment_fronts");P.advance(WorldSimulation.military,job,1.0);C.add("fit",2,.6))
	WorldSimulation.create_actor("other_closures",1262)
	WorldSimulation.scoped("other_closures",func()->void:C.add("leather",3,.8))
	var slot:="zipper_skiving_%d"%OS.get_process_id();assert_bool(SaveSystem.save_game(slot).get("ok",false)).is_true()
	...
	WorldSimulation.scoped("closures",func()->void:
		...
		assert_int(int(job.completed)).is_equal(1);assert_float(C.available("Zippered Garment Fronts")).is_equal(1.0)
