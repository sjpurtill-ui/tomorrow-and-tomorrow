...
func _run_capture_and_recapture_loop()->void:
	...
	var field_before_reinforcement:=int(MilitaryCampaign.home_army.troops)
	...
	assert(int(MilitaryCampaign.home_army.troops)<field_before_reinforcement)
	...
	var restation:=MilitaryCampaign.establish_occupation_force(String(civ.id),captured,float(target.occupation_required))
