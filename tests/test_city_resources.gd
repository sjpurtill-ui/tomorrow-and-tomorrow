...
func test_selection_does_not_change_simulation_or_primary_name()->void:
	assert_bool(SettlementModel.select_settlement("dawngate")).is_true()
	...
	assert_bool(SettlementModel.select_settlement("missing")).is_false()
	assert_str(GameState.selected_settlement_id).is_equal("dawngate")
...
func test_arrival_preserves_only_conserved_convoy_cargo()->void:
	...
	assert_float(float(stores.Timber)).is_equal(6.0)
