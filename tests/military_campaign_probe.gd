...
func _run()->void:
	...
	assert(String((retreat_result.get("termination",{}) as Dictionary).get("type",""))=="withdrawal")
	assert((retreat_result.get("termination",{}) as Dictionary).has("pursuit_pressure"))
	var retreat_prisoners:=int((retreat_result.get("termination",{}) as Dictionary).get("prisoners",0))
	assert(int(MilitaryCampaign.home_army.troops)==int(retreat_result.attacker.remaining_troops)-retreat_prisoners)
	assert(String(retreat_result.get("message",""))==String(retreat_result.termination.summary))
	assert(MilitaryCampaign.engagement_snapshot().is_empty())
	assert(MilitaryCampaign.pending_aftermath.is_empty())
	assert(MilitaryCampaign.validate_state().is_empty())
	var poor_withdrawal:Dictionary=MilitaryCampaign.simulator.create_formation_force("Poor withdrawal",[{"unit":"levy","weapon":"improvised","count":200,"equipment":200}],0.30,0.70)
	poor_withdrawal["commander"]=MilitaryCampaign.simulator.create_commander("Poor captain",0.0,0.0,0.0,0.0)
	var expert_withdrawal:Dictionary=MilitaryCampaign.simulator.create_formation_force("Expert withdrawal",[{"unit":"levy","weapon":"improvised","count":200,"equipment":200}],1.0,0.70)
	expert_withdrawal["commander"]=MilitaryCampaign.simulator.create_commander("Expert captain",1.0,1.0,1.0,1.0)
	var pursuers:Dictionary=MilitaryCampaign.simulator.create_formation_force("Pursuers",[{"unit":"cavalry","weapon":"lance","count":80,"equipment":80},{"unit":"line_infantry","weapon":"spear","count":120,"equipment":120}],0.85,0.85)
	pursuers["commander"]=MilitaryCampaign.simulator.create_commander("Pursuer",0.80,0.80,0.60,0.70)
	var poor_retreat:Dictionary=MilitaryCampaign._retreat_termination(poor_withdrawal,pursuers,881,2)
	var expert_retreat:Dictionary=MilitaryCampaign._retreat_termination(expert_withdrawal,pursuers,881,2)
	assert(float(poor_retreat.pursuit_pressure)>float(expert_retreat.pursuit_pressure))
	assert(int(poor_retreat.prisoners)>=int(expert_retreat.prisoners))
