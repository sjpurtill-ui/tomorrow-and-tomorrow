...
func test_whole_game_save_preserves_paid_plants_and_next_day_consumption()->void:
	CivilizationSystem.reset_for_new_world()
	running_cold_store()
	var slot := "technology_integration_%d" % OS.get_process_id()
	...
	var expected_operations := Ops.data().duplicate(true)
	...
	assert_dict(Ops.data()).is_equal(expected_operations)

func test_legacy_save_without_new_fields_does_not_inherit_installed_plants()->void:
	CivilizationSystem.reset_for_new_world()
	running_cold_store()
	var slot := "technology_legacy_%d" % OS.get_process_id()
	assert_bool(SaveSystem.save_game(slot).get("ok",false)).is_true()
	var payload := SaveSystem._read_payload(slot)
	for field:String in ["technology_operations","cultivation_nutrients","research_notification_mode"]:
		payload.reflected_GameState.erase(field)
