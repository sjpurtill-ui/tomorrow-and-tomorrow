...
func test_empirical_model_targets_remain_reachable_without_new_mathematics()->void:
	var excluded:Array=[]
	...
	# The overlay promises an empirical route for each modeled subject. Later
	# inventions can legitimately require mathematics as a physical foundation.
	for subject:String in Mathematics.MODELS:
		assert_bool(subject in known).override_failure_message(subject).is_true()
