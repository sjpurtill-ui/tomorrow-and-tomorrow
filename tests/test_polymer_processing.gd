...
func test_controlled_peg_requires_measured_starter_and_cannot_self_certify_size()->void:
	WorldSimulation.scoped("polymers",func()->void:
		prepare();var s=WorldSimulation.state
		...
		var items:Array[String]=["metered_peg_starter","controlled_chain_peg"]
		var targets:Array[int]=[1,3]
		for n:int in items.size():assert_int(int(run_batch(items[n],targets[n]).get("completed",0))).override_failure_message(items[n]).is_equal(targets[n])
		assert_float(float(s.resource_stockpiles["Metered PEG Starter"])).is_equal_approx(.88,.000001)
		assert_float(float(s.resource_stockpiles["Controlled-Chain PEG"])).is_equal(3.0)
		assert_float(float(s.resource_stockpiles.get("Size-Qualified PEG",0))).is_equal(0.0)
		assert_bool(WorldSimulation.military.start_production_line("characterized_controlled_peg",1).get("ok",false)).is_false()
