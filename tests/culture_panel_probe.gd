...
func _ready()->void:
	...
	for canvas in [Vector2i(1600,1000),Vector2i(1024,640)]:
		...
		for sub in [0,1]:
			hud.open_dock("civ",sub)
			for i in 8:await get_tree().process_frame
			check(hud.dock.get_global_rect().end.x<=canvas.x,"Culture fits width")
			if sub==0:
	...
	for i in 3:await get_tree().process_frame
	var panel=hud.dock.find_child("CulturePanel",true,false)
	panel.data.on_council.call()
	await get_tree().process_frame
	check(hud.dock.sub==1,"Speak action opens Council")
	print("CULTURE_PRESENTATION_CHECKS: ",failures)
