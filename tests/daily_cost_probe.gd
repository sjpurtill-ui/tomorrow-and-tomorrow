...
## Never reads or writes ordinary player userdata. See DAILY_COST_HANDOFF.md.
const DAYS := 24
var failures: Array[String] = []

func _ready() -> void:
	if DisplayServer.get_name() != "headless" or not OS.get_user_data_dir().ends_with("TomorrowDailyCostTests"):
		push_error("Daily cost replay requires headless mode and private TomorrowDailyCostTests userdata.")
	...
	if not WorldSimulation.enabled or WorldSimulation.actors.size()!=12:
		push_error("This replay requires a shared-rule campaign with twelve opponents.");get_tree().quit(2);return
	...
	print("DAILY_COST ",JSON.stringify({"mode":mode,"start_day":start_day,"days":DAYS,"opponents":WorldSimulation.actors.size(),"mean_ms":total/DAYS,"median_ms":sorted[DAYS/2],"p95_ms":sorted[ceili(DAYS*.95)-1],"max_ms":sorted[-1],"samples_ms":elapsed,"full_state_matches":mode=="optimized" and failures.is_empty(),"failures":failures}))
