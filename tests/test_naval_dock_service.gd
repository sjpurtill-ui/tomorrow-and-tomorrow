...
func test_paid_project_does_not_install_from_partial_inventory()->void:
	var goods:=float(Dock.BILL[GOODS])
	GameState.resource_stockpiles[GOODS]=goods-.5
	...
	GameState.resource_stockpiles[GOODS]=goods
	assert_bool(op.build_dock(int(port.id)).has("ok")).is_true()
	assert_float(float(GameState.resource_stockpiles[GOODS])).is_equal_approx(0.0,.000001)
...
func test_secondary_port_cannot_spend_primary_dock_supplies()->void:
	...
	assert_float(float(city.local_resources.resource_stockpiles[GOODS])).is_equal_approx(0.0,.000001)
...
func test_actual_install_button_pays_for_selected_port()->void:
	...
	assert_float(float(GameState.resource_stockpiles[GOODS])).is_equal_approx(1000.0-float(Dock.BILL[GOODS]),.000001)
...
func test_owned_actor_installs_imported_dock_and_full_save_retains_partial_work()->void:
	...
	WorldSimulation.scoped("dock_owner",func()->void:
		...
		assert_float(float(state.resource_stockpiles[GOODS])).is_equal_approx(1000.0-float(Dock.BILL[GOODS]),.000001)
	...
	WorldSimulation.scoped("dock_owner",func()->void:
		...
		assert_float(float(WorldSimulation.state.resource_stockpiles[GOODS])).is_equal_approx(1000.0-float(Dock.BILL[GOODS]),.000001))
	WorldSimulation.clear()
	GameState.set_process(true);CivilizationSystem.set_process(true);MilitaryCampaign.set_process(true)

func test_secondary_dock_waits_for_real_goods_delivery()->void:
	ship()
	GameState.player_settlements.append({"id":"docktown","name":"Docktown","primary":false,"position":Vector2(10,0),"population_share":.25,"founded_day":0})
	port.city_id="docktown"
	...
	SettlementModel.with_city_resources("docktown",func()->void:
		for item:String in Dock.BILL:GameState.resource_stockpiles[item]=float(Dock.BILL[item])
		GameState.resource_stockpiles[GOODS]=0.0)
	var planner=preload("res://scripts/naval_dock_investment.gd")
	assert_dict(planner.recommendation()).is_empty();assert_bool(port.has("dock_service")).is_false()
	SettlementModel.process_city_trade()
	var shipped:=0.0
	for shipment:Dictionary in GameState.city_trade_shipments:
		if shipment.resource==GOODS:shipped+=float(shipment.quantity)
	assert_float(shipped).is_greater_equal(goods)
	assert_float(float(GameState.resource_stockpiles[GOODS])).is_equal_approx(goods+1000.0-shipped,.0001)
	...
	assert_bool(port.has("dock_service")).is_true()
	var city:Dictionary=SettlementModel.settlement_record("docktown")
	assert_float(float(city.local_resources.resource_stockpiles[GOODS])).is_equal_approx(shipped-goods,.0001)
