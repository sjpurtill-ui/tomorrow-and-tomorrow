...
func _ready()->void:
	...
	_expect(terrain.close_army_figures.size()>0 and terrain.close_army_figures.size()<=4,"close army figures missing or over budget")
	for figures:Node3D in terrain.close_army_figures.values():
		_expect(figures.figure_count<=256,"close formation exceeded its representative budget")
		_expect(figures.scale.is_equal_approx(Vector3.ONE*terrain.SETTLEMENT_DETAIL_SCALE),"army figures do not match city detail scale")
		_expect(figures.get_node_or_null("Strength")!=null,"formation rebuild discarded strength label")
		_expect(figures.find_children("*","Skeleton3D",true,false).is_empty(),"crowd created per-soldier skeletons")
	terrain.camera.size=320.0
	terrain._refresh_player_field_army_markers()
	_expect(terrain.close_army_figures.is_empty(),"regional view retained close figure instances")
