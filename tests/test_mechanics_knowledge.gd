...
func test_empirical_model_targets_remain_reachable_without_mechanics_branch()->void:
	var excluded:Array=[]
	for entry:Dictionary in M.entries():excluded.append(entry.id)
	var reachable:=closure(excluded)
	# Later authored machinery can require mechanics; the empirical approaches
	# augmented by this overlay must remain independently reachable.
	for subject:String in M.MODELS:
		assert_bool(subject in reachable).override_failure_message(subject).is_true()
