...
func test_cistern_uses_one_real_lining_and_rain_adds_finite_unsafe_water()->void:
	...
	assert_float(float(GameState.resource_stockpiles["Civilian Goods"])).is_equal(2.0)
...
func test_full_save_restores_a_paid_unfinished_cistern()->void:
	...
	WorldSimulation.scoped("water_works",func()->void:
		...
		state.resource_stockpiles={"Stone":40.0,"Civilian Goods":4.0,"Freshwater":0.0}
	...
	WorldSimulation.scoped("water_works",func()->void:WorldSimulation.state.water_waste_works=State.empty();WorldSimulation.state.resource_stockpiles["Civilian Goods"]=99.0)
	...
	WorldSimulation.scoped("water_works",func()->void:
		...
		assert_float(float(WorldSimulation.state.resource_stockpiles["Civilian Goods"])).is_equal(2.0))
