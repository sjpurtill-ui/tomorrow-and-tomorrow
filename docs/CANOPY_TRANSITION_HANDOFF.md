...
HELD: the reconciled rendering correction passes all **27 headless cases**, zero errors/failures/skips/orphans, across canopy transition, landscape cover, seasonal landscape, camera distances and surface precision. The six canopy cases also pass alone. Both updated native probe scripts compile successfully in the headless project, with its autoloads available, through `tests/canopy_compile_check.tscn`; that check does not render or run the native probes. Final evidence is retained in ignored `artifacts/canopy-transition/reconciled-source-tests.log` and `project-compile-check.log`. Initial fixture runs exposed the incorrectly typed marker and the camera-input cooldown; those fixtures were corrected before the final run. Direct `--check-only --script` attempts lacked GameState and logged compile errors despite process exit 0; those are not successful checks. The project-scene compile check supersedes them and verifies both scripts with explicit PASS lines and no errors.
