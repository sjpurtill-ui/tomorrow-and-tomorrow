...
## Purchased research, imported evidence and scholar visits

Added three gdUnit cases that iterate all ten isolated normalized candidates. Purchased research pays and travels, requires returned evidence to be studied locally, retains a 2.5 pathway multiplier and grants neither mastery nor equipment. Imported evidence preserves every ALL and each OR alternative. Scholar visits require foundations, spend payment and food, temporarily remove a provider knowledge worker, affect only the contracted subject during the visit and restore the worker without changing population or granting mastery/components.

Candidates are injected only into the test's in-memory normalized catalog; no production registration was edited. Initial test setup called a non-static normalizer incorrectly, producing a parse failure/no-case run; corrected to instantiate the existing society model. Final gdUnit run: three cases, zero errors/failures/skips/orphans, 13.167s, exit 0; /tmp/tt-fabric-acquisition.log and reports/report_1/results.xml. This is three cases containing ten-method loops, not thirty separately reported tests. Partnerships, licensing/reverse engineering where applicable, combined regression and rendered review remain pending. HELD.

## Partnerships and owned specimen recovery

Partnership case iterates all ten candidates with normalized definitions in both in-memory test owners. One-sided study cannot produce findings; returning too early is refused, war blocks agreement, both parties study, another paid return and local study produce 1.6 evidence, and neither party gains mastery or equipment. One gdUnit case passes, zero errors/failures/skips/orphans, 6.363s, /tmp/tt-fabric-partnerships.log.

Owned-specimen case iterates all ten component recipes: missing foundations reject examination, one of two owned batches is consumed, duplicate assignment is refused, no-worker study does not advance, local work produces weaker 1.35 evidence without granting mastery or production. One case passes, zero errors/failures/skips/orphans, 6.246s, /tmp/tt-fabric-specimens.log. These use isolated normalized candidates, not registered runtime additions. Existing specimen mechanics describe weak manufacturing evidence, not structural qualification of a building.

Combined regression, licensing applicability, daily all-city production coverage, deferred site-input audit and actual rendered review remain pending. HELD.

## Recovered regression and daily production verification

The earlier combined run executed no tests because the import cache lacked audio resources. A second import also failed during threaded font import. A temporary worktree-only override of `editor/import/use_multiple_threads=false` allowed import to finish with exit 0 and no ERROR/SCRIPT ERROR; the override was removed by the wrapper afterward. No project settings change is included. Setting reference: https://docs.godotengine.org/en/4.7/classes/class_projectsettings.html#class-projectsettings-property-editor-import-use-multiple-threads

The recovered combined gdUnit run passed all 58 cases across building material operations (11), earthen buildings (9), secondary city design (10), architecture kit (4), organic town visual (9), and early settlement visual (15). Zero failures, errors, skips or flaky cases. Log `/tmp/tt-fabric-regression-recovered.log`; XML `reports/report_4/results.xml`; 15.119s reported execution. These remain behavior/mesh checks, not rendered appearance review.

`audit_settlement_fabric_supply.gd` now includes all ten deferred on-site trial costs in the private dependency graph. It reuses component recipe inspection-input fields only in the audit copy; no fake recipe or discovery is registered. Repair consumes the same manufactured components. Pass: 878 candidate definitions (868 integrated + 10 local), 741 recipes including 5 analyses, 27 facilities, 9 closure rounds, zero errors or blocked products/plants. Log `/tmp/tt-fabric-supply-final.log`. Availability, successful quality and knowledge remain assumptions.

`check_fabric_owner.gd` now runs the actual daily equipment-production allocator instead of injecting three units of workshop work. It verifies zero crafting labor produces no progress/output, then assigned craftspeople and logistics produce one paid shade component, followed by monthly selection, assembly, paid trial and installation. The original 30-day fixture deadline was insufficient at the real workforce rate; the bounded fixture allows up to 360 daily allocations. The successful test logs no assertion/script errors and exits 0. A completion marker now prevents an assertion inside the scoped callback from printing a false PASS. Log `/tmp/tt-fabric-owner-daily.log`. This proves daily allocated workshop work in one scoped actor, not automatic order selection or secondary-city manufacturing.

Still HELD: actual rendered appearance review, automatic production coverage across player/secondary cities, licensing applicability, and the documented generic tooling choice require resolution. No runtime registration, ledger acceptance or tech illustration change occurred here.

## Construction demand reaches secondary cities

Extracted `BuildingMaterialInvestment.fabric_targets()` so the same bounded demand drives production and intercity distribution. It accounts for installed-component repair, pending paid trials, and one compatible next retrofit. `fabric_recommendation()` aggregates outstanding secondary-city demand after subtracting destination stores and incoming shipments; occupied cities are excluded. `SettlementModel.process_city_trade()` now includes those goods in its existing city targets, preserving carrier capacity, route knowledge, source reserves, travel delay, shipment limits and actual stock debits. No separate transport or population authority was added.

Fourteen gdUnit cases pass: new construction delivery scenario plus thirteen existing city resource cases, zero errors/failures/skips (0.541s). The new case verifies a two-batch home-plus-secondary production target, source conservation, no destination stock before arrival, no duplicate manufacture/shipment request while in transit, arrival and secondary-local spending without modifying capital stocks. Log `/tmp/tt-fabric-delivery.log`, report 5. Actual daily workshop-to-installation scenario rerun passes without script errors: `/tmp/tt-fabric-owner-delivery.log`.

Player production retains ordinary explicit workshop orders; rival controllers consume these production recommendations. Both use the existing production and delivery systems. This change does not add autonomous player orders or manufacture independently inside every secondary city. Remote cities receive actual goods through the already modeled transport network. No save schema changes: targets are recomputed, shipments use the existing record format. Shared-file conflict surface: `settlement_model.gd` trade target aggregation and `building_material_investment.gd` planning.

## Licensed manufacture applicability

All ten methods are supported by the existing paid production-license system. Added an acquisition case iterating every candidate: manufacture denied before contract; Stone payment debited; authorization unavailable until envoys return; a full nominal work interval produces only 65% progress; supplier withdrawal prevents further spending; renewed support permits one physical output without mastery or adoption. Local building installation still requires the existing local foundations and adoption checks; a manufacturing license grants no inspection or installation approval. All four acquisition cases pass (18.694s, zero errors/failures/skips/orphans), `/tmp/tt-fabric-license.log`, report 7. Earlier failed fixture provisioned only 20 Stone, overwriting the diplomatic reserve; corrected before the successful run. No license runtime change was necessary.

Remaining batch hold: actual rendered appearance review and generic component-tooling refinement. Registration, ledger acceptance and illustration production remain integrator-owned; the ten candidates are still unregistered.

## Wall-block visual correction and CPU mesh review

Found and corrected a visible placement defect: the full-width façade overlay spanned open courtyards and stood in front of recessed villa/hall wings. Installed details now attach separately to each actual `_block` with that block's dimensions and height. Courtyard openings remain open; lowered wings and villa walls receive appropriately placed details. Diagonal braces are continuous rotated beams rather than stepped cubes. Early overlays retain their existing bounds-based mapping; this change does not establish their visual quality.

Added `export_fabric_meshes.gd` to export actual mesh triangles/colors and `render_fabric_mesh_review.py` to rasterize depth-buffered CPU projections. Reviewed before/after terrace, courtyard, villa and hall with all detail flags enabled, two storeys. Committed image: `evidence/settlement-fabric-wall-block-review.png`. Baseline source was the architecture kit at `1fa0afc`; current source provides the after image. Export takes output JSON path and optional alternate kit script path after `--`; Python takes before JSON, after JSON, output PNG. Temporary baseline script removed. Initial exporter failed because unindexed meshes return null indices; corrected to synthesize sequential indices, then both exports completed cleanly. These projections verify geometry, not Godot lighting, shadows, materials, full city composition or paper-and-gouache tech imagery.

Nineteen existing architecture/early-visual gdUnit cases pass, zero errors/failures/skips (report 8, `/tmp/tt-fabric-facade-tests.log`). The ten-method paid job/serialization/mesh-bound probe passes with no script errors (`/tmp/tt-fabric-facade-jobs.log`). Added a semantic courtyard regression requiring the entrance volume to remain free of façade vertices; all five architecture cases then passed, 0.109s, zero errors/failures/skips (report 9, `/tmp/tt-fabric-courtyard-tests.log`). No save schema changes. Shared integration surface: `settlement_architecture_kit.gd` block construction and detail mesh cache.

Still HELD: early organic/round-building overlay appearance, in-game rendering review, and generic tooling refinement. No discoveries registered, no ledger or illustration changes. Full 5,000-discovery objective remains incomplete.

## Bounded worker acceptance finished

Replaced timber-only setup with explicit constituent costs: wood fitting/batten lines use Timber .5, Stone .5, Fiber Plants .1; runoff forming uses Timber .5 and Stone .5; wind frames use Timber 1, Stone .5, Fiber Plants .5; capillary course preparation uses Timber .25 and Stone .75; copper flashing uses Timber .5 and Stone 1; shade lattices use Timber .5, Stone .25, Fiber Plants .2. These are manual work surfaces, cutting/smoothing stones and binding fixtures represented by the existing paid setup ledger, not separate manufactured-tool discoveries or calibrated historical quantities. The production probe now checks exact setup debits for every recipe and cannot report PASS after its callback aborts. Canonical pre-batch saves have no such unregistered lines; existing unrelated lines are unchanged. Earlier experimental worker lines retain their paid setup records and should be retooled if reused for this revised fixture.

Early detail generation now uses finite authored wall profiles instead of roof envelopes, conforms round-house front details to the wall arc, lowers shade lattices below the wall top, and leaves central entrances free of screen battens/posts. Placement reservation and renderer use the same early-detail factory. The original imported base meshes, LOD and shadow data are retained. CPU projections of actual generated round-house, earthen-house, covered-workshop and compact-town meshes were inspected at matched close-up scales; result `evidence/settlement-fabric-early-review.png`. The image is a geometry review, not an in-game lighting capture. Added all-14-asset/eight-feature checks for finite vertices, normals/colors, bounded envelopes and mesh caching. Integrator explicitly confirmed headless-only acceptance; interactive GPU capture is not required for this delivery. GPU appearance remains an integration limitation.

Final combined gdUnit run: 66 cases across ten suites, zero errors/failures/flaky/skips/orphans, 47.922s, `/tmp/tt-fabric-final-gdunit.log`, report 10. Added envelope case then passed with all six architecture cases, .314s, `/tmp/tt-fabric-final-envelope.log`, report 11. Seven standalone probes all exited 0 with logs explicitly checked for SCRIPT ERROR/ERROR: `check_fabric_production`, `check_fabric_owner`, `check_fabric_jobs`, `check_fabric_save`, `check_fabric_inspection`, `check_fabric_response`, `audit_settlement_fabric_supply`; logs `/tmp/tt-final-<script>.log`. Supply remains 878 isolated candidate definitions, 741 recipes including five analyses, 27 facilities; no errors/blocked sources, all raw access/knowledge/quality assumptions explicit.

Worker implementation is READY for bounded integration review; authoritative acceptance and registration belong to the integrator. See SETTLEMENT_FABRIC_READY_HANDOFF.md for final-state scope and limitations. This supersedes earlier HELD entries; it does not complete the full technology overhaul or claim any operating-count increase.
