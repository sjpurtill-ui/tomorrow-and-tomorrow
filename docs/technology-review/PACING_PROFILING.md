...
The then-current source e1278e0 run used session 52497, 476 discoveries and schema 10, with the same seed/scenario, a 91,250-day target and a four-hour limit. It included the cache correction and newer physical-production evidence, plus intervening gameplay changes, so it was not a controlled speed comparison. That run is now terminal: the complete 250-year result and source evidence are reconciled below. [Launch and terminal record](pacing/catalog-476-250-run.json).
