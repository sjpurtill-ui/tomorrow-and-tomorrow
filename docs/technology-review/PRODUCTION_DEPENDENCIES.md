...
Numeric validation rejects strings, booleans, nulls, containers, non-finite values and disallowed signs in manufacturing work/power, resource quantities and operating power/service amounts. On any validation error, `closure_performed` is false, rounds and verified reachable counts are zero, and the error list carries the cause. Empty blocked lists in that result mean dependency traversal was not run; they do not mean success. Valid input reports `closure_performed: true`. This remains a typed-catalog dependency audit, not a complete arbitrary-document schema validator.
