...
### Declared continuous hot hold

The selected normalizing program explicitly marks its hold stage. The thermal runner retains current and longest continuous work within that stage's selected800–950 temperature band, separate from cumulative hot work. Heating before the hold does not count; leaving the band or stage resets the current streak. Idle cooling below the band resets it as well. Normalizing acceptance now requires the full1.0 declared hold within floating-point tolerance. Validation bounds the hold record by its stage duration and accumulated hot work.

Eight thermal/workshop tests passed, zero errors/failures/skips/orphans,1.279s; /tmp/tt-declared-thermal-hold.log. The regression demonstrates >1 hot work during preheating with zero qualifying hold, then .5 and1.0 hold progression, rejection of an impossible2.0 hold record, and an interrupted cold specimen with zero qualifying hold despite prior hot work. Existing paid section/shaft-consumer behavior remains passing. No full campaign or full-save sweep repeated.

This replaces the cumulative-hot-work qualification limitation recorded in the previous entry for this selected normalizing process. It does not establish generalized alloy kinetics, calibrated physical time or tamper-proof thermal replay. Old isolated thermal runs without hold_streak/longest_hold no longer validate; all callers creating runs use Thermal.start. No canonical save schema changes. Remaining batch gates and all twelve HELD status are unchanged. Shared conflict is the declared hold flag in civilian_industry.gd; the thermal runner is shared by metallurgy's casting/phase-trial adapters and should be covered in the eventual combined batch verification.

### Seventeen apparatus recipes fabricated through daily production

Integrator task status was inspected read-only and is currently idle; no worker handoff was treated as integration. Canonical documentation remains856 integrated discoveries. Continued the existing metallurgy ownership in /Users/seanpurtill/Documents/Codex/tt-metallurgy-processes, codex/metallurgy-processes, base59f146e5 with verified856 merged.

Added one test with17 separate apparatus-fabrication scenarios: vacuum chamber, fracture frame/reference station, slitting set/calibrated station, induction station, indentation set, foam molding set, casting ventilation, phase-trial apparatus, surface comparator, honing/superfinishing heads, treatment furnace, reflected microscope, section preparation and vacuum pump. Each commissions a real steam generator and advances normal production days without injecting electricity. Output remains zero until completion; exactly one apparatus is produced; job consumption totals match recipe inputs, nonservice stock debits match, coal decreases and saved queue validation passes. One test/17scenarios passed, zero errors/failures/skips/orphans in4.769s, /tmp/tt-metallurgy-capital.log.

This replaces the missing apparatus-fabrication evidence from supplied components. It does not prove calibration, upstream construction of every input/tool, geographical availability, or player research pacing. Lead-tin phase charts are analytical outputs, already covered by retained survey tests, and were not counted as apparatus here. Do not repeat this unchanged fabrication sweep as a substitute for resolving the remaining physics/qualification, acquisition/recovery, casting/phase calendar, promotion and artwork gaps. All twelve remain HELD; no runtime or save shape changed in this verification commit.

### Casting cooling and idle calendar state

Investment and lost-foam recipes now contain an explicit2-work passive cooling stage between pour and inspection. The pour temperature carries into cooling and then inspection rather than being reset to ambient by a newly initialized stage. Inspection observes temperature and accepts only at150 or below alongside the existing fill/profile/section checks. Recipe durations increase by2; total electricity is unchanged and average daily demand is adjusted. The cooling coefficient is a declared normalized game parameter, not a calibrated solidification simulation.

Casting pending records now retain last_day/idle_days. Idle days cool the current stage without adding work, evaporated-water credit or spending supplies; repeated calls on the same day do not repeat cooling. Normal persistent-production early returns and direct adapter advancement both synchronize. Four tests pass with zero errors/failures/skips/orphans in1.129s, /tmp/tt-casting-cooling-final.log. New scenarios for both methods verify >1000 pour temperature survives entry into cooling, no early product, paused cooling below150 with unchanged progress/stocks, serialized resumption, and accepted cool castings. Existing route distinction, rejected worn molds, material balance and downstream bearing consumer checks remain passing.

HELD. Older isolated casting records lack the calendar fields and cooling stage and are not compatible; no canonical save schema changed. This does not solve latent heat, interrupted in-mold fill or full thermal-history replay; stage heat before the pour remains selected separate operations. Phase-survey calendar state, qualification/calibration, remaining research recovery routes, promotion and imagery remain open. Shared conflicts: two civilian_industry.gd recipe changes and one persistent_production.gd synchronization hook. No full-save or campaign sweep repeated.

### Phase-survey cooling and combined thermal recovery

The retained phase-survey sample now cools on missed days, with last_day/idle_days validated and serialized. No additional heat work, sample observations or material charges accrue while paused. Resumption continues from the cooled sample; observations retain the actual reached temperature and mobility, not the programmed target. Earlier sample observations remain immutable.

Three phase-survey tests pass, zero errors/failures/skips/orphans in.858s, /tmp/tt-alloy-idle.log. The new case pauses the eutectic185 target sample at1.99/2work, cools100days below25, preserves paid energy/progress, verifies same-day idempotence and serialized recovery, and observes an immobile sample below40 after its remaining.01work. Previous completed samples remain exact and the survey completes its remaining samples. This does not claim a failed target-temperature trial is equivalent to a successfully equilibrated point.

Because the recent hold/casting/calendar changes altered shared retained thermal state, the actual daily-production suite (11scenarios) and full SaveSystem recovery suite (13scenarios) were run together once after these changes. Both pass, zero errors/failures/skips/orphans in14.939s; /tmp/tt-metallurgy-thermal-combined.log. No new service injection was added to daily-production fixtures. The full-save test retains its existing supplied-service fixture. No player launch, canonical merge or registration.

All12 remain HELD. Old isolated alloy records without calendar fields do not validate; current records survive the full-save test. Remaining work is no longer basic calendar cooling or apparatus fabrication: focus on material/pattern and apparatus qualification, thermal-history integrity and selected process limitations, complete disadvantaged acquisition/recovery coverage, registration and integrator-owned artwork. The overall5000 target and multi-millennial progression remain far from complete. Shared conflict: one idle-sync call in persistent_production.gd.

### Final-record integrity and all-twelve scholar visits

Completed casting records must now have a final run exactly matching the last completed stage's thermal record. Completed alloy surveys must likewise match the last sample. This prevents contradictory saved final temperatures from being accepted even after their derived readings are rewritten. Seven casting/alloy tests pass, zero errors/failures/skips/orphans in1.893s, /tmp/tt-metallurgy-final-records.log, including both corruption cases. This is cross-record consistency, not complete physical replay.

Added twelve actual scholar-dispatch scenarios to the metallurgy acquisition suite. Missing foundations block the invitation; after foundations are supplied, dispatch spends goods and visitor provisions, accepted negotiation removes one Knowledge worker from the source, no teaching applies before delivery, and delivered teaching is1.5 for the requested subject only. It expires on leave day; the source worker returns on home day without changing population. Serialized visit records validate; no discovery mastery or equipment queue is granted. Three acquisition tests (12purchase scenarios, all branch mutations,12scholar scenarios) pass, zero errors/failures/skips/orphans in15.526s, /tmp/tt-metallurgy-scholars.log.

HELD. The existing systems own missions, population and research; no duplicate acquisition state or global modifiers were introduced. Valid current saves are unchanged; inconsistent completed thermal records are rejected. Remaining acquisition work includes partnership/recovery coverage. Remaining operating review includes material/pattern and apparatus qualification plus documented selected-model and thermal-replay limits; registration/artwork remain pending. All twelve remain unregistered and canonical count is unchanged. No player launch or shared military/discovery-system edits.

### Twelve bilateral research and interrupted-exchange scenarios

Added a metallurgy-specific partnership suite with the twelve unregistered definitions injected into both isolated test catalogs. Every scenario dispatches a paid proposal, completes only the local study, verifies that the remote study remains zero and no evidence appears, and exercises an actually refused premature exchange. After both sides complete study, war blocks the exchange quote. Peace permits a second paid delegation, but delivered results still grant no evidence until local validation completes. The resulting research-partnership evidence has the existing1.6 route multiplier; neither partner gains instant discovery mastery and no workshop queue appears.

One test with12 scenarios passes, zero errors/failures/skips/orphans in7.415s; /tmp/tt-metallurgy-partnerships.log. This removes the missing bilateral-study/exchange/validation coverage for these identities and demonstrates recovery from a refused exchange and wartime interruption without replacing earlier protocol progress. It does not verify every save/loss/exposure route or millennial pacing. The suite resets its test catalogs and uses existing mission, collection and labor ownership; no runtime or save schema changes.

All twelve remain HELD pending operating qualification/model review, final promotion and artwork. Canonical discovery count remains unchanged. No repeated full-save, daily production or capital sweep was run for this test-only change.

### Paid foam-pattern preparation and measured acceptance

Replaced automatic EPS pattern output with retained paid molding/conditioning work. New pattern_workshop owns one reserved charge under its production job, plus source identity, original mold wear, pause history, measured witness and disposition. foam_pattern_measurement models selected expansion under heat, cooling, and mold-wear dimensional error in normalized batch/cavity units. Its observer accepts measured dry mass, volume, form error and temperature only. Incomplete expansion or worn form fails; accepted patterns use the existing EPS Casting Patterns stock, rejected pieces use Rejected EPS Patterns. Spent water and expansion exhaust are recorded separately. Pattern mold wear persists on the job through retooling; new fixtures require existing paid production setup.

Added Pattern Measurement Sets (balance/volume/template fixtures) under existing dimensional_metrology, fabricated from steel/copper/timber/glassware with gauge blocks and machine tools. This is a physical apparatus recipe, not another discovery. Added its required tool to the existing pattern recipe. Persistent production handles reserved patterns, validation, pause cooling, cleanup and dispatch; technology_operations requests power after the last feed has been reserved.

Two focused tests pass in.575s: accepted/rejected worn molds, one-time input reservation, partial serialization, witness corruption rejection, actual lost-foam consumption of the produced accepted pattern, interruption-caused expansion failure and observer separation. /tmp/tt-foam-patterns.log. Extended daily generation (now12scenarios), full SaveSystem (14scenarios) and apparatus fabrication (18scenarios) suites pass together:3tests/44scenarios, zero errors/failures/skips/orphans in23.622s; /tmp/tt-pattern-production-save.log. These reruns cover the newly introduced retained process and apparatus.

HELD. Chemical burnout/residue remains unqualified; fixed selected mass/volume and expansion coefficients are normalized game behavior, not general EPS chemistry or calibrated instruments. Full instrument calibration and selected-source qualification remain open. Older isolated partial expanded_casting_patterns jobs without pattern_pending are rejected; completed physical stock is unchanged. No canonical schema, player session, discovery registration or artwork changed. Shared conflicts: civilian_industry.gd, persistent_production.gd and technology_operations.gd additive pattern hooks. All12 discoveries remain unregistered.

### Candidate graph/source reconciliation and review artifact

Extended the isolated supply audit to validate the full candidate graph with existing dormant-OR handling, exact authored names/AND/OR arrays from the three source catalogs, production/inspection bindings and inspection-gate references. It passes at856 existing+12unregistered=868candidate definitions,730recipes including5conditional analyses,27facilities,9closure rounds, zero errors/blocked products/blocked plants. /tmp/tt-metallurgy-candidate-audit.log. Structural quality and resource availability remain assumed; no discoveries were registered.

Added METALLURGY_BATCH_REVIEW.md with per-discovery implemented behavior, explicit remaining qualification/model limitations, verified evidence, shared-file/save concerns and the remaining full-project count gap. This is HELD review material, not a READY handoff or a redefinition of the overall objective. Next actions should close or resolve concrete operating-contract mismatches before final combined verification and integrator promotion.

### Burnout requires hot exposure as well as peak temperature

Investment-casting pattern removal now uses the lesser of reached-temperature response and retained hot-exposure response. A brief peak cannot completely remove the pattern. The selected small-pattern exposure parameter is0.2 normalized hot-work units; it is not a physical decomposition-rate claim. Material-balance reconstruction uses the same response, preserving saved pour/pattern consistency.

Five casting tests pass, zero errors/failures/skips/orphans in1.477s, /tmp/tt-casting-burnout-exposure.log. New regression reaches850 during burnout, verifies remaining pattern, pauses100days, serializes/resumes, and produces rejected casting with retained pre-pour pattern. Existing uninterrupted investment/lost-foam acceptance and bearing consumption remain passing. Review checklist updated to remove peak-only burnout as an unresolved defect while retaining chemical-residue and filling limitations.

HELD. No save fields changed; historical isolated records whose burnout quantities differ under the corrected response can be rejected by balance validation. No canonical data, imagery or registration changed. Remaining selected-material/instrument qualification and delivery steps remain as listed in METALLURGY_BATCH_REVIEW.md. No broad unchanged sweep repeated.

### Additional authored breadth: eight unreconciled treatment/testing proposals

Created proposals/metallurgy-treatment-expansion.json and the readable proposals/Metallurgy Treatment Expansion.md. Subjects: Jominy end-quench testing, Charpy impact testing, austempering, martempering, carbonitriding, controlled shot peening, electroslag remelting and vacuum arc remelting. Each has explicit common/alternative foundations, a distinct mechanism, paid physical operation, concrete downstream applications, failure limits and paid scholar/partnership/research/imported-service recovery terms. Primary technical sources are linked per entry; branching and game consequences are identified as design proposals rather than historical chronology.

Exact ID/name collision checks against array-backed drafts and dictionary-backed baseline/pending catalogs pass for all8; every referenced parent resolves. Manual review distinguishes them from existing generic hardening, nitriding, fracture-toughness testing, vacuum melting and residual-stress assessment. Further integrator semantic review is required. No shared ledger, runtime discovery, imagery or counts were changed. These are authored_proposal_unreconciled, not eight delivered operating discoveries. The twelve existing implementation candidates remain HELD. This advances the missing authored breadth while preserving separate implementation and integration accounting.

### Grain image scale derives from a separate reference observation

Added metallurgy_scale.gd to identify two resolved reference marks, measure their pixel separation and derive micrometres/pixel plus interval/localization uncertainty. Grain frames now require that reference and reject absent/ambiguous marks, excessive uncertainty or inconsistent claimed scale. The observer still receives no latent grain-size parameter. A new Microscope Scale Slides recipe supplies the required physical apparatus under dimensional_metrology; normalizing installs it alongside the reflected-light microscope and preparation tools. The existing paid section work now retains the reference observation with the grain image.

Seven grain/thermal-workshop tests pass, zero errors/failures/skips/orphans in1.242s, /tmp/tt-grain-scale-fixed.log. Added missing reference, extra mark, altered claimed scale and excessive reference-uncertainty cases. Existing normalizing section and shaft consumer remain passing. Initial helper name reference conflicted with RefCounted; renamed reference_image after a compile failure and engine crash. That initial run is not success evidence.

HELD. Reference slide spacing/tolerance is still a selected apparatus assumption; this does not prove its manufacture, absolute traceability, optical distortion or full microscope qualification. New apparatus fabrication and combined save checks are pending for the final batch sweep; no unchanged broad suites repeated here. Previously saved isolated observed grain sections without scale_reference no longer validate. No canonical state, counts or imagery changed. Shared change: civilian_industry.gd reference-slide recipe and normalizing tooling; measurement and section modules are worker-owned. Review checklist updated to distinguish observed scale derivation from remaining reference qualification.

### Reference-slide fabrication/use and integrator review dispatch

The actual normalizing workshop regression now fabricates its microscope reference slide from paid recipe inputs, checks zero early output and exact material debits, then installs that physical slide instead of seeding it. Its stock is consumed into tooling. The remaining test exercises blocked/adopted grain measurement, serialized sectioning and accepted steel consumed by the shaft recipe. Four workshop tests pass, zero errors/failures/skips/orphans in1.204s, /tmp/tt-reference-slide-flow.log. Reference spacing/tolerance remains a selected model assumption, not a claim of absolute metrological traceability.

Sent the current batch review and eight additional authored proposals to designated integrator task01a0978a-39f5-7620-a77f-5918ccb1d8ba, explicitly HELD and not permission for automatic integration. Requested concrete required operating changes versus accepted bounded game-model scope. A fresh wait_threads snapshot confirms its review turn is active (turn01a09c75-d7b2-7553-b691-2430e3a40b1d, revision3). No waiting status is treated as completion. Worker source code remained unchanged during that review; this commit adds only fabrication/use evidence and documentation. All12 remain unregistered, and the full objective remains active.


## Final bounded thermal integrity correction and READY verification

The integrator accepted the selected game-model scope and explicitly rejected indefinite calibration/material-assay expansion. Added conserved paid energy, carried initial heat, active/idle heat loss, exact stage coolant and hold-duration consistency to thermal records. Casting preserves explicit carried heat; metallurgy and alloy samples require ambient initial heat. Idle cooling spends no work and grants no hold. New regressions reject zero-energy hot treatment, underpaid coolant and contradictory holds, and retain valid fractional/reloaded/interrupted and passive-cooling records.

All twelve production contracts now describe actual selected implementations. Original editorial operating contracts, authored IDs/names/AND/OR remain intact. No candidate registered; no eight-proposal ledger promotion.

Final Godot 4.7.2 headless evidence, explicit worker path:
- `/tmp/tt-thermal-integrity.log`: 19/19 cases, 4/4 suites, 0 errors/failures/flaky/skipped/orphans; 3.438 seconds. Thermal cycle, metallurgy workshop, casting workshop, alloy phase trials.
- `/tmp/tt-metallurgy-final-sweep.log`: 92/92 cases, 20/20 suites, 0 errors/failures/flaky/skipped/orphans; 64.790 seconds. Acquisition, partnerships, daily power, full save, capital, persistent production, technology operations, machine process parts/support/coordinates, formed-source cleanup, induction, interruptions, feedstocks, residual slitting, fracture, weld, vacuum, foam and grain measurement. Daily/full-save/capital contain 12/14/18 scenarios respectively, plus the reference-slide fabrication in the affected workshop suite.
- `/tmp/tt-metallurgy-final-source.log`: 868 candidate definitions (856 baseline + 12 unregistered), 731 reachable recipes including five conditional analyses, 27 reachable facilities, 9 closure rounds, no errors or blocked products/plants. Structural closure assumes knowledge/raw stocks/quality success; it does not prove full-campaign pacing.
- `git diff --check` passed. A direct comparison preserved every original editorial operating contract and all twelve IDs/names.

Frozen delivery is READY for the integrator's registration/art/canonical verification, not integrated and not completion of the full-history goal. New thermal save fields intentionally reject older isolated WIP thermal records; no migration is promised for those prototypes. No player/editor launch or restart. See METALLURGY_READY_HANDOFF.md.
