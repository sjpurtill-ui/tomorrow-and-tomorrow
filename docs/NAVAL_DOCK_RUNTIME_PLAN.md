...
UI verification correction: standalone `--check-only` returned exit 0 while reporting an unresolved MilitaryCampaign autoload, so it was not accepted as UI evidence. A regular in-project test now loads the naval panel and checks its dated inspection summary without changing condition. All nine dock cases pass in `/tmp/tt-naval-dock-ui-tests.log`, giving 14 distinct cases with the earlier five joint regressions. The install-button callback remains to verify.
