...
Year-71 fixture, eight days: saved state matches the pre-change baseline on the worker branch. On the merged commit the only differences are the new art fields (`early_art_index`, `appearance_civ_id`, `appearance_world_seed`) from `46f4cd3`. Real-frame top speed: 1.1–1.6 s day freezes became median ~19 ms / p95 28–34 ms frames; throughput about 0.77 → 0.63 days/s. Slowest warm step ~31–49 ms. Paused map refresh peaks 31–35 → 15–19 ms. Total daily CPU (~1.0–1.4 s) is essentially unchanged; three days/second is not achieved. Details: `docs/performance/YEAR71_AUDIT.md`.

Merged validation (worktree `tt-day-jobs` at `3220e11`): day-job, map sample, year-71, save continuation, settlement, scouting and early-art suites pass except two failures that also occur on `main` without this merge: `test_same_orders_and_daily_inputs_produce_identical_owned_state` (from `46f4cd3`: `person_snapshot` lazily writes `early_art_index` into saved people) and the order-dependent `test_nomadic_bands_move_are_finite_and_end_at_world_urbanization`. No player launch or restart; a running game keeps its loaded scripts until saved and restarted.
...
Scouting gains a rare-resource prospecting focus once the civilization knows a recognition method. Returning parties evaluate the actual traversed terrain and may register a surveyed gold, petroleum, metal, coal, graphite, phosphate, or uranium occurrence. The report never awards inventory; access, extraction, processing, and transport remain separate physical work. Existing sighting-only nomad records are deterministically upgraded on load, capped to eight active bands; older scouting policies and saves remain valid.

Validation: `test_scouting_staff.gd` passes 22/22; `test_society_exchange.gd` passes 32/32; `test_resource_recognition.gd` passes 9/9; `test_civilization_owned_simulation.gd` passes 19/19. The recruitment report presentation case passes. The wider visual-presentation suite retains one unrelated pre-existing test-harness error in `command_rail_hud.gd` because its synthetic HUD has no camera-distance provider; the other 38 cases pass. The current player processes predate this integration and must not be represented as containing it.

## September 14 — 878 verified operating discoveries

Integrated ten authored D08 settlement-fabric discoveries from frozen worker source `842f51d06d3fcd721ff43cfcacb03e975c5ed5ea` at runtime/art commit `e86b1cd41070fb2f9af05eeaf9f3c9ebdaaa5722`. Ten finite workshop recipes make fitted timber connections, braces, movement joints, runoff channels, wind-test frames, capillary courses, flashing, rainscreen panels and shade lattices. Components remain physical stock until an occupied compatible plot consumes them, receives builder labor, completes a separate paid trial and records an accepted, rejected or inconclusive result. Accepted work can reduce rain wear, alter heat-constrained household use, constrain loading and require supplied repair parts. Secondary cities receive real deliveries; purchase, scholar, partnership, specimen and licensing routes preserve their costs and disadvantages.

The integrated batch passes **110 focused GdUnit cases**, **13 research-atlas cases**, **15 graph/route cases** and seven standalone production/ownership/job/save/inspection/response/supply probes. Graph and structural supply audits pass at **878 discoveries / 660 learning routes / 736 workshop recipes plus five conditional analyses / 27 facilities**. All 868 prior runtime definitions remain exact. The committed runtime snapshot was promoted without adding identities: **878 integrated + 2,592 drafts = 3,470 identities**, leaving **1,530 to author and 4,122 to implement**. Art is **317 subject illustrations / 878 live, 561 queued**.

Plot records and unfinished jobs are additive save data; older saves without them retain prior behavior. GovernmentPeopleSystem remains the labor owner, and population stays aggregate with bounded representatives. The response models are selected, uncertain construction qualifications rather than universal engineering certification. Generic late structural upgrades and recurring housing capacity still need conversion from immediate/raw-resource increments into paid manufactured construction. Evidence: `/tmp/tt-int-fabric-gdunit.log`, `/tmp/tt-int-check_fabric_*.log`, `/tmp/tt-878-{runtime-snapshot.json,graph.log,supply.log,atlas.log}` and `technology-review/SETTLEMENT_FABRIC_READY_HANDOFF.md`. The standalone game launched on September 14 from the preceding canonical commit `5b2a07f`; that running process does not contain this batch until a later normal relaunch from updated main. The 5,000-discovery and full-history art objectives remain unfinished.

## September 13 — 43 further industrial illustration corrections

Integrated at canonical `abbc3e388f21b058bba07d56c31249a129081452`. Forty-three existing research illustrations now match their machinery with representative industrial architecture, clothing and infrastructure. This extends the earlier nine corrections across polymer laboratories, chemical plants, precision machine shops, textile mills, rail work and metal refining. Outdoor processes retain appropriate industrial yards or exposure racks. Exact prompts and retained native paths are recorded in `assets/ui/research/paper/INDUSTRIAL_ERA_EXPANDED_CORRECTIONS.md`; the general guide points to the superseding records.

All 43 actual discovery textures load at 768px with mipmaps on isolated and canonical checkouts. Both imports are clean and all 13 research-atlas tests pass on each, with zero errors, failures, flaky tests, skips or orphans. Native PNG hashes match the copied assets. Evidence: `/tmp/tt-expanded-era-{isolated,canonical}-{import,textures,atlas}.log`. This is a targeted correction of identified mismatches, not a completed historical audit of all catalog imagery.

Worktree `/Users/seanpurtill/.codex/worktrees/ef8f/tomorrow-and-tomorrow`, branch `codex/industrial-art-expanded`, base `ce96154a2959ee1d4ff0065dc823e876af4606e1`. Conflict-free fast-forward; no runtime, binding, save or ledger changes. Counts remain 868 operating discoveries and 307 subject illustrations, with 561 images queued. The ten-method paid settlement construction implementation remains in its worker checkout pending acceptance. No player launch or package rebuild. The 5,000-discovery objective remains unfinished.
...
## September 10 — standing scouting, recruitment clarity and year-31 stalls

INTEGRATED `ce7ed7a348ab21a71e06c5b5bd0d0b7f65ad2b16` by conflict-free fast-forward from canonical Mac `d95fa67964e0e2515875436107709562caa3d819`. Scouting is a standing population allocation with exploration or recruitment/influence focus; staff organize paid, physically traveled expeditions. Connected-ground routes favor unexplored paths and checked alternate returns. City reports and map labels share the original observed population range, with age and unverified projections clearly separate; time actually observing improves precision. Recruitment trains missing people in capacity-limited groups while preserving soldiers already at home. Shared equipment stocks and the immediate limiting resource are shown concisely. See `SCOUTING_RECRUITMENT_CLARITY_HANDOFF.md` for exact behavior and compatibility.

The year-31 investigation found route planning inside HUD refreshes and monthly regeneration of close vegetation while it was invisible at regional zoom. Interface refresh fell from roughly a second to 3–9 milliseconds in instrumented frames; the measured 2.84-second vegetation-related footprint rebuild is deferred until the detail can be seen, with a physical-clearance cache preventing upkeep-only replants. Shared troop views reuse one owner-position catalog and keep independent observer copies. No civilization, population, resource or research turn is skipped.

The final normal-rendering release replay of the actual day-11238 save with 12 opponents advances **85.029 days in 30.020 seconds (2.832 days/second)** at the 3-day setting, producing **246 frames**. Before the hidden-vegetation fix, the same release probe measured 1.828 days/second and 70 frames. Host load varied; these are local measurements, not a guaranteed improvement factor. Synchronous daily work still stalls frames, and neither steady 3 days/second nor smooth 60 FPS is claimed. The experimental separate rendering thread remains disabled. The isolated probe exited and its actual map capture was inspected.

All **158 combined canonical cases pass**, zero errors/failures/skips/orphans, across 13 suites (`/tmp/tt-scouting-canonical-tests.log`), including identical-order opponent parity, full world save/restore, RNG/save continuation, scouting, recruitment, map intelligence, joint services and landscape/secondary-city rendering. Worktree verification also passed 156 core cases, 36 final scouting/recruitment/performance cases and 23 landscape cases. The final native city/policy/recruitment layout and real click probe passes and exits (`/tmp/tt-city-render.log`).

Normal release **2026.09.10.2** is packaged with `tools/launch_game_macos.py --build-only`; its `build.ok` records the final integrated revision. Owned test overrides and profiling edits are removed. No player/editor was stopped or relaunched. Existing active scouts and queued legacy whole-class orders keep their existing completion rules; new recruitment uses rolling groups. Player scouting defaults Off until selected. The subject-art and canopy worktrees remain HELD; review the canopy change against the new close-vegetation guard before integration. Continue measured frame-time and visual improvements without redoing this delivery.
...
## September 14 — aerial terrain material and streaming pass

Integrated worker `71ce163` from `/Users/seanpurtill/Documents/Codex/tt-google-earth-landscape` as canonical `2c66a8c`, based on `749b7c8`. Dry terrain now selects a dedicated true-nadir semiarid soil/gravel albedo from authoritative precipitation instead of recoloring the lush temperate ground photograph into flat brown. Woodland density is resolved into irregular stand mosaics, close/regional texture tiers stop sampling once their footprint is unresolved, and dry channel treatment no longer produces the repeated dark strokes rejected during the intermediate capture. The new generated asset is `assets/terrain/semiarid_ground_albedo_v1.png`; it contains no settlements, roads, farms, coast, text or resource claims.
