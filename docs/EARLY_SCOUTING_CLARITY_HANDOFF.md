...
The research atlas and discovery popup suites cover 24 cases with zero errors or failures, including compact width, responsive column count, source-art identity, subject cropping, small-window bounds and discovery effects. Isolated graphical probes passed at 1440×900, 1200×900 and 800×600; the 1440×900 known-discovery capture shows three 220-pixel cards occupying only the space they need, with filled artwork in both cards and the inspector. No save data changes are involved. Integration conflicts for this follow-up are limited to `scripts/hud/research_atlas.gd`, `scripts/hud/research_visuals.gd` and their visual tests.

The Established grid is also explicitly vertically scrollable. Card input now passes wheel and trackpad gestures to its scroll container without sacrificing click selection; a 60-discovery catalogue test verifies a visible scrollbar and nonzero travel at 800×600. Established cards and their detail inspector show `YEAR n, DAY n`, resolved from the saved learning-route origin and falling back to the historical discovery event. Legacy or manually staged knowledge with no historical timestamp is labeled `DATE NOT RECORDED` rather than assigned a false date. This adds no save fields or migration. The atlas suite now covers 16 passing cases, including the long-catalogue and date assertions.
