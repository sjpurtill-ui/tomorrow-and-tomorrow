...
## Addendum 2 — Love and dread; the god's wrath and favour (Worker FL)

The ruler is a god to their own people. `scripts/divine_regard.gd` (static) reads each official's **love** (optional `relationships.sovereign.love`, derived from trust/respect/obligation/resentment until first moved) and **dread** (`fear`) into a two-gauge meter and a one-line read ("worships you", "obeys out of terror", "loves you and fears nothing", "quietly resents you", ...). A people-wide read blends officials with legitimacy/cohesion and recent wrath; a foreign people's reverence/dread comes from opinion, leader trust, border tension and remembered terror.

- `AudienceHall.divine_options(id)` / `divine(id, action, words)`: summoned officials — Terrify, Demand penance, Cast out (exile via `GovernmentPeopleSystem.person_departs(pid,"exiled")`), Strike down (`remove_central_officeholder`/`remove_settlement_leader` "execute"), Bless, Grant a boon (debits real stores), Raise up. Envoys — Terrify (intimidation through `ForeignDiplomacy.apply_conversation_reaction`; the engine picks the posture; that people's dread rises). Each act at most once per audience; bounded bond shifts for the target and every witness (graded by courage and pride) with memories. Nothing supernatural.
- Free text: `AudienceHall.divine_intent(id,text)` (offline keywords) and the live voice's bounded `divine` schema field (speak stage only) may trigger only the spoken acts (terrify, penance, bless, raise up); exile, execution and gifts need the explicit menu.
- Consequences: dread raises willingness and forecast optimism (overpromising, honesty tempers it) and makes officials deferential; love straightens forecasts and makes them plain-spoken. Dread ≥0.5 with resentment ≥0.3 first shows in speech ("warned", telegraphed flight hints), then bounded sabotage of execution (≤0.12), and after 20 days above a higher risk, possible flight (`person_departs(pid,"fled")`).
- State: optional `divine` block in the hall state (events ≤24, warned, civ_dread); older saves load without it.
- UI: meter + read on the speaker portrait; WRATH/FAVOUR menus (TERRIFY for envoys) beside SPEAK; love/dread icons and reads in the council's summon list with a people-wide row.

## Addendum 3 — The god's word is law: court commands (Worker DC)

Supersedes Addendum 2's rule that exile, execution and gifts need the explicit menu. Everything typed in the Court (summoned officials, envoys, and the court at rest once someone answers) is read by `scripts/court_commands.gd` as a speech act — question, statement, command, threat, blessing — offline by lexicon, and live through a bounded `command` object in the speak-stage schema (`{act, verb, actor_ref, target_ref, object, confidence}`). References resolve against the people present or known: names, first/last names, office titles ("the war leader", "the pathfinder"), pronouns and epithets ("him", "the traitor") by conversational salience (the last person the god dealt with, else the one standing before the god), "you"/"yourself", the guards.

- The ENGINE decides first. `obedience()` reads the actor's love and dread (divine_regard.gd): obey (the frightened at once), reluctant (the loving or gentle, for cruel acts), hesitate (a loving, unafraid, gentle hand ordered to kill pleads once; "I DEMAND IT" / "do it" / "obey" carries the pending order out), refuse (only dread ≤0.15 with courage ≥0.75 and pride or resentment high; ≈1% of killing orders across ordinary officials) — the refuser then flees (`person_departs "fled"`) or is seized and kneels bound, and the god may strike them down.
- Acts go through real systems: kill → `AudienceHall.divine(id,"strike_down",…,target_pid,{agent})` (same effects as WRATH Strike down on any official present, executioner gains dread and, if reluctant, resentment, with a memory); a summoned field general dies through `HistoricalFigures.record_death`; exile (`cast_out`), imprisonment (`person_departs(pid,"detained")`), penance/terrify/bless/raise/boon on anyone present, demote (office removal), appoint (`mark_central_appointment`), give (real stores; to an envoy's people via `_credit_civ`), take (from an envoy's people via the exchange, with relation cost), send (`CivilizationSystem.dispatch_scouts` / `dispatch_diplomat`), envoys killed/held/driven out (relation, trust, their people's dread, memory). Any other order: `CourtCommands.custom_order(text, context)` — civic pipeline for a settlement leader or a catalog policy, otherwise the universal custom-directive path (`custom_directive.gd` through `ConsequenceEngine.apply_directive`); `custom_directive_handler` may override.
- The voice only describes: `AudienceVoice.command_reaction(id,result)` (stage "command") is told WHAT ACTUALLY HAPPENED and the decided obedience; it writes one bracketed stage direction (`[...]`, 1–2 sentences, era weapons only: flint knife, stone axe, club, spear, cord before metal) plus the actor's answer and a witness aside; offline banks per act × weapon kind, on the same no-repeat ledger. The factual outcome follows as narration, and terminal acts show the receipt. A live classifier's order drops the model's provisional lines. Validators reject refusals when the engine decided obedience, meaningless stock proverbs, and over-long or quoted stage directions. The system prompt states the god's command is law.
- State: optional `pending_command` and `command_focus` on an audience; `divine` entries may be `action:person_id`. Older saves load unchanged.
