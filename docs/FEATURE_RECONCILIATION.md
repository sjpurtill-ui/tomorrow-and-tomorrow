## September 25 — All-era research rebalance and court stages integrated

Branch `codex/integrate-research` from main `08950f16`, with envoy-pace (`e22e71b5`) merged in. `codex/research-1200` (`72754faa`) brings all five research blocks 0–3000 with effects, era ceilings to 3000, modern survival, the fertility transition, literacy and mechanization, research pacing and parallel capacity, the research-cleanup recipes and renamed ids with save aliases, the WMD sovereign gate, the benchmarks and `tools/sim` to 3000. Research data, effects, society model, early-life conditions and ceilings come from it; game systems and UI come from main. Conflicts: `chief_scout.gd` keeps main's plain-speech closing line; `court_commands.gd` keeps main's `live_verb_allowed` and adds the sovereign decree; `test_era_words.gd` combines both suites. `resource_names.gd` and the felt-temperature words were already identical on both sides, so there is no duplicate. fun-pop's named-death accounting and founding pregnancies are kept, with the rebalance's food-labour floor and mortality rates on top. `test_early_life_conditions` now uses a neutral profile as the bare-table baseline, because fun-pop computes an empty profile on demand. One hill-country weather line in `opening_arc.gd` is reworded so the maxim detector passes.

`codex/civic-evolution` (`471dc42c`) adds 13 court stages driven by discoveries (throne or assembly path) with backdrops, roster attendants, stage titles, forms of address and the HighPriest, Justice and Treasurer offices. The stages choose the form of address, but plain speech's reverent god-address tables stay authoritative for wording. A stage form that the speaker's dialect already uses is preferred, and forms that are not reverent, are kennings or read as maxims are dropped (`CharacterVoice.reverent_address`). In `y1200_triggers.json`, `border_march_wardens` was not in the baked 1200–1800 block and is remapped to `frontier_march_lords`; the other ids resolve.

Quick checks: boot with 0 script or parse errors; the research 600/1200/1800/2400/3000/blocks suites pass; so do announcements, controller viability, foundations, materials, redistribution, tree navigation and visual atlas, early-life conditions, chronicle, one chronicle, era words, opening arc, civic stages, and plain speech. Several failures already exist on research-1200 or main and are unchanged: research licenses and partnerships, one supply-planner case, one government-people roster case (early art fields), and court_probe's F12 toggle and 1366/1280 fit. `tools/sim/check.py --rev 6a9abee0` is OK; plain `check.py` fails with stale truth on the poor scenarios, identically on research-1200 alone. Skipped: `early_consequences_probe` and `test_research_purchase` (too slow), long harnesses and GPU captures.
...
### Player-summoned court (main 065df093)

Fast-forwarded `codex/audience-variety` checkpoint 2. The council tab lists every official (and the Chief Scout) under "Summon to the hall" with a quiet matter count; the works screen has summon buttons for master builders. A summoned person raises their most pressing matter, can offer others, or asks what the ruler needs; typed orders route through the civic pipeline; Chief Scouts deliver pending reports and builders their decisions and pitches. Factual questions are answered with real figures; condescending address to the ruler is blocked. audience_summon, audience_hall, audience_modal, great_works_experience probes and the transcript gate (18/18 questions answered, 0 duplicates) pass.
...
### Responsive decrees and the interaction database (main)

Fast-forwarded `codex/responsive-decrees`. Civic leaders stay in character (lifelong voice, era, love/dread) and never talk about records or systems; asked about people they name persistent village notables. Any order the catalog does not cover becomes a bounded custom directive over 20 alterable parameters (fertility, infant mortality, disease, violence, cohesion, legitimacy, knowledge, labor, food, construction, water, ecology, migration, resentment…) scaled by compliance and administration, paying real costs, with seeded side effects reported later in character; impossible orders run as costly attempts. Receipts appear as a small line beneath the leader's speech in the Court. The local interaction database records every live exchange; an offline matcher and 78-type catalogue answer without the API; Menu > AI Connection adds Live/Hybrid/Offline and interaction export. responsive_decree, universal_order (41 orders), interaction_store, responsive_load_check, court, known_world, audience_* , fear_love and pronouncement probes pass; remaining civic suite failures fail identically on 56880c30.

### Early food, survival and growth consequences (main)

Fast-forwarded `codex/early-consequences`. Seven early protections (clean water, wound care, remedies, birth care, child care and weaning food, cooked food, lean-season stores) cut excess infant, child, adult, newborn and maternal deaths by adoption; thin diets and overwork raise deaths and lower conception; wild grounds renew and thin, extra food hands give diminishing returns, diet quality rewards variety, hard winters occur. Founding life expectancy ~39 with ~190 infant deaths per 1,000, rising to ~47–50 by year 100 with practice; stacked poor decisions shrink a settlement slowly without collapse. Older saves blend the rules in over two game years. early_consequences, responsive_decree, universal_order and court probes pass; test_early_life_conditions 11/11, test_campaign_save_continuation 5/5, test_civilization_system 69/69 alone. Known pre-existing issue: running test_campaign_save_continuation before test_civilization_system fails test_records_contact_and_inference_progressively_reveal_strategic_comparison on earlier main too (shared-state leak between suites).
