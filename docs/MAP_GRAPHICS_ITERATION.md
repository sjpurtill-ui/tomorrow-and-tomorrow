...
## 2026-09-05: Slice zoom-triggered close terrain sampling

Worker C:/Users/sjpur/tt-sliced-detail, codex/sliced-detail, base39ceb3c08c089c424ecd35eacdeb80957e286fbc. Indexed builder integrated before task. Coordinated scope: new close_terrain_job helper/tests, close LOD request/process advance/mesh installation, audit and record.

Zoom-triggered first close terrain construction now samples in2.5ms-budget main-thread batches, checking the clock every8 vertices. Regional ground remains visible until completion. Sampling pauses during camera motion; zooming beyond1.8km span or moving the convoy cancels stale work. New requests replace different targets. The existing synchronous founding/audit path remains available and cancels pending work. Installation is shared and replaces any old close patch. No worker thread touches game state.

Validation:18/18 close-job, close-grid and map-onboarding tests pass; sliced/uninterrupted job arrays match. Final editor import clean. Isolated GPU --sliced-detail-profile verifies zoom-out cancellation, moved-convoy cancellation, completion and the synchronous expanded-triangle fingerprint2888042706. First probe:90 frames, max sampling slice2709us, finalization51843us. Final probe with both cancellation cases:95frames, max sampling slice2753us, finalization60123us. Reviewed artifacts/sliced-final.png. Terrain attributes match; a comparison with the prior task's capture showed differences around vegetation, so no pixel-identical whole-scene claim. No script/shader errors; existing shutdown leaks persist.

Limitations:final mesh upload/material/vegetation installation still takes about52–60ms in these probes; founding remains synchronous. Close detail appears after sampling completes (roughly90–95 probe frames), and sustained navigation defers it. This removes the large single sampling block, not all zoom stalls or total CPU work. Save schema, resource state and terrain geometry unchanged. Next performance target is the remaining finalization/vegetation cost. Original Google Earth references remain the scale guide; no assets added or photorealism claim. Shared-file conflicts are the named LOD/process/install functions and audit. Integrator owns merge and player launch.


## 2026-09-05: Reuse the registered ground material

Worker C:/Users/sjpur/tt-shared-ground-material, codex/shared-ground-material, base555b395cba19affbc9b2c94f07caebcaf7f3965f. Sliced terrain integrated5cedfb9. Coordinated scope: _install_close_terrain_mesh material assignment only, audit checks and record.

Close terrain now reuses the active regional terrain material, whose world-space fog, woodland, resource lens and seed parameters are identical. The existing material factory remains the fallback when regional terrain/material is absent. Previously every close rebuild created and registered another shader/material in the strong-reference terrain_fog_materials list. This avoids that accumulation for the normal regional/close path and avoids duplicate shader creation.

Validation:17/17 close-job and map-onboarding tests pass. Editor import clean. Isolated GPU audit verifies close/regional material identity, unchanged registered-material count5 after three rebuilds, resource lens parameter propagation, cancellation and expanded triangle fingerprint2888042706. Reviewed artifacts/shared-after.png. No script/shader errors; pre-existing exit cleanup warnings persist. Baseline finalization51952us versus48740us after is a small single-run difference, not a demonstrated performance improvement. The repeated-build median250509us is also not a before/after benchmark.

Save compatibility:unchanged; geometry, shaders' code and game state untouched. This prevents redundant close-ground registrations, not all existing fog-material lifetime growth. Shared diagnostic shader mutations now affect both layers, as expected for shared resources. Next priority remains the vegetation/final mesh installation cost; no claim of seamlessness or photorealism. Original Google Earth references remain the scale guide; no imagery imported. Conflict risk limited to named installation assignment and audit block. Integrator owns merge and canonical launch.


## 2026-09-05: Invalidate vegetation when its location changes

Worker C:/Users/sjpur/tt-vegetation-location, codex/vegetation-location, base95a6b0db4f76a4cb461a49e86cf73cddb9abc5d1. Ground-material reuse integratedc101f42. Scope coordinated and kept isolated at integrator's request while the current delivery is closed.

The close vegetation cache previously returned whenever morphology_revision matched, even if the requested center changed. It now also requires identical X/Z center and world seed, plus a valid root not queued for deletion. Otherwise the existing deterministic vegetation builder regenerates the requested location. Unchanged requests still reuse their root. This avoids retaining trees from an earlier close-ground location; no change to sampling distributions, geometry or population state.
